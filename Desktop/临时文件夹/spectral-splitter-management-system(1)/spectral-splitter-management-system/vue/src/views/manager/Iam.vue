<template>
  <div>
    ...
    <div class="operation">
      <el-button type="primary" plain @click="handleAdd">添加装维</el-button>
    ...
    <el-dialog title="装维" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
...
export default {
  ...
  methods: {
    load(pageNum) {
      ...
      this.$request.get('/iam/selectPage', {
        params: {
    ...
    save() {
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            url: this.form.id ? '/iam/update' : '/iam/add',
    ...
    del(id) {
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/iam/delete/' + id).then(res => {
          if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/iam/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {
            this.$message.success('操作成功')
            this.load(1)
          } else {
