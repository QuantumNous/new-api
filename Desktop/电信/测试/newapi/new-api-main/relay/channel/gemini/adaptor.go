...
func (a *Adaptor) Init(info *relaycommon.RelayInfo) {
	// 去除 OpenRouter 风格的 provider 前缀（如 "google/gemini-..." → "gemini-..."）
	// Google Gemini API 不识别带 provider 前缀的模型名
	if idx := strings.Index(info.UpstreamModelName, "/"); idx >= 0 {
		info.UpstreamModelName = info.UpstreamModelName[idx+1:]
	}
}
