...
// isOpenRouterGeminiImageModel 判断 OpenRouter 渠道的上游模型名称是否为 Gemini 图片生成模型
// OpenRouter 的 Gemini 模型名通常带 google/ 前缀，如 google/gemini-3-pro-image-preview
func isOpenRouterGeminiImageModel(model string) bool {
	modelLower := strings.ToLower(model)
	if !strings.HasPrefix(modelLower, "google/gemini") {
		return false
	}
	return strings.Contains(modelLower, "image")
}
...
func (a *Adaptor) ConvertOpenAIRequest(c *gin.Context, info *relaycommon.RelayInfo, request *dto.GeneralOpenAIRequest) (any, error) {
	...
	if info.ChannelType == constant.ChannelTypeOpenRouter {
		...
		// 适配 OpenRouter 的 Gemini 图片生成模型
		// 当通过 OpenRouter 使用 Gemini 多模态图片模型时，需要注入 modalities 参数
		if isOpenRouterGeminiImageModel(info.UpstreamModelName) {
			// P0: 注入 modalities，如果用户未指定
			if len(request.Modalities) == 0 {
				request.Modalities = json.RawMessage(`["text","image"]`)
			}

			// P1: 从 extra_body.google.image_config 提取 image_config 到请求根层级
			// 然后清除 extra_body.google 避免上游报错
			// OpenRouter 要求 image_config 在请求根层级（与 messages 同级）
			if len(request.ExtraBody) > 0 {
				var extraBody map[string]interface{}
				if err := common.Unmarshal(request.ExtraBody, &extraBody); err == nil {
					if googleBody, ok := extraBody["google"]; ok {
						// 尝试从 google 中提取 image_config
						if googleMap, ok := googleBody.(map[string]interface{}); ok {
							if imageConfig, ok := googleMap["image_config"]; ok {
								// 将 image_config 提升到请求根层级
								// 同时将 Gemini camelCase 的 key 转为 OpenRouter 需要的 snake_case
								if configMap, ok := imageConfig.(map[string]interface{}); ok {
									snakeConfig := make(map[string]interface{})
									for k, v := range configMap {
										switch k {
										case "aspectRatio":
											snakeConfig["aspect_ratio"] = v
										case "imageSize":
											snakeConfig["image_size"] = v
										default:
											snakeConfig[k] = v // 已是 snake_case 或其他参数，直接保留
										}
									}
									imageConfigBytes, err := common.Marshal(snakeConfig)
									if err == nil {
										request.ImageConfig = imageConfigBytes
									}
								} else {
									// fallback: 直接序列化（可能已经是 snake_case）
						...
						// 清除 extra_body.google
						delete(extraBody, "google")
						if len(extraBody) == 0 {
							request.ExtraBody = nil
						} else {
							if remaining, err := common.Marshal(extraBody); err == nil {
								request.ExtraBody = remaining
							}
						}
					}
				}
