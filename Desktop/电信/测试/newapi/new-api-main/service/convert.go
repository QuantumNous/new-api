...
func GeminiToOpenAIRequest(geminiRequest *dto.GeminiChatRequest, info *relaycommon.RelayInfo) (*dto.GeneralOpenAIRequest, error) {
	...
	// 转换 Gemini responseModalities 到 OpenAI modalities
	if len(geminiRequest.GenerationConfig.ResponseModalities) > 0 {
		modalitiesLower := make([]string, len(geminiRequest.GenerationConfig.ResponseModalities))
		for i, m := range geminiRequest.GenerationConfig.ResponseModalities {
			modalitiesLower[i] = strings.ToLower(m)
		}
		modalitiesBytes, err := common.Marshal(modalitiesLower)
		if err == nil {
			openaiRequest.Modalities = modalitiesBytes
		}
	}

	// 转换 Gemini imageConfig 到 extra_body.google.image_config
	if len(geminiRequest.GenerationConfig.ImageConfig) > 0 {
		googleBody := map[string]interface{}{
			"image_config": json.RawMessage(geminiRequest.GenerationConfig.ImageConfig),
		}
...
func ResponseOpenAI2Gemini(openAIResponse *dto.OpenAITextResponse, info *relaycommon.RelayInfo) *dto.GeminiChatResponse {
	...
	for _, choice := range openAIResponse.Choices {
		...
		// 处理 OpenRouter 返回的 images 数组（Gemini 图片生成响应）
		// OpenRouter 格式: choices[0].message.images: [{"type":"image_url","image_url":{"url":"data:image/png;base64,..."}}]
		if len(choice.Message.Images) > 0 {
			...
			if err := json.Unmarshal(choice.Message.Images, &images); err == nil {
				for _, img := range images {
					if img.ImageURL.URL != "" {
						mimeType, base64Data := parseDataURL(img.ImageURL.URL)
						if base64Data != "" {
							part := dto.GeminiPart{
...
// parseDataURL 从 Data URL 中提取 MIME 类型和 Base64 数据
// 输入格式: "data:image/png;base64,iVBOR..."
// 返回: mimeType="image/png", base64Data="iVBOR..."
func parseDataURL(dataURL string) (mimeType string, base64Data string) {
	// 默认 MIME 类型
	mimeType = "image/png"

	if !strings.HasPrefix(dataURL, "data:") {
		// 不是 Data URL，假设整个字符串是 base64 数据
