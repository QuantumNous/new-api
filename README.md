...
| `CLAUDE_ARCHIVE_ENABLED` | Archive complete upstream request/response data for selected Claude models | `false` |
| `CLAUDE_ARCHIVE_DIR` | Claude archive directory (0700 directory, 0600 files) | `data/claude-archive` |
| `CLAUDE_ARCHIVE_MAX_FILE_MB` | Per-channel archive file limit; individual records are not split | `100` |
| `CLAUDE_ARCHIVE_SYNC` | Call fsync after each archive record | `false` |
