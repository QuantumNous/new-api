...
<p align="center">
  <strong>感謝 <a href="https://www.jetbrains.com/?from=new-api">JetBrains</a> 為本專案提供免費的開源開發許可證</strong>
</p>
...
> **💡 提示：** `-v ./data:/data` 會將資料保存在當前目錄的 `data` 資料夾中，你也可以改為絕對路徑如 `-v /your/custom/path:/data`
...
| 📡 介面文件 | [API 文件](https://docs.newapi.pro/zh/docs/api) |
...
> 詳細功能請參考 [功能說明](https://docs.newapi.pro/zh/docs/guide/wiki/basic-concepts/features-introduction)
...
- 🔄 **Google Gemini → OpenAI Compatible** - 僅支援文字，暫不支援函數呼叫
...
- `gemini-2.5-flash-nothinking` - 停用思考模式
...
> 詳情請參考 [介面文件 - 中繼介面](https://docs.newapi.pro/zh/docs/api)
...
<details>
<summary>查看完整介面列表</summary>

- [聊天介面 (Chat Completions)](https://docs.newapi.pro/zh/docs/api/ai-model/chat/openai/createchatcompletion)
- [回應介面 (Responses)](https://docs.newapi.pro/zh/docs/api/ai-model/chat/openai/createresponse)
- [圖片介面 (Image)](https://docs.newapi.pro/zh/docs/api/ai-model/images/openai/post-v1-images-generations)
- [音訊介面 (Audio)](https://docs.newapi.pro/zh/docs/api/ai-model/audio/openai/create-transcription)
- [影片介面 (Video)](https://docs.newapi.pro/zh/docs/api/ai-model/audio/openai/createspeech)
- [嵌入介面 (Embeddings)](https://docs.newapi.pro/zh/docs/api/ai-model/embeddings/createembedding)
- [重排序介面 (Rerank)](https://docs.newapi.pro/zh/docs/api/ai-model/rerank/creatererank)
...
| `STREAMING_TIMEOUT` | 串流超時時間（秒）                                                    | `300` |
| `STREAM_SCANNER_MAX_BUFFER_MB` | 串流掃描器單行最大緩衝（MB），產生圖片等超大 `data:` 片段（如 4K 圖片 base64）需適當調大 | `64` |
| `MAX_REQUEST_BODY_MB` | 請求body最大大小（MB，**解壓縮後**計；防止超大請求/zip bomb 導致記憶體暴漲），超過將回傳 `413` | `32` |
| `AZURE_DEFAULT_API_VERSION` | Azure API 版本                                                 | `2025-04-01-preview` |
| `ERROR_LOG_ENABLED` | 錯誤日誌開關                                                       | `false` |
| `PYROSCOPE_URL` | Pyroscope 服務位址                                            | - |
| `PYROSCOPE_APP_NAME` | Pyroscope 應用程式名稱                                        | `new-api` |
...
<details>
<summary><strong>方式 2：Docker 指令</strong></summary>
...
> **💡 路徑說明：**
> - `./data:/data` - 相對路徑，資料保存在當前目錄的 data 資料夾
...
如果您所在的組織政策不允許使用 AGPLv3 許可的軟體，或您希望規避 AGPLv3 的開源義務，請寄送郵件至：[support@quantumnous.com](mailto:support@quantumnous.com)
