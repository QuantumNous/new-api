...
func GetAudioDuration(ctx context.Context, f io.ReadSeeker, ext string) (duration float64, err error) {
	SysLog(fmt.Sprintf(Translate(DefaultLang, "common.getaudioduration_ext"), ext))
	// 根据文件扩展名选择解析器
	switch ext {
	case ".mp3":
	...
	default:
		return 0, fmt.Errorf(Translate(DefaultLang, "common.unsupported_audio_format"), ext)
	}
	SysLog(fmt.Sprintf(Translate(DefaultLang, "common.getaudioduration_duration"), duration))
	return duration, err
}
...
func getWAVDuration(r io.ReadSeeker) (float64, error) {
	...
	if !dec.IsValidFile() {
		return 0, errors.New(Translate(DefaultLang, "common.invalid_wav_file"))
	}
	...
	if sampleRate == 0 || numChans == 0 || bitDepth == 0 {
		return 0, errors.New(Translate(DefaultLang, "common.invalid_wav_header_metadata"))
	}

	bytesPerFrame := numChans * (bitDepth / 8)
	if bytesPerFrame == 0 {
		return 0, errors.New(Translate(DefaultLang, "common.invalid_byte_depth_calculation"))
	}
...
func getAIFFDuration(r io.ReadSeeker) (float64, error) {
	...
	if !dec.IsValidFile() {
		return 0, errors.New(Translate(DefaultLang, "common.invalid_aiff_file"))
	}
...
func getWebMDuration(r io.ReadSeeker) (float64, error) {
	...
	if n > 0 {
		...
		if len(buf) >= 4 && binary.BigEndian.Uint32(buf[0:4]) == 0x1A45DFA3 {
			// 这是一个有效的 EBML 文件
			// 但完整解析需要更复杂的逻辑
			return 0, errors.New(Translate(DefaultLang, "common.webm_duration_parsing_requires_full_ebml_parser_consider"))
		}
	}

	return 0, errors.New(Translate(DefaultLang, "common.failed_to_parse_webm_file"))
}
...
func getAACDuration(r io.ReadSeeker) (float64, error) {
	...
	if sampleRate == 0 || totalFrames == 0 {
		return 0, errors.New(Translate(DefaultLang, "common.no_valid_aac_frames_found"))
	}
