...
func newDiskStorage(data []byte, cachePath string) (*diskStorage, error) {
	...
	if err != nil {
		file.Close()
		os.Remove(filePath)
		return nil, fmt.Errorf(Translate(DefaultLang, "common.failed_to_write_to_temp_file"), err)
	}
	...
	if _, err := file.Seek(0, io.SeekStart); err != nil {
		file.Close()
		os.Remove(filePath)
		return nil, fmt.Errorf(Translate(DefaultLang, "common.failed_to_seek_temp_file"), err)
	}
...
func CreateBodyStorage(data []byte) (BodyStorage, error) {
	...
	if IsDiskCacheEnabled() &&
		...
		if err != nil {
			// 如果磁盘存储失败，回退到内存存储
			SysError(fmt.Sprintf(Translate(DefaultLang, "common.failed_to_create_disk_storage_falling_back_to"), err))
...
func CreateBodyStorageFromReader(reader io.Reader, contentLength int64, maxBytes int64) (BodyStorage, error) {
	...
	if IsDiskCacheEnabled() &&
		...
		if err != nil {
			...
			return nil, fmt.Errorf(Translate(DefaultLang, "common.disk_storage_creation_failed"), err)
		}
