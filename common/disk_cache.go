...
func CreateDiskCacheFile(cacheType DiskCacheType) (string, *os.File, error) {
	if err := EnsureDiskCacheDir(); err != nil {
		return "", nil, fmt.Errorf(Translate(DefaultLang, "common.failed_to_create_cache_directory"), err)
	}
	...
	if err != nil {
		return "", nil, fmt.Errorf(Translate(DefaultLang, "common.failed_to_create_cache_file"), err)
	}
...
func WriteDiskCacheFile(cacheType DiskCacheType, data []byte) (string, error) {
	...
	if err != nil {
		file.Close()
		os.Remove(filePath)
		return "", fmt.Errorf(Translate(DefaultLang, "common.failed_to_write_cache_file"), err)
	}

	if err := file.Close(); err != nil {
		os.Remove(filePath)
		return "", fmt.Errorf(Translate(DefaultLang, "common.failed_to_close_cache_file"), err)
	}
