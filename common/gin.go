...
func GetRequestBody(c *gin.Context) (io.Seeker, error) {
	...
	if storage, exists := c.Get(KeyBodyStorage); exists && storage != nil {
		if bs, ok := storage.(BodyStorage); ok {
			if _, err := bs.Seek(0, io.SeekStart); err != nil {
				return nil, fmt.Errorf(Translate(DefaultLang, "common.failed_to_seek_body_storage"), err)
			}
			return bs, nil
		}
	}
	...
	if err != nil {
		if IsRequestBodyTooLargeError(err) {
			return nil, errors.Wrap(ErrRequestBodyTooLarge, fmt.Sprintf(Translate(DefaultLang, "common.request_body_exceeds_mb"), maxMB))
		}
		return nil, err
	}
...
func GetBodyStorage(c *gin.Context) (BodyStorage, error) {
	...
	if !ok {
		return nil, errors.New(Translate(DefaultLang, "common.unexpected_body_storage_type"))
	}
	return bs, nil
}
...
var Translate func(lang, key string, args ...map[string]any) string

// DefaultLang mirrors i18n.DefaultLang so callers that cannot import i18n
// still have access to the runtime default language.
...
func init() {
	...
	Translate = func(lang, key string, args ...map[string]any) string {
		return key
	}
	// Initialize error variables that depend on Translate
	ErrRequestBodyTooLarge = errors.New(Translate(DefaultLang, "common.request_body_too_large"))
	ErrStorageClosed = errors.New(Translate(DefaultLang, "common.body_storage_is_closed"))
	errBoundaryNotFound = errors.New(Translate(DefaultLang, "common.multipart_boundary_not_found"))
}
