...
func parsePortRanges(portConfigs []string) ([]int, error) {
	...
	for _, config := range portConfigs {
		...
		if strings.Contains(config, "-") {
			...
			if len(parts) != 2 {
				return nil, fmt.Errorf(Translate(DefaultLang, "common.invalid_port_range_format"), config)
			}

			startPort, err := strconv.Atoi(strings.TrimSpace(parts[0]))
			if err != nil {
				return nil, fmt.Errorf(Translate(DefaultLang, "common.invalid_start_port_in_range"), config, err)
			}

			endPort, err := strconv.Atoi(strings.TrimSpace(parts[1]))
			if err != nil {
				return nil, fmt.Errorf(Translate(DefaultLang, "common.invalid_end_port_in_range"), config, err)
			}

			if startPort > endPort {
				return nil, fmt.Errorf(Translate(DefaultLang, "common.invalid_port_range_start_port_cannot_be_greater"), config)
			}

			if startPort < 1 || startPort > 65535 || endPort < 1 || endPort > 65535 {
				return nil, fmt.Errorf(Translate(DefaultLang, "common.port_range_contains_invalid_port_numbers_must_be"), config)
			}
			...
			if err != nil {
				return nil, fmt.Errorf(Translate(DefaultLang, "common.invalid_port_number"), config)
			}

			if port < 1 || port > 65535 {
				return nil, fmt.Errorf(Translate(DefaultLang, "common.invalid_port_number_must_be_1_65535"), port)
			}
...
func (p *SSRFProtection) ValidateURL(urlStr string) error {
	...
	if err != nil {
		return fmt.Errorf(Translate(DefaultLang, "common.invalid_url_format_b201"), err)
	}
	...
	if err != nil {
		return fmt.Errorf(Translate(DefaultLang, "common.invalid_port"), portStr)
	}

	if !p.isAllowedPort(port) {
		return fmt.Errorf(Translate(DefaultLang, "common.port_is_not_allowed"), port)
	}
	...
	if ip := net.ParseIP(host); ip != nil {
		if !p.IsIPAccessAllowed(ip) {
			if isPrivateIP(ip) {
				return fmt.Errorf(Translate(DefaultLang, "common.private_ip_address_not_allowed"), ip.String())
			}
			if p.IpFilterMode {
				return fmt.Errorf(Translate(DefaultLang, "common.ip_not_in_whitelist"), ip.String())
			}
			return fmt.Errorf(Translate(DefaultLang, "common.ip_in_blacklist"), ip.String())
		}
		return nil
	}
	...
	if !p.isDomainAllowed(host) {
		if p.DomainFilterMode {
			return fmt.Errorf(Translate(DefaultLang, "common.domain_not_in_whitelist"), host)
		}
		return fmt.Errorf(Translate(DefaultLang, "common.domain_in_blacklist"), host)
	}
	...
	if err != nil {
		return fmt.Errorf(Translate(DefaultLang, "common.dns_resolution_failed_for"), host, err)
	}
	for _, ip := range ips {
		if !p.IsIPAccessAllowed(ip) {
			if isPrivateIP(ip) && !p.AllowPrivateIp {
				return fmt.Errorf(Translate(DefaultLang, "common.private_ip_address_not_allowed_resolves_to"), host, ip.String())
			}
			if p.IpFilterMode {
				return fmt.Errorf(Translate(DefaultLang, "common.ip_not_in_whitelist_resolves_to"), host, ip.String())
			}
			return fmt.Errorf(Translate(DefaultLang, "common.ip_in_blacklist_resolves_to"), host, ip.String())
		}
	}
	return nil
}
...
func ValidateURLWithFetchSetting(urlStr string, enableSSRFProtection, allowPrivateIp bool, domainFilterMode bool, ipFilterMode bool, domainList, ipList, allowedPorts []string, applyIPFilterForDomain bool) error {
	...
	if err != nil {
		return fmt.Errorf(Translate(DefaultLang, "common.request_reject_invalid_port_configuration"), err)
	}
