...
| `user.go` | 用户注册/登录/登出/信息修改，支持密码登录、2FA、Passkey；注册时支持归因跟踪：`AdsAttribution` 字段通过 `sanitizeAdsAttribution` 清洗后随用户一同写入 DB（广告漏斗追踪）；`google_id` 字段已纳入管理员更新用户接口；错误通过 `i18n` 国际化返回 |
...
| `topup_stripe.go` / `topup_creem.go` / `topup_paddle.go` / `topup_waffo.go` / `topup_waffo_pancake.go` | 各支付渠道充值 handler（Stripe / Creem / Paddle / Waffo / Waffo-Pancake）；`topup_stripe.go` 还负责开票相关功能：`StripePayRequest` 新增 `InvoiceRequested`/`InvoiceProfile` 字段，`RequestStripePay` 在支付时可同步创建 `PaymentInvoice` 快照；`RequestStripeTopUpInvoice` 支持已支付订单的补开发票（创建/更新 Stripe Invoice 并回写状态） |
...
| `blog.go` | 远程 CMS 博客接口：`GetBlogList`（分页列表，支持关键字搜索和分类过滤）、`GetBlogPost`（按 slug 获取详情），委托 `service.FetchBlogList` / `service.FetchBlogPost` 完成远程调用 |
| `invoice.go` | Stripe 发票资料管理：`GetSelfInvoiceProfile` / `UpdateSelfInvoiceProfile`（用户自助读写）、`AdminGetUserInvoiceProfile` / `AdminUpdateUserInvoiceProfile`（管理员操作），包含 `validateInvoiceProfile` 字段校验（公司名、邮箱格式、国家必填）；`normalizeInvoiceProfileForRequest` 负责标准化 TaxIDType（小写）和 Country（大写） |
| `model_meta.go` | 模型元数据（ModelMeta）管理，供前端展示模型详情；`enrichModels` 批量填充端点类型、绑定渠道、可用分组、配额类型及模型可用性状态（`GetModelAvailabilityStateMap`），精确/规则匹配两条路径分别处理，规则匹配通过定价缓存内存批量匹配避免 N+1 查询 |
...
| `model_availability_task.go` | 「检测上游不支持模型」后台任务：`StartModelAvailabilityDetectionTask`（master 节点单例，每日定时触发）按模型逐个发送 ping-pong 探针请求（`probeOneModelAvailability`），将结果分为 `available` / `temporary_failure` / `official_unsupported` / `unknown_failure` 四类（`classifyModelProbeError`），写入 `ModelAvailabilityState` 表；可通过 `MODEL_AVAILABILITY_CHECK_ENABLED`、`MODEL_AVAILABILITY_CHECK_TIME`、`MODEL_AVAILABILITY_CHECK_INTERVAL_SECONDS`、`MODEL_AVAILABILITY_OFFICIAL_UNSUPPORTED_THRESHOLD` 环境变量配置 |
| `usage_reconciliation.go` | BlockRun 对账数据接口：`GetUsageSummary`（按时间段汇总 token 用量和费用，按模型/API Key 分组）、`GetUsageTransactions`（分页明细流水，含缓存 token、channel 信息、stream 状态）、`GetUsageValidation`（按模型+渠道维度交叉验证）、`GetUsageModels`（返回所有启用模型的定价信息），全部挂载在 `/usage` 路由组（非 `/api`），使用静态 token 鉴权 |
...
| `oauth.go` | 统一 OAuth 入口：`GenerateOAuthCode`（生成 CSRF state，同时将 `aff`/`ads_attribution` 存入 session）、`HandleOAuth`（通用回调，支持 GitHub/Discord/OIDC/LinuxDO/Google 等所有注册到 `oauth.GetProvider` 的提供商）、`handleOAuthBind`（绑定已登录账号）、`findOrCreateOAuthUser`（查找或创建用户，含 GitHub login→numeric ID 迁移逻辑，新建用户时回写 `AdsAttribution`） |
| `github.go` / `discord.go` / `wechat.go` / `oidc.go` / `linuxdo.go` / `telegram.go` / `custom_oauth.go` | 各 OAuth 提供商登录/绑定回调 handler（WeChat/Telegram 因协议差异保留独立路由，其余通过 `/oauth/:provider` 统一路由分发） |
