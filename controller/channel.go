...
func UpdateChannel(c *gin.Context) {
	...
	pluginBindErr := errors.New("task plugin channels require the task_plugin.bind permission")
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if channel.Type == constant.ChannelTypeTaskPlugin && !authz.Can(c.GetInt("id"), c.GetInt("role"), authz.TaskPluginBind) {
			return pluginBindErr
		}
	...
	if errors.Is(err, pluginBindErr) {
		common.ApiErrorMsg(c, pluginBindErr.Error())
		return
	}
...
// FetchModelsRequest supports saved and unsaved model discovery previews.
