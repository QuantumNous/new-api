...
func FetchUpstreamModels(c fuego.ContextNoBody) (dto.ApiResponse, error) {
	...
	if err != nil {
		return dto.FailAny(fmt.Sprintf("获取模型列表失败: %s", err.Error()))
	}
...
func AddChannel(c fuego.ContextWithBody[AddChannelRequest]) (dto.MessageResponse, error) {
	...
	if addChannelRequest.Channel == nil {
		return dto.FailMsg("channel cannot be empty")
	}
...
func EditTagChannels(c fuego.ContextWithBody[ChannelTag]) (dto.MessageResponse, error) {
	...
	if channelTag.ParamOverride != nil {
		...
		if trimmed != "" && common.Unmarshal([]byte(trimmed), new(any)) != nil {
			...
			if originChannel.Key != "" {
				...
				if strings.HasPrefix(strings.TrimSpace(originChannel.Key), "[") {
					...
					if err := common.Unmarshal([]byte(strings.TrimSpace(originChannel.Key)), &arr); err == nil {
						existingKeys = make([]string, 0, len(arr))
						for _, v := range arr {
							b, err := common.Marshal(v)
							if err != nil {
								continue
							}
							existingKeys = append(existingKeys, string(b))
						}
					}
				} else {
					// 换行分隔格式
