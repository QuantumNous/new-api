...
func buildUpstreamModelUpdateTaskNotificationContent(
	...
	builder.WriteString(fmt.Sprintf(
		"Upstream model inspection summary: checked %d channels, detected changes in %d channels, added %d models, removed %d models, auto-synced %d added models, failed channels %d.",
		checkedChannels,
		changedChannels,
		detectedAddModels,
	...
	if len(channelSummaries) > 0 {
		...
		if len(channelSummaries) > displayCount {
			builder.WriteString(fmt.Sprintf("\n- Omitted remaining %d channels", len(channelSummaries)-displayCount))
		}
	}
	...
	if len(normalizedAddModelSamples) > 0 {
		...
		if len(normalizedAddModelSamples) > displayCount {
			builder.WriteString(fmt.Sprintf(" (omitted remaining %d)", len(normalizedAddModelSamples)-displayCount))
		}
	}
	...
	if len(normalizedRemoveModelSamples) > 0 {
		...
		if len(normalizedRemoveModelSamples) > displayCount {
			builder.WriteString(fmt.Sprintf(" (omitted remaining %d)", len(normalizedRemoveModelSamples)-displayCount))
		}
	}
	...
	if failedChannels > 0 {
		...
		if failedChannels > displayCount {
			builder.WriteString(fmt.Sprintf(" (omitted remaining %d)", failedChannels-displayCount))
		}
	}
