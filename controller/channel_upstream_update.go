...
func fetchChannelUpstreamModelIDs(channel *model.Channel) ([]string, error) {
	...
	if channel.Type == constant.ChannelTypeGemini {
		...
		if apiErr != nil {
			return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "ctrl.failed_to_get_channel_key"), apiErr)
		}
...
func refreshChannelRuntimeCache() {
	if common.MemoryCacheEnabled {
		func() {
			defer func() {
				if r := recover(); r != nil {
					common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "ctrl.initchannelcache_panic"), r))
				}
			}()
...
func buildUpstreamModelUpdateTaskNotificationContent(
	...
	builder.WriteString(i18n.Translate(i18n.DefaultLang, "channel.upstream_summary", map[string]any{
	...
	if len(channelSummaries) > 0 {
		...
		builder.WriteString(i18n.Translate(i18n.DefaultLang, "channel.upstream_changed_details", map[string]any{"Display": displayCount, "Total": len(channelSummaries)}))
		...
		if len(channelSummaries) > displayCount {
			builder.WriteString(i18n.Translate(i18n.DefaultLang, "channel.upstream_more_omitted", map[string]any{"Count": len(channelSummaries) - displayCount}))
		}
	}
	...
	if len(normalizedAddModelSamples) > 0 {
		...
		builder.WriteString(i18n.Translate(i18n.DefaultLang, "channel.upstream_added_samples", map[string]any{
		...
		if len(normalizedAddModelSamples) > displayCount {
			builder.WriteString(i18n.Translate(i18n.DefaultLang, "channel.upstream_added_omitted", map[string]any{"Count": len(normalizedAddModelSamples) - displayCount}))
		}
	}
	...
	if len(normalizedRemoveModelSamples) > 0 {
		...
		builder.WriteString(i18n.Translate(i18n.DefaultLang, "channel.upstream_removed_samples", map[string]any{
		...
		if len(normalizedRemoveModelSamples) > displayCount {
			builder.WriteString(i18n.Translate(i18n.DefaultLang, "channel.upstream_removed_omitted", map[string]any{"Count": len(normalizedRemoveModelSamples) - displayCount}))
		}
	}
	...
	if failedChannels > 0 {
		...
		builder.WriteString(i18n.Translate(i18n.DefaultLang, "channel.upstream_failed_ids", map[string]any{
			"Display": displayCount, "Total": failedChannels, "Ids": strings.Join(displayIDs, ", "),
		}))
		if failedChannels > displayCount {
			builder.WriteString(i18n.Translate(i18n.DefaultLang, "channel.upstream_failed_omitted", map[string]any{"Count": failedChannels - displayCount}))
		}
	}
...
func runChannelUpstreamModelUpdateTaskOnce() {
	...
	for {
		...
		if err != nil {
			common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "ctrl.upstream_model_update_task_query_failed"), err))
			break
		}
		if len(channels) == 0 {
			break
		...
		for _, channel := range channels {
			...
			if err != nil {
				...
				common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "ctrl.upstream_model_update_check_failed_channel_id"), channel.Id, channel.Name, err))
				continue
			}
	...
	if changedChannels > 0 || failedChannels > 0 {
		...
		service.NotifyUpstreamModelUpdateWatchers(
			i18n.Translate(i18n.DefaultLang, "channel.upstream_notify_subject"),
...
func StartChannelUpstreamModelUpdateTask() {
	channelUpstreamModelUpdateTaskOnce.Do(func() {
		...
		if !common.GetEnvOrDefaultBool("CHANNEL_UPSTREAM_MODEL_UPDATE_TASK_ENABLED", true) {
			common.SysLog(i18n.Translate(i18n.DefaultLang, "ctrl.upstream_model_update_task_disabled_by_channel"))
			return
		}
		...
		go func() {
			common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "ctrl.upstream_model_update_task_started_interval"), interval))
...
func ApplyChannelUpstreamModelUpdates(c *gin.Context) {
	...
	if req.ID <= 0 {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": i18n.Translate(i18n.DefaultLang, "distributor.invalid_channel_id"),
		})
		return
	}
