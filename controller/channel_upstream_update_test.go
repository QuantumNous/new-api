...
func TestFailedAdvancedCustomDetectionDoesNotStageFullRemoval(t *testing.T) {
	...
	modelsChanged, autoAdded, err := checkAndPersistChannelUpstreamModelUpdates(channel, &settings, true, true, false)
...
func TestApplyChannelUpstreamModelUpdatesRejectsChangedAdvancedCustomRoute(t *testing.T) {
	db := setupChannelUpstreamUpdateTestDB(t)
	config := &dto.AdvancedCustomConfig{Routes: []dto.AdvancedCustomRoute{{IncomingPath: dto.AdvancedCustomModelListPath, UpstreamPath: "/old/models", Converter: "none"}}}
	settings := dto.ChannelOtherSettings{AdvancedCustom: config}
	channel := createChannelUpstreamUpdateTestChannel(t, db, "gpt-4o", nil, settings)
	channel.Type = constant.ChannelTypeAdvancedCustom
	require.NoError(t, db.Model(channel).Update("type", channel.Type).Error)
	var newSettings string
	stubChannelUpstreamModelFetch(t, func(_ *model.Channel) ([]string, error) {
		fresh := channel.GetOtherSettings()
		fresh.AdvancedCustom.Routes[0].UpstreamPath = "/new/models"
		data, err := common.Marshal(fresh)
		require.NoError(t, err)
		newSettings = string(data)
		require.NoError(t, db.Model(&model.Channel{}).Where("id = ?", channel.Id).Update("settings", newSettings).Error)
		return []string{"gpt-4o", "new-model"}, nil
	})
	_, _, _, _, _, err := applyChannelUpstreamModelUpdates(channel, []string{"new-model"}, nil, nil)
	require.ErrorContains(t, err, "channel fetch configuration changed")
	persisted, abilities := loadChannelUpstreamUpdateTestState(t, db, channel.Id)
	assert.Equal(t, "gpt-4o", persisted.Models)
	assert.JSONEq(t, newSettings, persisted.OtherSettings)
	require.Len(t, abilities, 1)
	assert.Equal(t, "gpt-4o", abilities[0].Model)
}
