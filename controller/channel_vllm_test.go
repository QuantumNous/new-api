...
func TestVLLMStatusPartialFailureAndCredentials(t *testing.T) {
	...
	status, err := fetchVLLMStatus(context.Background(), channel)
	...
	assert.Equal(t, vllmEndpointStatus{Status: 403, Error: "http_error"}, status.Endpoints["/metrics"])
...
func TestVLLMStatusRejectsInvalidTargetsAndRedirects(t *testing.T) {
	...
	for _, base := range []string{"", "file:///etc/passwd", "http://user:secret@localhost", "http://localhost?key=secret"} {
		_, err := fetchVLLMStatus(context.Background(), &model.Channel{Type: constant.ChannelTypeVLLM, BaseURL: &base})
		require.Error(t, err)
		assert.NotContains(t, err.Error(), "secret")
	}
	_, err := fetchVLLMStatus(context.Background(), &model.Channel{Type: constant.ChannelTypeOllama})
	require.Error(t, err)
	...
	defer upstream.Close()
	status, err := fetchVLLMStatus(context.Background(), &model.Channel{Type: constant.ChannelTypeVLLM, BaseURL: &upstream.URL, Key: "EMPTY"})
	require.NoError(t, err)
	for _, endpoint := range status.Endpoints {
		assert.Equal(t, vllmEndpointStatus{Status: 302, Error: "http_error"}, endpoint)
	}
}
...
func TestVLLMStatusInvalidAndOversizedResponses(t *testing.T) {
	...
	cancel()
	_, err = fetchVLLMStatus(ctx, channel)
	assert.ErrorIs(t, err, context.Canceled)
}

func TestParseVLLMMetricsPreservesSeriesAndHistogramTotals(t *testing.T) {
	metrics, err := parseVLLMMetrics(`# TYPE vllm:num_requests_running gauge
...
`)
	require.NoError(t, err)
	assert.ElementsMatch(t, []vllmMetric{
	...
	_, err = parseVLLMMetrics("vllm:broken{engine=oops} 1\n")
	require.Error(t, err)
}
...
func TestVLLMStatusLive(t *testing.T) {
	...
	service.InitHttpClient()
	status, err := fetchVLLMStatus(context.Background(), &model.Channel{
