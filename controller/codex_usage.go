...
func GetCodexChannelUsage(c *gin.Context) {
	...
	if err != nil {
		common.SysError(i18n.Translate("ctrl.failed_to_fetch_codex_usage") + err.Error())
	...
	if (statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden) && strings.TrimSpace(oauthKey.RefreshToken) != "" {
		...
		if refreshErr == nil {
			...
			if err != nil {
				common.SysError(i18n.Translate("ctrl.failed_to_fetch_codex_usage_after_refresh") + err.Error())
				c.JSON(http.StatusOK, gin.H{"success": false, "message": common.TranslateMessage(c, "common.retry_later")})
				return
			}
		}
