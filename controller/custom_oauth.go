...
func GetCustomOAuthProvider(c *gin.Context) {
	...
	if err != nil {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "common.invalid_id"))
		return
	}

	provider, err := model.GetCustomOAuthProviderById(id)
	if err != nil {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.not_found"))
		return
	}
...
func FetchCustomOAuthDiscovery(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.invalid_params")+": "+err.Error())
		return
	}
	...
	if wellKnownURL == "" && issuerURL == "" {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.discovery_empty"))
		return
	}
	...
	if err != nil || parsedURL.Host == "" || (parsedURL.Scheme != "http" && parsedURL.Scheme != "https") {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.discovery_invalid"))
		return
	}
	...
	if err != nil {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.discovery_req_failed")+": "+err.Error())
		return
	}
	...
	if err != nil {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.discovery_get_failed")+": "+err.Error())
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.discovery_get_failed")+": "+message)
		return
	}

	var discovery map[string]any
	if err = common.DecodeJson(resp.Body, &discovery); err != nil {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.discovery_parse_failed")+": "+err.Error())
		return
	}
...
func CreateCustomOAuthProvider(c *gin.Context) {
	...
	if model.IsSlugTaken(req.Slug, 0) {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.slug_exists"))
		return
	}

	// Check if slug conflicts with built-in providers
	if oauth.IsProviderRegistered(req.Slug) && !oauth.IsCustomProvider(req.Slug) {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.slug_conflict"))
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": common.TranslateMessage(c, "common.create_success"),
		"data":    toCustomOAuthProviderResponse(provider),
	})
}
...
func UpdateCustomOAuthProvider(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": common.TranslateMessage(c, "common.update_success"),
		"data":    toCustomOAuthProviderResponse(provider),
	})
}
...
func DeleteCustomOAuthProvider(c *gin.Context) {
	...
	if err != nil {
		common.SysError(i18n.Translate(i18n.DefaultLang, "ctrl.failed_to_get_binding_count_for_provider") + strconv.Itoa(id) + ": " + err.Error())
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.check_binding_failed"))
		return
	}
	if count > 0 {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.has_bindings_delete"))
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": common.TranslateMessage(c, "common.delete_success"),
	})
}
...
func GetUserOAuthBindings(c *gin.Context) {
	userId := c.GetInt("id")
	if userId == 0 {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "common.not_logged_in"))
		return
	}
...
func GetUserOAuthBindingsByAdmin(c *gin.Context) {
	...
	if myRole <= targetUser.Role && myRole != common.RoleRootUser {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "user.no_permission_same_level"))
		return
	}
...
func UnbindCustomOAuth(c *gin.Context) {
	...
	if err != nil {
		common.ApiErrorMsg(c, common.TranslateMessage(c, "custom_oauth.invalid_provider_id"))
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": common.TranslateMessage(c, "custom_oauth.unbind_success"),
	})
}
