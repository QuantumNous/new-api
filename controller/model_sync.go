...
// SyncUpstreamPreviewResponse exposes the versioned catalog selection preview.
...
func fetchMetadataCatalog(c *gin.Context, locale string) (SyncUpstreamSource, map[string]model.MetadataValues, map[string]model.Vendor, error) {
	resolved, valid := normalizeLocale(locale)
	if !valid {
		return SyncUpstreamSource{}, nil, nil, errors.New("unsupported metadata language")
	}
	modelsURL, vendorsURL := getUpstreamURLs(resolved)
	source := SyncUpstreamSource{Locale: resolved, ModelsURL: modelsURL, VendorsURL: vendorsURL}
...
func SyncUpstreamPreview(c *gin.Context) {
	...
	common.ApiSuccess(c, SyncUpstreamPreviewResponse{Source: source, Candidates: candidates})
}
