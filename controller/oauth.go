...
func HandleOAuth(c *gin.Context) {
	...
	if provider == nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": i18n.T(c, "oauth.unknown_provider"),
		})
		return
	}
	...
	if state == "" || session.Get("oauth_state") == nil || state != session.Get("oauth_state").(string) {
		c.JSON(http.StatusForbidden, gin.H{
			"success": false,
			"message": i18n.T(c, "oauth.state_invalid"),
		})
		return
	}
	...
	if legacyID, ok := oauthUser.Extra["legacy_id"].(string); ok && legacyID != "" {
		if provider.IsUserIDTaken(legacyID) {
			...
			if user.Id != 0 {
				...
				common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "ctrl.oauth_migrating_user_rom_legacy_id_o_new"),
					user.Id, legacyID, oauthUser.ProviderUserID))
				if err := user.UpdateGitHubId(oauthUser.ProviderUserID); err != nil {
					common.SysError(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "ctrl.oauth_failed_to_migrate_user"), user.Id, err.Error()))
