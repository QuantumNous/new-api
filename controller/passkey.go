...
func PasskeyRegisterBegin(c *gin.Context) {
	if !system_setting.GetPasskeySettings().Enabled {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": i18n.T(c, i18n.MsgPasskeyNotEnabled),
		})
		return
	}
...
func PasskeyLoginFinish(c *gin.Context) {
	...
	handler := func(rawID, userHandle []byte) (webauthnlib.User, error) {
		...
		if user.Status != common.UserStatusEnabled {
			return nil, errors.New(i18n.MsgUserDisabled)
		}
		...
		if len(userHandle) > 0 {
			...
			if parseErr != nil {
				...
				return nil, errors.New("passkey user handle does not match credential")
			}
		}
...
func AdminResetPasskey(c *gin.Context) {
	...
	if _, err := model.GetPasskeyByUserID(user.Id); err != nil {
		if errors.Is(err, model.ErrPasskeyNotFound) {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": i18n.T(c, i18n.MsgPasskeyNotBound),
			})
			return
		}
		common.ApiError(c, err)
...
func getSessionUser(c *gin.Context) (*model.User, error) {
	...
	if idRaw == nil {
		return nil, errors.New(i18n.MsgUnauthorized)
	}
	id, ok := idRaw.(int)
	if !ok {
