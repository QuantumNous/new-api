...
func UniversalVerify(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		common.ApiErrorI18n(c, i18n.MsgInvalidParams, map[string]any{"Error": err.Error()})
		return
	}
	...
	if !has2FA && !hasPasskey {
		common.ApiErrorI18n(c, i18n.MsgSecureNotEnabled)
		return
	}
	...
	case "passkey":
		...
		if !verified {
			common.ApiErrorI18n(c, i18n.MsgSecurePasskeyFinishReq)
			return
		}
		verifyMethod = "Passkey"

	default:
		common.ApiErrorI18n(c, i18n.MsgSecureMethodUnsupported, map[string]any{"Method": req.Method})
		return
	}
	...
	if err != nil {
		common.ApiErrorI18n(c, i18n.MsgSecureSessionSaveFailed, map[string]any{"Error": err.Error()})
		return
	}
...
func consumePasskeyReady(c *gin.Context) (bool, error) {
	...
	if !ok {
		session.Delete(PasskeyReadySessionKey)
		_ = session.Save()
		return false, fmt.Errorf("invalid passkey ready state")
	}
	session.Delete(PasskeyReadySessionKey)
	if err := session.Save(); err != nil {
		return false, err
