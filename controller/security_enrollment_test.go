...
func TestSecurityEnrollmentMissingTargetsAreBusinessErrors(t *testing.T) {
	...
	for _, target := range []struct {
		path, key                     string
		handler                       gin.HandlerFunc
		notFoundStatus, invalidStatus int
	}{
		{"/api/channel/999/key", i18n.MsgChannelNotExists, GetChannelKey, http.StatusNotFound, http.StatusBadRequest},
		{"/api/user/999/2fa", i18n.MsgUserNotExists, AdminDisable2FA, http.StatusOK, http.StatusOK},
	} {
		...
		assert.Equal(t, target.notFoundStatus, response.Code)
		var body securityEnrollmentResponse
		require.NoError(t, common.Unmarshal(response.Body.Bytes(), &body))
		assert.False(t, body.Success)
		...
		for _, id := range []string{"invalid", "0", "-1"} {
			...
			assert.Equal(t, target.invalidStatus, invalid.Code)
