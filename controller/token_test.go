...
func verifyAPITokenAudit(t *testing.T) {
	...
	tokenRoutes.POST("/", func(c *gin.Context) {
		if c.GetHeader("X-Test-Large-Error") != "" {
			c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"success": false, "message": strings.Repeat("x", 64*1024+1)})
			return
		}
		c.Next()
	}, AddToken)
	...
	for index, tc := range []struct {
		...
		{name: "create", status: 201, method: "POST", path: "/", body: `{"name":"created","expired_time":-1,"unlimited_quota":true}`, action: "token.create", success: true},
		{name: "invalid create", status: 400, method: "POST", path: "/", body: `{"name":"attempt","remain_quota":-1}`, action: "token.create", params: `{"name":"attempt"}`},
		{name: "malformed body", status: 400, method: "POST", path: "/", body: `{"key":"raw-body-secret"`, action: "token.create", params: `{}`},
		{name: "failed response exceeds audit buffer", status: 400, largeError: true, method: "POST", path: "/", body: `{}`, action: "token.create", params: `{}`},
		{name: "create storage failure", status: 500, method: "POST", path: "/", body: `{"name":"attempt","unlimited_quota":true}`, action: "token.create", params: `{"name":"attempt"}`, failWrite: true},
		...
		{name: "update storage failure", status: 500, method: "PUT", path: "/", body: `{"id":$id,"name":"failed-rename","unlimited_quota":true}`, action: "token.update", params: `{"id":$id,"name":"owned"}`, failWrite: true},
		{name: "foreign update", status: 404, method: "PUT", path: "/", body: `{"id":$other,"name":"forged-name","unlimited_quota":true}`, action: "token.update", params: `{"id":$other}`},
		...
		{name: "expired enable", status: 409, method: "PUT", path: "/?status_only=true", body: `{"id":$id,"status":1}`, action: "token.status_update", params: `{"id":$id,"name":"owned"}`, initialStatus: common.TokenStatusExpired},
		{name: "delete", method: "DELETE", path: "/$id", action: "token.delete", success: true, params: `{"id":$id,"name":"owned"}`},
		{name: "foreign delete", status: 404, method: "DELETE", path: "/$other", action: "token.delete", params: `{"id":$other}`},
		{name: "missing delete", status: 404, method: "DELETE", path: "/999999", action: "token.delete", params: `{"id":999999}`},
		...
		{name: "foreign key view", status: 404, method: "POST", path: "/$other/key", action: "token.key_view", params: `{"id":$other}`},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.largeError {
				request.Header.Set("X-Test-Large-Error", "1")
			}
