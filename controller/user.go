...
func Register(c *gin.Context) {
	...
	if singlePrimary && inviteToken != "" {
		...
		if inviteFlow.Payload != "" {
			var inviteData struct {
				Email    string `json:"email"`
				Username string `json:"username"`
				Role     int    `json:"role"`
			}
			if err := common.Unmarshal([]byte(inviteFlow.Payload), &inviteData); err != nil || model.NormalizeEmail(inviteData.Email) == "" || strings.TrimSpace(inviteData.Username) == "" {
				common.ApiErrorI18n(c, i18n.MsgUserInviteInvalid)
				return
			}
