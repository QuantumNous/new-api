...
func SearchUsers(c *gin.Context) {
	...
	tag := c.Query("tag")
	pageInfo := common.GetPageQuery(c)
	users, total, err := model.SearchUsers(keyword, group, ip, tag, pageInfo.GetStartIdx(), pageInfo.GetPageSize())
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func DeleteUser(c *gin.Context) {
	...
	ratio_setting.DeleteUserGroupRatioCacheByUser(id)
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "",
	})
...
func TopUp(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, model.ErrSubscriptionPurchaseLimit) {
			common.ApiErrorI18n(c, i18n.MsgSubscriptionPurchaseLimit)
			return
		}
