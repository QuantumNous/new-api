...
func TestRegisterSinglePrimaryRejectsInvitationWithoutPayload(t *testing.T) {
	db := setupAPIKeyLoginTestDB(t)
	previousRegisterEnabled := common.RegisterEnabled
	previousPasswordRegisterEnabled := common.PasswordRegisterEnabled
	previousEmailVerificationEnabled := common.EmailVerificationEnabled
	...
	inviteToken, _, err := model.CreateAuthFlow(model.AuthFlowCreate{
		Purpose:   model.AuthFlowPurposeUserInvite,
		ExpiresAt: time.Now().Add(time.Minute),
	})
	require.NoError(t, err)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/user/register", strings.NewReader(`{"invite_token":"`+inviteToken+`","email":"attacker@example.com","username":"attacker","role":100}`))
	c.Request.Header.Set("Content-Type", "application/json")
