...
func getGeminiVideoURL(channel *model.Channel, task *model.Task, apiKey string) (string, error) {
	if channel == nil || task == nil {
		return "", errors.New(i18n.Translate(i18n.DefaultLang, "ctrl.invalid_channel_or_task"))
	}
	...
	if adaptor == nil {
		return "", errors.New(i18n.Translate(i18n.DefaultLang, "ctrl.gemini_task_adaptor_not_found"))
	}

	if apiKey == "" {
		return "", errors.New(i18n.Translate(i18n.DefaultLang, "ctrl.api_key_not_available_for_task"))
	}
	...
	if err != nil {
		return "", fmt.Errorf(i18n.Translate(i18n.DefaultLang, "ctrl.fetch_task_failed"), err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", fmt.Errorf(i18n.Translate(i18n.DefaultLang, "ctrl.read_task_response_failed"), err)
	}
	...
	if parseErr != nil {
		return "", fmt.Errorf(i18n.Translate(i18n.DefaultLang, "ctrl.parse_task_result_failed"), parseErr)
	}

	return "", errors.New(i18n.Translate(i18n.DefaultLang, "ctrl.gemini_video_url_not_found"))
}
...
func getVertexVideoURL(channel *model.Channel, task *model.Task) (string, error) {
	...
	adaptor := relay.GetTaskAdaptor(constant.TaskPlatform(strconv.Itoa(channel.Type)))
	if adaptor == nil {
		return "", errors.New(i18n.Translate(i18n.DefaultLang, "ctrl.vertex_task_adaptor_not_found"))
	}

	key := getVertexTaskKey(channel, task)
	if key == "" {
		return "", errors.New(i18n.Translate(i18n.DefaultLang, "ctrl.vertex_key_not_available_for_task"))
	}
	...
	return "", errors.New(i18n.Translate(i18n.DefaultLang, "ctrl.vertex_video_url_not_found"))
}
...
func ensureAPIKey(uri, key string) string {
	...
	if strings.Contains(uri, "?") {
		return fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "ctrl.key"), uri, key)
	}
