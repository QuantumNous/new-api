...
| **入口** | 后端 `https://staging-console.flatkey.ai`、官网 `https://staging-website.flatkey.ai`；通过 Cloud Run domain mapping 直连服务，**不接生产 LB** |
...
| — | **测试域名** | 后端 `https://staging-console.flatkey.ai`，官网 `https://staging-website.flatkey.ai`。用 Cloud Run domain mapping + DNS CNAME，不改 `lb_domains`、不动生产 LB |
...
| Staging 域名 | **新增 Cloud Run domain mapping** | `staging-console.flatkey.ai` → `newapi-staging`；`staging-website.flatkey.ai` → `newapi-web-staging` |
...
- **入口使用 staging 专属域名，但不接生产 LB。** `staging-console.flatkey.ai` / `staging-website.flatkey.ai` 通过 Cloud Run domain mapping 直接映射到两个 staging 服务；不改 `lb_domains`、不动现有 HTTPS proxy / managed cert，因此不会触发生产证书轮换。
...
4. 配置 DNS CNAME（见 §4），并在 Google Search Console 验证 `flatkey.ai` 或两个 staging 子域名的所有权；随后 `terraform apply -var='enable_staging=true' -var='enable_staging_domains=true'` 创建 Cloud Run domain mapping，等待证书 ACTIVE。
5. 收敛 DB 用户权限：
