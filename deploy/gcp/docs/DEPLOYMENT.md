...
Go app workflow 会同时构建同一份 Go 镜像，并按 workflow 配置部署到 console/router 目标。不要把 website 变更放到 Go workflow，也不要在 `web/default` 里恢复公开网站页面。

## 日常发布

### 触发方式

1. **自动**：PR 合并到 `main` 后触发对应 workflow。
2. **手动**：GitHub → Actions → 选择 workflow → Run workflow。生产 deploy job 仍需 `production` Environment 审批。
...
- 如果代码改动同时影响 console/router，两边都要验证；不要只看一个域名。
