...
- `proposed_case` still uses the closed result-schema origin enum (`staging_website`, `staging_console`) as logical labels. Those labels resolve to the execution-scoped target profile; they are not a way for the model or caller to pass raw URLs. `docs` is allowed only in committed fixed-case YAML, not in model-authored `proposed_case`.
