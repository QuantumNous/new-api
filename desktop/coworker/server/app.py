...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    @app.middleware("http")
    async def harden_callback_responses(request: Request, call_next):
        # OAuth callbacks are the one surface reached directly by the system browser,
        # so their query strings hold live authorization codes. Keep those responses
        # out of caches, history-driven referrers, and MIME sniffing.
