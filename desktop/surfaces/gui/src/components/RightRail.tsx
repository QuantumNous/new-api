...
function ProgressSummary({ running, toolNames, todo }: { running: boolean; toolNames: string[]; todo: TodoItem[] }) {
  ...
  if (running) {
    return (
      <div className="rail-muted">
        {t("Working on this task")}{toolNames.length ? ` with ${toolNames.length} tool call${toolNames.length === 1 ? "" : "s"} so far.` : "."}
      </div>
    );
  }
  return (
    <div className="rail-muted">
      {t("For longer multi-step tasks, progress will appear here while BoxAI Desktop plans, uses tools, waits for approval, and produces artifacts.")}
    </div>
  );
}
      ...
      <div className="artifact-preview">
        {!content ? (
          ...
          <div className="artifact-open-prompt">
            ...
            <p>{t("This {{kind}} file can’t be previewed here.", { kind: /\.pptx?$/i.test(artifact.name) ? "PowerPoint" : "Word" })}</p>
