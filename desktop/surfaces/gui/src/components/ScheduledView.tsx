    ...
    <Shell>
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <PanelHead title={t("Automations")} sub={t("Recurring tasks BoxAI Desktop runs on a schedule.")} />
        </div>
        <button
      ...
      {empty ? (
        !showForm && (
          <div className={CARD + " p-4 text-[12.5px] text-muted"}>
            {t("No scheduled tasks yet — use a template above, click")} <strong>{t("+ New automation")}</strong>{t(", or just ask BoxAI Desktop in a session.")}
          </div>
        )
      ) : (
          ...
          {tasks.map((task) => (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 text-[12px] text-muted">
                ...
                {task.enabled ? task.schedule : t("Paused")} {t("· next")} {fmt(task.next_run)} · {task.run_count} {task.run_count === 1 ? t("run") : t("runs")}
