...
## 7A. 朋友评审建议吸收：降级、首次大额激励和大客户治理

### P2-大额首次付费一次性激励

业务结论：

- 该建议应采纳，但不能通过直接提高 5000 元以上长期分佣比例实现。
- 大额首次付费激励应是独立一次性事件，触发一次后后续消耗仍回到普通 paid 净消耗阶梯。
- 激励必须可配置、可封顶、可退款扣回，并纳入结算审计。

后续任务：

- [ ] 设计 `affiliate_first_paid_bonus_rules` / `affiliate_first_paid_bonus_events` sidecar，或等价的规则集配置与事件表；新增 GORM model 前后必须跑 schema impact。
- [ ] 配置字段至少包含分销等级、触发 paid 金额门槛、触发窗口、奖励金额或比例、封顶金额、退款扣回策略、状态、排序和备注。
- [ ] 事件字段至少包含 affiliate_user_id、downstream_user_id、rule_set_id、trigger_paid_cents、bonus_cents、status、settlement_id、source_log_id 或账本事件引用、metadata 和审计时间。
- [ ] 服务层先写 RED 测试：同一下线只触发一次；gift/trial/legacy_unknown 不触发；退款后扣回或转人工审核；跨周期幂等不重复发放。
- [ ] 管理端规则 UI 使用表格配置，金额用元、比例用百分比，保存时转换 cents/bps；default/classic 保持 parity 和 i18n。
- [ ] 分销商端将该项展示为“大额首次付费激励”或“首付激励”，不要混入普通人头费。
- [ ] 未完成该功能前，不修改默认 seed 中 5000 元以上长期分佣比例。

### P2/P3-大客户独立管控

业务结论：

- 5000 元以上用户应被视作大客户候选，不应完全依赖通用阶梯自动结算。
- 大客户可以继续统计归属和试算佣金，但超过阈值部分应进入人工审批或专属维护。
- 该机制主要保护毛利率，避免大客户规模上来后自动分佣侵蚀利润。

后续任务：

- [ ] 设计 `affiliate_key_accounts` 或 `affiliate_customer_overrides` sidecar，记录大客户候选、人工确认、维护人、审批状态、特殊分佣覆盖和备注；新增 schema 前后跑 impact。
- [ ] 自动识别条件可配置：单用户累计 paid 净消耗 >=5000 元、单周期 paid 净消耗达到阈值、或管理员手动标记。
- [ ] 命中大客户阈值后，超过阈值部分默认进入 manual_review，不直接自动 paid 结算。
- [ ] 管理端增加大客户列表：显示分销商、下线用户、累计 paid、周期 paid、退款、估算佣金、审批状态和维护备注。
- [ ] 佣金服务增加测试：普通区间照常计佣；命中大客户阈值后的超额部分生成待审批事件；审批后才可进入结算。
- [ ] 结算服务增加测试：未审批大客户佣金不得进入 payable；审批、作废、扣回必须可审计。
- [ ] 前端增加醒目标识和过滤条件，避免运营误把大客户当普通下线批量结算。

### P3-分销商降级机制预留

业务结论：

- 起步阶段不启用自动降级；当前先通过 KPI 档位、风控冻结、人工禁用来控制质量。
- 等分销规模和样本量稳定后，再引入连续周期不达标的降级或暂停机制。

后续任务：

- [ ] 设计降级策略配置：观察周期、连续不达标次数、宽限期、降级动作、恢复条件、申诉/人工复核入口和是否影响二级 scope。
- [ ] 降级动作必须只影响后续周期，不追溯已完成结算。
- [ ] 降级前必须写入通知/审计记录，管理员可以人工豁免或恢复。
- [ ] 服务层测试覆盖：连续不达标进入观察；观察期仍不达标降级；重新达标恢复；严重违规直接冻结；历史结算不变。
- [ ] 前端管理端展示分销商当前考核状态、连续不达标周期、观察期结束时间、降级原因和恢复条件。
- [ ] 未完成配置、审计和前端提示前，不启用自动降级任务。
