...
- [ ] RED: 新增 Go 测试，要求 head fee rule 缺省为 `active`，`disabled` 时不生成人头费事件，历史 snapshot 缺字段时表单补 `active`。
- [ ] GREEN: 在后端输入、保存、seed、复制、回滚和生成人头费事件时规范化 `active/disabled`。
- [ ] RED/GREEN: default/classic 表格 helper 测试要求人头费表格显示 `status`，列顺序在 `name` 后、金额和门槛字段前。
- [ ] 验证：
