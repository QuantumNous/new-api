...
- **重试策略**：失败后自动重试 3 次，间隔 5s、13s、34s（共 4 次尝试）
- **重试条件**：网络错误、超时、HTTP 5xx 会重试；HTTP 4xx 不重试（客户端错误）
- 回调超时 10 秒
- 回调失败仅记录日志，不影响任务状态
...
1. **验证请求**：检查 `task_id` 是否是你提交的
2. **处理结果**：解析 `status` 和 `data`/`error`
3. **返回 2xx**：响应 HTTP `200` 或 `204`，表示已成功接收
...
只需返回 HTTP 2xx 状态码即可。非 2xx 响应会触发重试。
