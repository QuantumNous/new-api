...
### P0-1 quota 错误「只重试、不反馈」，坏渠道持续吃首跳
...
| 优先级 | 事项 | 改动面 | 效果 |
|--------|------|--------|------|
| 1 | 自动禁用关键词补 quota 文案 + 配置定时探活 | 纯线上配置 | 坏渠道自动下线/恢复，闭环成型 |
| 2 | abilities/channels priority 一致性（启动校验或运维 SOP） | 小 | 消除选路与配置不一致 |
| 3 | quota 关键词表挪 operation_setting；接入视频任务重试 | 小 | 新平台文案可配，视频也能转移 |
| 4 | 渠道能力标签化（参考图/比例注入等），替换厂商字符串匹配 | 中 | 接新平台零通用层改动 |
| 5 | ImageGenerationModels 配置化 | 中 | 加模型不发版 |
| 6 | 补第二家参考图渠道 | 配置+验证 | 消除参考图单点 |

## 六、一句话结论

骨架（priority 桶选路 + 跳桶重试 + provider 适配模式）是合理且已被生产验证的，可以放心在其上加平台；当前主要欠的不是架构，而是**反馈回路（坏渠道自动下线/恢复）没接通**和**厂商特例没有配置化**——前者是纯配置动作建议立刻做，后者决定接入第 10 家平台时的边际成本，建议在下一批平台接入前完成。
