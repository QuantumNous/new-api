...
本文记录 newapi 接入 [BlockRun](https://blockrun.ai) 作为上游 AI 中转渠道的完整设计、协议、配置与运维要点。

BlockRun 是一个 OpenAI 兼容的 LLM 网关，不使用传统 API key — **每次调用都通过 x402 v2 微支付协议在 Base 链上用 USDC 即时结算**。渠道在 newapi 后台的"密钥"字段存的是 EVM 钱包私钥，每个请求由 newapi 在本地用 EIP-712 / ERC-3009 签一次授权后再发送，私钥从不离开服务器。
...
| 上游 endpoint | `POST https://blockrun.ai/api/v1/chat/completions`（OpenAI 兼容）|
| 计费模式 | Pay-per-call（按调用一次性扣固定 USDC，无 token 单价）|
...
└─────────────────────────────────────────────────────────────────────────────┘
   ...
   │                                                          │
   │  ┌────────────────────────────────────────────────────┐  │
   │  │  relay/channel/blockrun/   ← 新增渠道适配器        │  │
   │  │                                                    │  │
   │  │  · Init / ConvertOpenAIRequest                    │  │
   │  │  · ConvertClaudeRequest  → 委托 openai.Adaptor    │  │
   │  │  · GetRequestURL → 强制重写到 /v1/chat/completions│  │
   │  │  · SetupRequestHeader → 不设 Authorization,       │  │
   │  │                          retry 时注入             │  │
   │  │                          Payment-Signature 头     │  │
   │  │                                                    │  │
   │  │  DoRequest 两跳：                                  │  │
   ...
   │  │                                                    │  │
   │  │  DoResponse → 委托 openai.Adaptor                 │  │
   │  │    · OpenAI 入流 → 直接透传                       │  │
   │  │    · Claude 入流 → OpenAI 响应反向转 Claude 流    │  │
   │  └────────────────────────────────────────────────────┘  │
   │     │                                                    │
   │     ▼                                                    │
...
└─────────────────────────────────────────────────────────────────────────────┘
   ...
   └──────────────────────────────────────────────────────┘
            │
            ▼
   第一次 POST /v1/chat/completions (no auth)
            │
   ...
   └──────────────────────────────────────────────────────┘
            ...
            │  Body: OpenAI 格式 ChatCompletion / SSE stream
            ▼
   newapi DoResponse → openai.Adaptor
            · OpenAI 入流 → 透传
            · Claude 入流 → 反向转 Claude 流
            ▼
   终端用户
...
└─────────────────────────────────────────────────────────────────────────────┘
   ...
   BlockRun 自身是聚合中转。同一个 /v1/chat/completions endpoint 根据
   model 字段路由到不同上游：
...
├── adaptor.go                  channel.Adaptor 接口实现
├── x402.go                     x402 v2 协议核心：签名、校验、头解析
├── constants.go                ChannelName + ModelList（37 个 LLM）
├── x402_validate_test.go       validatePaymentOption / parsePrivateKey 单测
├── url_test.go                 GetRequestURL Claude/Gemini 路径重写单测
└── x402_e2e_test.go            实链 e2e（build tag e2e_blockrun 门控）
```

依赖：`github.com/BlockRunAI/blockrun-llm-go v0.7.0`（**仅调它的 `CreatePaymentPayload` 和 `ParsePaymentRequired` 两个公开函数**，自写 HTTP 二跳逻辑以支持流式 SSE）。
...
| 命名空间 | 模型 |
|---|---|
| `anthropic/` | claude-opus-4.7, claude-opus-4.5, claude-sonnet-4.6, claude-haiku-4.5 |
...
Claude Code 会调 `POST /v1/messages`，newapi 识别为 Claude format → BlockRun adaptor 委托 openai.Adaptor 做 Claude→OpenAI 转换 → BlockRun 上游 → 反向转 Claude 流回 Claude Code。
...
覆盖：
- `TestValidatePaymentOption_*` — 4 个 accept + 24 个 reject case，所有 bypass 向量
...
- `TestGetRequestURL_*` — Claude/Gemini format 路径重写覆盖
...
curl -N localhost:3000/v1/chat/completions \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"model":"anthropic/claude-haiku-4.5","messages":[{"role":"user","content":"hi"}],"stream":true,"max_tokens":20}'

# Claude Messages API（Claude Code 路径）
...
| 能力 | 当前 | 备注 |
|---|---|---|
| Chat Completions (OpenAI / Claude Messages) | ✅ | 流式 + 非流式 + tool use + 多模态全部支持 |
...
| Prompt caching（`cache_control`）端到端 | ⚠️ | 上游 response usage 已包含 `cache_*_tokens` 字段；请求方向的 `cache_control` 透传依赖 newapi 现有 Claude→OpenAI 转换链的实现 |
...
- BlockRun Go SDK：<https://github.com/BlockRunAI/blockrun-llm-go>
