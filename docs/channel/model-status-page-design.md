...
因此，aiapi114 需要新增一个独立的用户可访问模型状态页，基于现有 `/api/uptime/status` 聚合数据展示上游模型健康状态。页面设计参考 `https://status.rjj.cc/status/foxcode` 的状态页结构，但不直接照搬 Uptime Kuma 的通用服务监控表达，而是调整为“供应商 + 分组 + 模型”的模型可用性视角。
...
- 按分组组织监控项，适合 aiapi114 的“供应商 / 分组 / 模型”结构。
...
- 管理员：观察上游供应商和模型分组的健康趋势，为后续动态调度策略提供人工判断依据。
...
- 标题：`模型状态`
- 副标题：`展示已接入状态同步的上游供应商模型可用性，数据每 60 秒缓存更新。`
- 全局状态：
  - 全部正常：`全部模型运行正常`
  - 部分异常：`部分模型出现波动`
  - 大面积异常：`多个供应商出现异常`
...
```text
Ikun
已接入 32 个模型 · 最近同步 1 分钟前 · 当前 29 正常 / 3 波动
...
```text
暂无模型状态数据
当前还没有完成上游状态同步。请稍后刷新，或联系管理员确认状态同步任务是否已启用。
```
