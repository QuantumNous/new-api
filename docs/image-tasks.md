...
仅支持 JSON 图片生成，暂不提供 multipart 编辑任务。最多 32 MiB 输入、每任务 1–20 张；每用户待处理不超过 10 个任务/20 张图片。同一用户最多并行 2 个 API 图片任务；全局 API 图片任务并发沿用绘图并发配置。该并发池与网页批次池独立。最大后台执行时间 20 分钟，结果保留 24 小时，提交编号保留共 7 天防止重复。结果文件使用约 49 MiB 的写入上限，过大或写入中断会标记结果未知，不能自动重新生成。
