# Origin AI Codex / Claude MVP 与现有 new-api Go 后端集成
...
Codex / Claude MVP 的目标生产链路只有：

```text
Codex / OpenAI SDK / Claude Code / Anthropic SDK
...
| 暴露 `POST /v1/messages`、`POST /v1/messages/count_tokens` | `router/relay-router.go`、`controller/origin_messages.go` |
...
| 观察 Messages/cache usage，返回 Anthropic JSON 或 SSE | `relay/channel/claude/relay-claude.go` |
...
BeeNex 已原生支持 OpenAI Responses 和 Anthropic Messages。Origin 受控版本不得新增 BeeNex 专用协议 Adaptor，不得在 Responses、Chat Completions 和 Messages 之间转换。

因此，当前实现不需要多重组装模型格式：Responses 与 Messages 都各自保持原生协议，中间只经过一次同协议 relay。现有 typed request 路径可能为了校验、模型名替换和发送而解析/序列化请求；非流式响应会读取原始正文观察 usage，SSE 会扫描 `data:` 事件观察 usage/tools。这些都是同协议内处理，不是跨协议转换或多个 Adaptor 串联。
...
- 回程响应把上游模型名恢复为公开的平台模型名，不暴露 BeeNex 模型 ID；
...
- 改写 SSE 语义或 tool arguments（平台模型名反向虚拟化除外）；
...
本仓锁定的 `origin-contracts` v4 消费提交为 `1e34df23e450e9a2326772b3fea4e5c923d621be`，聚合 SHA-256 为 `8207d66fc947531556f8f67fab921ee07023c7f85d78e7f897b232499ee71b8e`。v4 在不可变 v1/v2/v3 之上增加原生 Anthropic Messages、按 operation 模型发现、Messages admission/catalog/usage 与 CallLog operation。同步文件与逐文件哈希记录在 `contracts/origin/contract-lock.json`；模型发现、admission、catalog 和 usage 事件不得偏离该快照。
...
数据库自动迁移新增 `origin_request_attempts` 与 `origin_usage_outboxes`。前者在上游调用前记录 reservation、catalog Route、权威 operation 和尝试租约；历史记录的 operation 回填为 `responses`，新 Messages 记录持久化为 `messages`。后者与 attempt 终态在同一事务写入。Kafka 发布为持久化 outbox 上的至少一次投递，稳定 `event_id` 由 Platform 按锁定契约幂等消费。Kafka 不可用只会推进重试/DEAD 状态，不会删除 usage；DEAD 记录需在告警处理后显式恢复。
...
- `POST /v1/messages/count_tokens` 通过 `operation=messages` 验证可售模型，只做本地估算，不创建 reservation、usage、Charge/Ledger，也不访问 BeeNex；
- Messages 非流与 SSE 的 usage 生成 `messages` / `messages_stream`，cache read 使用 `CACHED_TOKEN`，cache creation 只并入一次输入量；
