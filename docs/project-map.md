...
| Token 管理 | `/api/token` -> `controller/token.go`、`controller/ccswitch_import.go` | `service/ccswitch_import.go`、`service/ccswitch_model_cache.go`、`model/token*.go`、模型/供应商元数据 | 用户鉴权；CC Switch 导入选项与链接生成使用令牌所有权校验，模型列表来自导入专用缓存 |
