...
| `period` | varchar(16) | 限制周期：`total` / `daily` / `weekly` / `monthly` |
...
模型额度规则最终都限制到**用户个人**，不是分组共享池或套餐共享池。
...
- **分组规则周期**：
  - `total`：总额限制，`period_end` 使用远期时间，默认不自动重置
  - `daily`：当天 00:00 到次日 00:00
  - `weekly`：本周一 00:00 到下周一 00:00
  - `monthly`：本月 1 日 00:00 到下月 1 日 00:00
- **套餐规则周期**：计数器的 `period_start` / `period_end` 跟随用户活跃订阅周期
- **Redis TTL 自动过期**：key 在 `period_end` 时自动过期
- **DB 查询自动排除过期记录**：只读取 `status='active' AND period_end > now` 的 usage
- **DB 过期标记**：新周期懒创建前会将过期 usage 标记为 `expired`
- **新周期懒创建**：下次请求命中规则时创建新的 `active` 计数器，`quota_used = 0`
