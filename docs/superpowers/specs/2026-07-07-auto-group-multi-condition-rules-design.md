...
```text
一级组织名称 = 产品与解决方案中心 -> itbp
岗位包含 AIBP -> itbp
岗位包含 产品 且 一级组织名称 = 产品与解决方案中心 -> itbp
```

不直接把 `深圳市一应科技有限公司` 全部归为 itbp。
...
首批内置规则直接覆盖已经确认的城区SC、城区级职能部门，以及当前数据中高度稳定的项目BMG、事业部SC、集团高层、部分 itbp。冲突岗位全部进入待确认，由管理员确认后再逐步沉淀。