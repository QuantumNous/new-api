...
PR #819 的评审暴露出离线队列、跨标签页清空和非流式请求状态的边界问题。以下约束补充并细化原设计；与前文冲突时以本节为准。

### 待同步队列

- 完整 `PlaygroundRecordPayload` 不再以单个 JSON 数组保存在 `localStorage`。浏览器使用原生 IndexedDB，以 `(user_id, record_id)` 为唯一键逐条保存记录，并按 `(client_completed_at, record_id)` 排序后 FIFO 提交。
- IndexedDB 的单条 `put` 在事务内完成，因此不同标签页写入不同记录时不会通过读-改-写覆盖彼此；相同记录仍保持幂等更新。
- IndexedDB 不可用或达到配额时，当前页面使用内存队列继续重试，并向用户显示非阻塞告警。无法持久写入且网络同时失败时不能静默丢弃记录。
- 已成功提交的记录立即从持久队列和内存队列删除。读取当前服务端快照失败不能使已提交记录重新进入队列。

### 清空顺序

- `clear` 只参与目标 `conversation_id` 的顺序比较。服务端锁定用户后，将 `(client_completed_at, record_id)` 与目标会话的最新记录比较；较旧的 clear 只作为非最新历史写入，不改变恢复状态。
- 只有当目标会话仍是该用户当前会话且 clear 更新时，才取消它的 `is_current`。来自旧标签页的 clear 不得清除另一个会话已经接管的当前状态。
- `clear` 成为目标会话最新状态后，任何复用相同 `conversation_id` 的 turn 仍不得重新成为当前会话；新会话必须使用新的 ID。

### 请求身份与恢复

- 每个 active turn 绑定发送时创建的 assistant message key。终态落表按该 key 查找回复，不能隐式使用消息数组最后一项。
- 流式和非流式请求都必须暴露统一的生成中状态；请求期间禁用再次发送、编辑、删除和重新生成。
- 初次读取当前快照发生临时错误时，保留本地状态并进行有限次数重试。重试耗尽后允许继续使用本地 Playground，不以空数据覆盖。
- 媒体生成仍不进入第一版服务端分析表。媒体请求开始后标记该浏览器中的会话为“本地优先”；自动恢复不得用旧服务端聊天覆盖这份本地媒体会话。该保证只覆盖同一浏览器，换浏览器仍只恢复已落表的聊天数据。
- Playground 统一通过带 Web Crypto 降级路径的 ID 生成函数创建记录和会话 ID，避免直接依赖 `crypto.randomUUID()`。

### 保持不变的边界

- 不读取或迁移旧的未按用户隔离的 `localStorage` key，避免跨账号归属错误。
- 不通过时间较晚的 turn 复活已经 clear 的同一 `conversation_id`。
- 本轮不新增清空/新对话 UI；只保证已存在的 clear API 在未来接入时并发语义正确。
