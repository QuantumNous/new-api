...
async function goToWaffoPayMethodsSection(page: Page) {
  ...
  // 等待 Waffo 设置区域加载
  await page.waitForSelector('text=Waffo 设置', { timeout: 15000 });
...
test.describe('TC-PM: Waffo 支付方式配置', () => {
  ...
  test('TC-PM-001: 添加支付方式（正常路径）', async ({ page }) => {
    ...
    // 弹窗应关闭，新行出现在表格中
    await expect(page.getByText(methodName)).toBeVisible({ timeout: 5000 });
  ...
  test('TC-PM-002: 添加支付方式（名称为空，校验拦截）', async ({ page }) => {
    ...
    const errorToast = page.locator('.semi-toast-content, [class*="toast"]');
  ...
  test('TC-PM-004: 上传图标（>100KB，被限制拦截）', async ({ page }) => {
    ...
    const toastContent = page.locator('.semi-toast-content, [class*="toast"]');
    await expect(toastContent.first()).toBeVisible({ timeout: 3000 });
    await expect(toastContent.first()).toContainText('100KB');
