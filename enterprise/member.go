...
func listMembers(c *gin.Context) {
	...
	if minQuota, err := optionalInt(c, "min_quota"); err != nil {
		failureI18n(c, http.StatusBadRequest, i18n.MsgEnterpriseInvalidQuotaRange)
		return
	} else if minQuota != nil {
		query = query.Where("users.quota >= ?", *minQuota)
	}
	if maxQuota, err := optionalInt(c, "max_quota"); err != nil {
		failureI18n(c, http.StatusBadRequest, i18n.MsgEnterpriseInvalidQuotaRange)
		return
	} else if maxQuota != nil {
		query = query.Where("users.quota <= ?", *maxQuota)
	}
	...
	if err := query.Order("members.joined_at DESC").Scan(&records).Error; err != nil {
		failureI18n(c, http.StatusInternalServerError, i18n.MsgEnterpriseMemberListFailed)
		return
	}
	success(c, memberResponses(records))
}
...
func parseTagIDs(c *gin.Context) ([]int, bool) {
	raw := strings.TrimSpace(c.Query("tag_ids"))
	if raw == "" {
		if legacy := strings.TrimSpace(c.Query("tag_id")); legacy != "" {
