...
套餐目录允许管理员为单个套餐设置固定十六进制强调色（`PlanAccent.token === 'custom'`），这是语义令牌规则的一处受管例外，约束如下：

- 仅可用于套餐卡的强调件：顶部渐变条、权益勾选图标、列表左侧色条。**不得**用于文字色、表面底色或边框主色。
- 值经 `normalizeOpaqueColor()` 校验并统一存为 `#rrggbb`，非法输入在服务端拒绝而非静默回退。
- 表单对日间 `#fffdf8` 与夜间 `#32363f` 两个卡面双向计算对比度，低于 3:1 时给出非阻塞提示（阈值取强调件标准，不按正文 4.5:1）。
- 自定义色两个主题同色，不随主题切换；需要跟随主题的场景应使用 `accent` / `signal` / `support` 三个语义令牌。
