...
const props = withDefaults(
  defineProps<{
    ...
    max?: number
    /**
     * Leading unit glyph. Defaults to the currency sign because most callers
     * take money; pass '' for counts such as days or a display order.
     */
    prefix?: string
    /** Rejects the decimal separator outright, for whole-number fields. */
    integer?: boolean
  }>(),
  {
    placeholder: '',
...
<template>
  ...
  >
    ...
    <input
      ...
      v-bind="inputAttrs"
      class="h-11 w-full border-0 border-b-[1.5px] bg-transparent pr-4 font-mono text-[15px] font-semibold leading-[22px] text-[var(--text-primary)] placeholder:font-normal placeholder:font-sans placeholder:text-[var(--text-tertiary)] transition-[border-color,background-color,box-shadow] focus:outline-none focus:border-[var(--accent)]"
      :class="prefix ? 'pl-10' : 'pl-4'"
