export default {
  auth: {
    signInTitle: '欢迎回来',
    signInSubtitle: '登录以继续使用 Ren2Hub 控制台',
    signUpTitle: '创建账户',
    signUpSubtitle: '一个 Key，接入所有大模型',
    resetTitle: '重置密码',
    resetSubtitle: '请输入您的电子邮件地址重置密码',
    username: '用户名',
    usernameOrEmailPlaceholder: '请输入邮箱或用户名',
    email: '邮箱',
    password: '密码',
    passwordPlaceholder: '请输入密码',
    twoFactorCode: '两步验证代码',
    twoFactorPlaceholder: '请输入验证器代码或备用代码',
    verifyTwoFactor: '验证并登录',
    backToPassword: '返回密码登录',
    twoFactorExpired: '验证请求已过期，请重新输入密码',
    newPassword: '新密码',
    confirmPassword: '再次确认密码',
    forgot: '忘记密码？',
    signIn: '登录',
    loginSuccess: '登录成功',
    signUp: '注册',
    sendReset: '发送重置链接',
    continue: '继续',
    noAccount: '还没有账户？',
    hasAccount: '已有账户？',
    goSignUp: '立即注册',
    goSignIn: '直接登录',
    showPassword: '显示密码',
    hidePassword: '隐藏密码',
    passwordHint: '12 个字母和数字组合',
    strength: ['弱密码', '中等', '强密码'],
    resetSent: '重置链接已发送，请查收邮箱',
    mismatch: '两次输入的密码不一致',
    usernameRequired: '请输入用户名',
    inviteCode: '邀请码',
    inviteCodePlaceholder: '输入邀请码',
    inviteCodeRequired: '请输入有效邀请码',
    inviteCodeInvalid: '邀请码无效',
    inviteCodeRequiredHint: '当前注册必须使用有效邀请码',
    inviteCodeOptionalHint: '选填，绑定后用于返利归因',
    emailInvalid: '邮箱格式不正确',
    passwordTooShort: '密码至少 8 位',
    brandSlogan: 'One Key, All Models',
    brandTagline: '稳定与平价、人人用得起的 AI 网关',
    tickerModels: '支持模型',
    tickerUptime: '在线率',
    tickerVersion: '版本',
    backHome: '返回首页',
    cardMotto: '最安心的大模型中转平台',
    oauthCompleting: '正在完成 OAuth 绑定...',
    oauthUnavailable: '当前 OAuth 绑定窗口已失效。',
    oauthLinked: 'OAuth 账号绑定成功。',
    oauthFailed: 'OAuth 绑定失败。',
    relay: {
      prompt: '提示词原文',
      passthrough: '字节级直通',
      upstream: '官方上游',
      fanout: '聚合转发',
    },
    editorial: {
      word1: '稳定',
      word2: '平价',
      word3: '触手可及',
      line1: '一钥之便',
      line2: '接万模之智',
      point1: '不存请求体 · 不参与训练',
      point2: '全链路 TLS · 全程可审计',
      point3: '注册即开 · 秒级接入',
      statusLabel: 'STATUS · 实时运行',
    },
  },
}
