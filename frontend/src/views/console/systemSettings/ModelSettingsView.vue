...
const globalDirty = computed(() => {
  ...
  return global['global.pass_through_request_enabled'] !== s['global.pass_through_request_enabled'] ||
    global['general_setting.ping_interval_enabled'] !== s['general_setting.ping_interval_enabled'] ||
    global['general_setting.ping_interval_seconds'] !== s['general_setting.ping_interval_seconds']
})
async function saveGlobal() {
  globalSaving.value = true
  const s = settings.value
  const patch: Record<string, boolean | number> = {}
  if (global['global.pass_through_request_enabled'] !== s['global.pass_through_request_enabled'])
    patch['global.pass_through_request_enabled'] = global['global.pass_through_request_enabled']
  if (global['general_setting.ping_interval_enabled'] !== s['general_setting.ping_interval_enabled'])
    patch['general_setting.ping_interval_enabled'] = global['general_setting.ping_interval_enabled']
  if (global['general_setting.ping_interval_seconds'] !== s['general_setting.ping_interval_seconds'])
    patch['general_setting.ping_interval_seconds'] = global['general_setting.ping_interval_seconds']
...
const routingDirty = computed(() => {
  const s = settings.value
  return routing.RetryTimes !== s.RetryTimes ||
    ...
    routing['monitor_setting.auto_test_channel_enabled'] !== s['monitor_setting.auto_test_channel_enabled'] ||
    routing['monitor_setting.auto_test_channel_minutes'] !== s['monitor_setting.auto_test_channel_minutes']
})
async function saveRouting() {
  ...
  if (routing.AutomaticDisableChannelEnabled !== s.AutomaticDisableChannelEnabled)
    patch.AutomaticDisableChannelEnabled = routing.AutomaticDisableChannelEnabled
  ...
  if (routing['monitor_setting.auto_test_channel_enabled'] !== s['monitor_setting.auto_test_channel_enabled'])
    patch['monitor_setting.auto_test_channel_enabled'] = routing['monitor_setting.auto_test_channel_enabled']
  if (routing['monitor_setting.auto_test_channel_minutes'] !== s['monitor_setting.auto_test_channel_minutes'])
    patch['monitor_setting.auto_test_channel_minutes'] = routing['monitor_setting.auto_test_channel_minutes']
  const ok = await saveOptions(patch)
...
const affinityDirty = computed(() => {
  const s = settings.value
  return affinity['channel_affinity_setting.enabled'] !== s['channel_affinity_setting.enabled'] ||
    affinity['channel_affinity_setting.switch_on_success'] !== s['channel_affinity_setting.switch_on_success'] ||
    affinity['channel_affinity_setting.default_ttl_seconds'] !== s['channel_affinity_setting.default_ttl_seconds']
})
async function saveAffinity() {
  affinitySaving.value = true
  const s = settings.value
  const patch: Record<string, boolean | number> = {}
  if (affinity['channel_affinity_setting.enabled'] !== s['channel_affinity_setting.enabled'])
    patch['channel_affinity_setting.enabled'] = affinity['channel_affinity_setting.enabled']
  if (affinity['channel_affinity_setting.switch_on_success'] !== s['channel_affinity_setting.switch_on_success'])
    patch['channel_affinity_setting.switch_on_success'] = affinity['channel_affinity_setting.switch_on_success']
  if (affinity['channel_affinity_setting.default_ttl_seconds'] !== s['channel_affinity_setting.default_ttl_seconds'])
    patch['channel_affinity_setting.default_ttl_seconds'] = affinity['channel_affinity_setting.default_ttl_seconds']
  const ok = await saveOptions(patch)
...
const claudeDirty = computed(() => {
  const s = settings.value
  return claude['claude.default_max_tokens'] !== s['claude.default_max_tokens'] ||
    claude['claude.thinking_adapter_enabled'] !== s['claude.thinking_adapter_enabled']
})
async function saveClaude() {
  ...
  if (claude['claude.thinking_adapter_enabled'] !== s['claude.thinking_adapter_enabled'])
    patch['claude.thinking_adapter_enabled'] = claude['claude.thinking_adapter_enabled']
  const ok = await saveOptions(patch)
...
onMounted(async () => {
  ...
  Object.assign(global, {
    'global.pass_through_request_enabled': s['global.pass_through_request_enabled'],
    'general_setting.ping_interval_enabled': s['general_setting.ping_interval_enabled'],
    'general_setting.ping_interval_seconds': s['general_setting.ping_interval_seconds'],
  })
  Object.assign(routing, {
    ...
    'monitor_setting.auto_test_channel_enabled': s['monitor_setting.auto_test_channel_enabled'],
    'monitor_setting.auto_test_channel_minutes': s['monitor_setting.auto_test_channel_minutes'],
  })
  Object.assign(affinity, {
    'channel_affinity_setting.enabled': s['channel_affinity_setting.enabled'],
    'channel_affinity_setting.switch_on_success': s['channel_affinity_setting.switch_on_success'],
    'channel_affinity_setting.default_ttl_seconds': s['channel_affinity_setting.default_ttl_seconds'],
  })
    ...
    >
      ...
      <div v-if="global['general_setting.ping_interval_enabled']" class="mt-4">
        <SysInputRow
          ...
          type="number"
          @update:model-value="global['general_setting.ping_interval_seconds'] = Number($event) || 60"
        />
      </div>
    </SysSettingsFormCard>
    ...
    >
      <div class="grid gap-4 sm:grid-cols-2 mb-4">
        ...
        <SysInputRow
          ...
          type="number"
          @update:model-value="routing.ChannelDisableThreshold = Number($event) || 0"
        />
      </div>
      <div class="divide-y divide-[var(--border-subtle)]">
        <SysToggleRow
      ...
      <div v-if="routing['monitor_setting.auto_test_channel_enabled']" class="mt-4">
        <SysInputRow
          :label="t('systemSettings.models.autoTestMinutes')"
          :model-value="String(routing['monitor_setting.auto_test_channel_minutes'])"
          type="number"
          @update:model-value="routing['monitor_setting.auto_test_channel_minutes'] = Number($event) || 60"
        />
      </div>
    </SysSettingsFormCard>
    ...
    >
      <div class="divide-y divide-[var(--border-subtle)]">
        ...
        <SysToggleRow
          ...
          :description="t('systemSettings.models.channelAffinitySwitchOnSuccessDesc')"
        />
      </div>
      <div class="mt-4">
        <SysInputRow
          :label="t('systemSettings.models.channelAffinityTtl')"
          :model-value="String(affinity['channel_affinity_setting.default_ttl_seconds'])"
          type="number"
          @update:model-value="affinity['channel_affinity_setting.default_ttl_seconds'] = Number($event) || 3600"
        />
      </div>
    </SysSettingsFormCard>
    ...
    >
      ...
      <div class="mt-4">
        <SysInputRow
          ...
          type="number"
          @update:model-value="claude['claude.default_max_tokens'] = Number($event) || 0"
        />
      </div>
    </SysSettingsFormCard>
  </div>
