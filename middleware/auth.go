...
func authHelper(c *gin.Context, minRole int) {
	...
	if apiUserIdStr == "" {
		c.JSON(http.StatusUnauthorized, gin.H{
			"success": false,
			"message": "unauthorized: New-Api-User header is required",
		})
		c.Abort()
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{
			"success": false,
			"message": "unauthorized: invalid New-Api-User format",
		})
		c.Abort()
		return
	...
	if id != apiUserId {
		c.JSON(http.StatusUnauthorized, gin.H{
			"success": false,
			"message": "unauthorized: New-Api-User does not match current user",
		})
		c.Abort()
		return
	}
...
func SetupContextForToken(c *gin.Context, token *model.Token, parts ...string) error {
	...
	if len(parts) > 1 {
		if model.IsAdmin(token.UserId) {
			...
			abortWithOpenAiMessage(c, http.StatusForbidden, "specifying a channel is not supported for normal users")
			return fmt.Errorf("specifying a channel is not supported for normal users")
		}
	}
	return nil
}
