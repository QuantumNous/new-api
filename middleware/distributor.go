...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	if strings.Contains(c.Request.URL.Path, "/mj/") {
		...
		c.Set("relay_mode", relayMode)
	} else if !strings.HasPrefix(c.Request.URL.Path, "/v1/tasks/") && !strings.HasPrefix(c.Request.URL.Path, "/v1/audio/transcriptions") && !strings.Contains(c.Request.Header.Get("Content-Type"), "multipart/form-data") {
		req, err := getModelFromRequest(c)
		if err != nil {
			return nil, false, err
		}
