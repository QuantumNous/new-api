...
func checkSystemPerformance() *types.NewAPIError {
	...
	if config.CPUThreshold > 0 && int(status.CPUUsage) > config.CPUThreshold {
		return types.NewErrorWithStatusCode(errors.New(i18n.Translate(i18n.DefaultLang, "mw.system_cpu_overloaded")), "system_cpu_overloaded", http.StatusServiceUnavailable)
	}

	// 检查内存
	if config.MemoryThreshold > 0 && int(status.MemoryUsage) > config.MemoryThreshold {
		return types.NewErrorWithStatusCode(errors.New(i18n.Translate(i18n.DefaultLang, "mw.system_memory_overloaded")), "system_memory_overloaded", http.StatusServiceUnavailable)
	}

	// 检查磁盘
	if config.DiskThreshold > 0 && int(status.DiskUsage) > config.DiskThreshold {
		return types.NewErrorWithStatusCode(errors.New(i18n.Translate(i18n.DefaultLang, "mw.system_disk_overloaded")), "system_disk_overloaded", http.StatusServiceUnavailable)
	}
