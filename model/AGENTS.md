...
| `user.go` | 用户实体定义及 CRUD，包含密码验证（`ValidateAndFill`）、额度操作、访问 Token 验证；新增注册归因字段：`InviterId`（邀请人 ID，indexed）和 `AdsAttribution`（广告归因数据，`type:text`） |
...
| `topup.go` | 充值记录实体及查询；新增 Stripe 支持：`PaymentMethodStripe`/`PaymentProviderStripe` 常量；充值记录通过 `Preload("Invoice")` 关联 `PaymentInvoice`（`gorm:"foreignKey:TradeNo;references:TradeNo"`），分页查询接口均已注入预加载 |
...
| `model_meta.go` | 模型元信息（`Model`）及厂商（`Vendor`）实体的 CRUD；`GetBoundChannelsByModelsMap` 获取模型-渠道绑定关系；`Model` 结构体新增可用性字段（`AvailabilityStatus`/`AvailabilityReasonType` 等，`gorm:"-"` 不落库）及 `ApplyAvailabilityStateToModel` 辅助函数，用于将 `ModelAvailabilityState` 投影到列表返回值；`GetPreferredModelOwnerChannelTypes` 按 priority/weight 取各模型首选渠道类型 |
...
| `invoice.go` | Stripe 发票数据模型：`UserInvoiceProfile`（用户开票档案，每用户唯一）、`PaymentInvoice`（支付单发票快照，含 Stripe checkout/invoice ID 及状态机）；`NormalizeInvoiceProfileFields`、`SaveUserInvoiceProfile`、`CreatePaymentInvoiceSnapshot`、`GetPaymentInvoiceByTradeNo`、`UpdatePaymentInvoiceStripeSession`、`UpdatePaymentInvoiceStripeInvoice`、`UpdatePaymentInvoiceStatus`、`UpdatePaymentInvoiceProfile`；发票状态常量：`requested/pending/paid/failed/expired` |
| `model_availability.go` | **新增** 上游模型可用性探测：`ModelAvailabilityState`（以 `model_name` 为主键，记录 status/reason/连续失败次数/首次/最后检测时间）；`SaveModelAvailabilityState`（upsert）、`GetModelAvailabilityState`、`GetModelAvailabilityStateMap`（批量）、`IsModelOfficiallyUnsupported`、`GetModelAvailabilityProbeModelNames`、`GetModelAvailabilityProbeTargets`；状态常量：`available/temporary_failure/official_unsupported/unknown_failure`；内置表缺失容错（`isModelAvailabilityTableMissingError`，兼容 SQLite/MySQL/PostgreSQL 的错误文本判断） |
| `usage_reconciliation.go` | 用量对账（BlockRun 渠道）：`BlockRunChannel`/`BlockRunChannelTypes`（按 `ChannelTypeNames` 前缀动态发现，无需硬编码类型号）；`GetBlockRunChannels`、`GetBlockRunEnabledModelChannels`（model→渠道映射，去重）；`StreamBlockRunUsageLogs`（游标逐行回调，低内存）、`CountBlockRunUsageLogs`、`QueryBlockRunUsageLogsPaged`；查询列集合 `usageReconLogColumns` 裁剪传输量 |
...
- **Rule 2（DB 兼容）**：这是 DB 兼容规则最核心的实施层：
  ...
  - 新增迁移（2026-06-10）：`UserInvoiceProfile`、`PaymentInvoice`（Stripe 发票表）和 `ModelAvailabilityState`（模型可用性探测表）均通过 `AutoMigrate` 注册，三个数据库均已兼容；`ModelAvailabilityState` 使用 `model_name` 作为字符串主键（`autoIncrement:false`），查询前有表缺失容错，迁移前系统可正常运行。
...
- 构建验证：`go build ./...`
- 单元测试：`go test ./model/...`
- 测试文件：`batch_update_test.go`、`dingtalk_alert_cooldown_test.go`、`dsn_defaults_test.go`、`invoice_test.go`、`model_owner_test.go`、`option_group_rename_test.go`、`payment_method_guard_test.go`、`task_cas_test.go`、`token_validation_test.go`、`usage_reconciliation_test.go`、`usedata_test.go`、`user_google_oauth_test.go`
