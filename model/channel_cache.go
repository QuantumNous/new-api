...
// GetRandomSatisfiedChannel selects a channel for the given group/model.
//
// excludeChannels lists channel IDs already tried in this request (i.e. failed
// on a previous attempt); they are removed from the candidate pool so retries
// always move to a fresh channel. Pass nil on the first attempt.
//
// Selection walks priority tiers from highest to lowest. Within the highest
// tier that still has non-excluded channels, one channel is chosen by weighted
// random. This guarantees every channel in a tier is exhausted before
// descending to a lower priority, and no already-failed channel is retried.
//
...
func GetRandomSatisfiedChannel(group string, model string, retry int, requestPath string, excludeChannels map[int]bool) (*Channel, error) {
	...
	// Group non-excluded candidates by priority.
	priorityChannels := make(map[int64][]*Channel)
	for _, channelId := range channels {
		if excludeChannels != nil && excludeChannels[channelId] {
			continue
		}
		...
		priorityChannels[p] = append(priorityChannels[p], channel)
	}

	// No eligible channel left (all excluded / exhausted).
	if len(priorityChannels) == 0 {
		return nil, nil
	}

	// Pick the highest priority tier that has candidates.
	...
	return weightedRandomChannel(targetChannels), nil
}

// weightedRandomChannel picks one channel from a same-priority pool using the
// existing weighted-random smoothing rules. Callers guarantee len >= 1.
func weightedRandomChannel(targetChannels []*Channel) *Channel {
	sumWeight := 0
