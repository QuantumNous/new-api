...
func validateCustomOAuthProvider(provider *CustomOAuthProvider) error {
	if provider.Name == "" {
		return errors.New(i18n.Translate(i18n.DefaultLang, "model.provider_name_is_required"))
	}
	if provider.Slug == "" {
		return errors.New(i18n.Translate(i18n.DefaultLang, "model.provider_slug_is_required"))
	}
	// Slug must be lowercase and contain only alphanumeric characters and hyphens
	slug := strings.ToLower(provider.Slug)
	for _, c := range slug {
		if !((c >= 'a' && c <= 'z') || (c >= '0' && c <= '9') || c == '-') {
			return errors.New(i18n.Translate(i18n.DefaultLang, "model.provider_slug_must_contain_only_lowercase_letters_numbers"))
		}
	}
	provider.Slug = slug

	if provider.ClientId == "" {
		return errors.New(i18n.Translate(i18n.DefaultLang, "model.client_id_is_required"))
	}
	if provider.AuthorizationEndpoint == "" {
		return errors.New(i18n.Translate(i18n.DefaultLang, "model.authorization_endpoint_is_required"))
	}
	if provider.TokenEndpoint == "" {
		return errors.New(i18n.Translate(i18n.DefaultLang, "model.token_endpoint_is_required"))
	}
	if provider.UserInfoEndpoint == "" {
		return errors.New(i18n.Translate(i18n.DefaultLang, "model.user_info_endpoint_is_required"))
	}
	...
	if strings.TrimSpace(provider.AccessPolicy) != "" {
		...
		if err := common.UnmarshalJsonStr(provider.AccessPolicy, &policy); err != nil {
			return errors.New(i18n.Translate(i18n.DefaultLang, "model.access_policy_must_be_valid_json"))
		}
		if err := validateAccessPolicyPayload(&policy); err != nil {
			return fmt.Errorf(i18n.Translate(i18n.DefaultLang, "model.access_policy_is_invalid"), err)
		}
	}
...
func validateAccessPolicyPayload(policy *accessPolicyPayload) error {
	...
	if logic != "and" && logic != "or" {
		return fmt.Errorf(i18n.Translate(i18n.DefaultLang, "model.unsupported_logic"), logic)
	}
	...
	for index, condition := range policy.Conditions {
		...
		if _, ok := supportedAccessPolicyOps[op]; !ok {
			return fmt.Errorf(i18n.Translate(i18n.DefaultLang, "model.condition_op_is_unsupported"), index, op)
		}
		if op == "in" || op == "not_in" {
			if _, ok := condition.Value.([]any); !ok {
				return fmt.Errorf(i18n.Translate(i18n.DefaultLang, "model.condition_value_must_be_an_array_for_op"), index, op)
			}
		}
	}
	...
	for index := range policy.Groups {
		if err := validateAccessPolicyPayload(&policy.Groups[index]); err != nil {
			return fmt.Errorf(i18n.Translate(i18n.DefaultLang, "model.group"), index, err)
		}
	}
