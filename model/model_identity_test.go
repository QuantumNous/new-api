...
func TestNormalizeChannelModel(t *testing.T) {
	tests := []struct {
		...
		{
			name:             "strips arbitrary OpenRouter provider namespace",
			channelType:      constant.ChannelTypeOpenRouter,
			upstreamModel:    "moonshotai/kimi-k2.5",
			expectedModel:    "kimi-k2.5",
			expectedUpstream: "moonshotai/kimi-k2.5",
		},
		{
			name:             "strips Amazon namespace returned by OpenRouter",
			channelType:      constant.ChannelTypeOpenRouter,
			upstreamModel:    "amazon/nova-pro-v1",
			expectedModel:    "nova-pro-v1",
			expectedUpstream: "amazon/nova-pro-v1",
		},
		{
			name:             "strips tilde provider namespace and preserves moving alias",
			channelType:      constant.ChannelTypeOpenRouter,
			upstreamModel:    "~anthropic/claude-fable-latest",
			expectedModel:    "claude-fable-latest",
			expectedUpstream: "~anthropic/claude-fable-latest",
		},
		{
			name:             "strips arbitrary tilde provider namespace",
			channelType:      constant.ChannelTypeOpenRouter,
			upstreamModel:    "~moonshotai/kimi-latest",
			expectedModel:    "kimi-latest",
			expectedUpstream: "~moonshotai/kimi-latest",
		},
		{
			name:             "preserves OpenRouter synthetic model ID",
			channelType:      constant.ChannelTypeOpenRouter,
			upstreamModel:    "openrouter/auto",
			expectedModel:    "openrouter/auto",
			expectedUpstream: "openrouter/auto",
		},
		{
...
func TestCanonicalizeChannelModels(t *testing.T) {
	tests := []struct {
		...
		{
			name: "canonicalizes arbitrary OpenRouter namespaces and preserves router models",
			models: []string{
