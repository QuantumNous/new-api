...
func InitOptionMap() {
	...
	common.OptionMap["ModelConcurrentLimitEnabled"] = strconv.FormatBool(setting.ModelConcurrentLimitEnabled)
	common.OptionMap["ModelConcurrentLimit"] = strconv.Itoa(setting.ModelConcurrentLimit)
	common.OptionMap["ModelConcurrentLimitGroup"] = setting.ModelConcurrentLimitGroup2JSONString()
...
func updateOptionMap(key string, value string) (err error) {
	...
	if strings.HasSuffix(key, "Enabled") || key == "DefaultCollapseSidebar" || key == "DefaultUseAutoGroup" || key == "SMTPForceAuthLogin" || key == "SMTPInsecureSkipVerify" {
		...
		case "ModelConcurrentLimitEnabled":
			setting.ModelConcurrentLimitEnabled = boolValue
	...
	case "ModelConcurrentLimit":
		setting.ModelConcurrentLimit, _ = strconv.Atoi(value)
	case "ModelConcurrentLimitGroup":
		err = setting.UpdateModelConcurrentLimitGroupByJSONString(value)
	case "RetryTimes":
