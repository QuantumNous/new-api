...
func GetPasskeyByUserID(userID int) (*PasskeyCredential, error) {
	if userID == 0 {
		common.SysLog(i18n.Translate(i18n.DefaultLang, "model.getpasskeybyuserid_empty_user_id"))
	...
	if err := DB.Where("user_id = ?", userID).First(&credential).Error; err != nil {
		...
		// 只有真正的数据库错误才记录日志
		common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "model.getpasskeybyuserid_database_error_for_user"), userID, err))
		return nil, ErrFriendlyPasskeyNotFound
	}
...
func GetPasskeyByCredentialID(credentialID []byte) (*PasskeyCredential, error) {
	if len(credentialID) == 0 {
		common.SysLog(i18n.Translate(i18n.DefaultLang, "model.getpasskeybycredentialid_empty_credential_id"))
		return nil, ErrFriendlyPasskeyNotFound
	}
	...
	if err := DB.Where("credential_id = ?", credIDStr).First(&credential).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "model.getpasskeybycredentialid_passkey_not_found_for_credential_id"), len(credentialID)))
			return nil, ErrFriendlyPasskeyNotFound
		}
		common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "model.getpasskeybycredentialid_database_error_for_credential_id"), err))
		return nil, ErrFriendlyPasskeyNotFound
	}
...
func UpsertPasskeyCredential(credential *PasskeyCredential) error {
	if credential == nil {
		common.SysLog(i18n.Translate(i18n.DefaultLang, "model.upsertpasskeycredential_nil_credential_provided"))
		return errors.New(i18n.Translate(i18n.DefaultLang, "passkey.save_failed"))
	}
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Unscoped().Where("user_id = ?", credential.UserID).Delete(&PasskeyCredential{}).Error; err != nil {
			common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "model.upsertpasskeycredential_failed_to_delete_existing_credential_for"), credential.UserID, err))
			return errors.New(i18n.Translate(i18n.DefaultLang, "passkey.save_failed"))
		}
		if err := tx.Create(credential).Error; err != nil {
			common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "model.upsertpasskeycredential_failed_to_create_credential_for_user"), credential.UserID, err))
			return errors.New(i18n.Translate(i18n.DefaultLang, "passkey.save_failed"))
		}
		return nil
	})
...
func DeletePasskeyByUserID(userID int) error {
	if userID == 0 {
		common.SysLog(i18n.Translate(i18n.DefaultLang, "model.deletepasskeybyuserid_empty_user_id"))
		return errors.New(i18n.Translate(i18n.DefaultLang, "passkey.delete_failed"))
	}
	// 使用Unscoped()进行硬删除，避免唯一索引冲突
	if err := DB.Unscoped().Where("user_id = ?", userID).Delete(&PasskeyCredential{}).Error; err != nil {
		common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "model.deletepasskeybyuserid_failed_to_delete_passkey_for_user"), userID, err))
		return errors.New(i18n.Translate(i18n.DefaultLang, "passkey.delete_failed"))
	}
	return nil
}
