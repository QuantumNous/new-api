...
// RenewSubscriptionOrder handles subscription renewal (period changed).
// Finds the user's active UserSubscription for this order's plan,
// extends EndTime by one period and adds TotalAmount quota.
func RenewSubscriptionOrder(tradeNo string, providerPayload string) error {
	if tradeNo == "" {
		return errors.New("tradeNo is empty")
	}
	refCol := "`trade_no`"
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		// 续期只对已完成的订阅订单有效
		if order.Status != common.TopUpStatusSuccess {
			return fmt.Errorf("subscription order status is %s, expected success", order.Status)
		}
		plan, err := GetSubscriptionPlanById(order.PlanId)
		if err != nil {
			return err
		...
		// 查找该用户该套餐最新的 active UserSubscription
		var sub UserSubscription
		if err := tx.Where("user_id = ? AND plan_id = ? AND status = ?", order.UserId, order.PlanId, "active").
			Order("end_time desc, id desc").
			First(&sub).Error; err != nil {
			// 没有 active 的订阅（可能已过期），走新建逻辑
			_, err = CreateUserSubscriptionFromPlanTx(tx, order.UserId, plan, "renewal")
			if err != nil {
				return err
			}
			logAmountAdded = plan.TotalAmount
		} else {
			// 在已有的额度池上续期：延长 EndTime + 增加 AmountTotal
			now := time.Now()
			oldEnd := time.Unix(sub.EndTime, 0)
			// 从较晚的时间点开始计算新周期（避免回调延迟导致 EndTime 缩短）
			base := oldEnd
			if now.After(oldEnd) {
				base = now
			}
			...
			// 如果 NextResetTime 已过期或为0，重新计算
			if sub.NextResetTime > 0 && sub.NextResetTime <= now.Unix() {
				sub.NextResetTime = calcNextResetTime(now, plan, newEnd)
			} else if sub.NextResetTime == 0 {
				// reset period 是 never，不需要更新
			}
		...
		// 更新 provider payload 记录最新的续期回调数据
		if providerPayload != "" {
			order.ProviderPayload = providerPayload
			if err := tx.Save(&order).Error; err != nil {
				return err
	...
	if logUserId > 0 {
		msg := fmt.Sprintf("订阅续期成功，套餐: %s，新增额度: %d", logPlanTitle, logAmountAdded)
		RecordLog(logUserId, LogTypeTopup, msg)
	}
	return nil
}
