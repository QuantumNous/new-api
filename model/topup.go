...
func RechargeAlipayF2F(tradeNo string, providerPayload string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("支付宝当面付充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money))
	}
