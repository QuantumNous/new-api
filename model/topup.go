...
// RechargeAirwallex marks a pending Airwallex TopUp row as success and credits
// the user's quota. Mirrors the Stripe path: TopUp.Money is the charged amount
// in the user's chosen currency (already group-ratio-adjusted at intent
...
func RechargeAirwallex(tradeNo string, callerIp string, customerId, consentId, pmId, originalTxnId string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.Status == common.TopUpStatusSuccess {
			return nil // idempotent: webhook may retry
		}
	...
	if err != nil {
		common.SysError("airwallex topup failed: " + err.Error())
		return errors.New("充值失败，请稍后重试")
	}
