...
var ErrTwoFANotEnabled = errors.New("2FA is not enabled for user")
...
func (t *TwoFA) Create() error {
	...
	if existing != nil {
		return errors.New("2FA settings already exist for user")
	}
...
func ValidateBackupCode(userId int, code string) (bool, error) {
	if !common.ValidateBackupCode(code) {
		return false, errors.New("verification code or backup code is invalid")
	}
...
func (t *TwoFA) ValidateTOTPAndUpdateUsage(code string) (bool, error) {
	...
	if !common.ValidateTOTPCode(t.Secret, code) {
		...
		if err := t.IncrementFailedAttempts(); err != nil {
			common.SysLog("failed to update 2FA failed attempts: " + err.Error())
		}
		return false, nil
	}
