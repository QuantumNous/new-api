...
func TestGoBucketMirrorMatchesSQLBucketsAcrossMonthBoundary(t *testing.T) {
	...
	for _, granularity := range []struct {
		...
		t.Run(granularity.name, func(t *testing.T) {
			...
			assert.Equal(t, common.DashboardBucketStart(julyLateNight, granularity.step), rows[0].Period)
			assert.Equal(t, common.DashboardBucketStart(augustEarly, granularity.step), rows[1].Period)
