...
func BatchUpdateUserGroup(ids []int, group string) (int64, error) {
	...
	chunks := [][]int{ids}
	if common.UsingMainDatabase(common.DatabaseTypeSQLite) && len(ids) > sqliteChunk {
		chunks = chunks[:0]
		...
		gopool.Go(func() {
			for _, id := range ids {
				if err := RefreshUserGroupCache(id); err != nil {
					common.SysError(fmt.Sprintf("failed to refresh batch user group cache for user %d: %s", id, err))
				}
			}
		})
	}
