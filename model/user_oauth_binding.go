...
func CreateUserOAuthBinding(binding *UserOAuthBinding) error {
	if binding.UserId == 0 {
		return errors.New(i18n.Translate(i18n.DefaultLang, "model.user_id_is_required"))
	}
	if binding.ProviderId == 0 {
		return errors.New(i18n.Translate(i18n.DefaultLang, "model.provider_id_is_required"))
	}
	if binding.ProviderUserId == "" {
		return errors.New(i18n.Translate(i18n.DefaultLang, "model.provider_user_id_is_required"))
	}

	// Check if this provider user ID is already taken
	if IsProviderUserIdTaken(binding.ProviderId, binding.ProviderUserId) {
		return errors.New(i18n.Translate(i18n.DefaultLang, "model.this_oauth_account_is_already_bound_to_another"))
	}
