...
func (p *DiscordProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	logger.LogDebug(ctx, i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_exchangetoken_code"), code[:min(len(code), 10)])
	...
	logger.LogDebug(ctx, i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_exchangetoken_redirect_uri"), redirectUri)
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_exchangetoken_error"), err.Error()))
		return nil, NewOAuthErrorWithRaw("oauth.connect_failed", map[string]any{"Provider": "Discord"}, err.Error())
	}
	defer res.Body.Close()

	logger.LogDebug(ctx, i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_exchangetoken_response_status"), res.StatusCode)

	var discordResponse discordOAuthResponse
	err = json.NewDecoder(res.Body).Decode(&discordResponse)
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_exchangetoken_decode_error"), err.Error()))
		return nil, err
	}

	if discordResponse.AccessToken == "" {
		logger.LogError(ctx, i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_exchangetoken_failed_empty_access_token"))
		return nil, NewOAuthError("oauth.token_failed", map[string]any{"Provider": "Discord"})
	}

	logger.LogDebug(ctx, i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_exchangetoken_success_scope"), discordResponse.Scope)
...
func (p *DiscordProvider) GetUserInfo(ctx context.Context, token *OAuthToken) (*OAuthUser, error) {
	logger.LogDebug(ctx, i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_getuserinfo_fetching_user_info"))
	...
	res, err := client.Do(req)
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_getuserinfo_error"), err.Error()))
		return nil, NewOAuthErrorWithRaw("oauth.connect_failed", map[string]any{"Provider": "Discord"}, err.Error())
	}
	defer res.Body.Close()

	logger.LogDebug(ctx, i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_getuserinfo_response_status"), res.StatusCode)

	if res.StatusCode != http.StatusOK {
		logger.LogError(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_getuserinfo_failed_status"), res.StatusCode))
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_getuserinfo_decode_error"), err.Error()))
		return nil, err
	}

	if discordUser.UID == "" || discordUser.ID == "" {
		logger.LogError(ctx, i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_getuserinfo_failed_empty_user_fields"))
		return nil, NewOAuthError("oauth.user_info_empty", map[string]any{"Provider": "Discord"})
	}

	logger.LogDebug(ctx, i18n.Translate(i18n.DefaultLang, "oauth.oauth_discord_getuserinfo_success_uid_username_name"), discordUser.UID, discordUser.ID, discordUser.Name)
