...
func LoadCustomProviders() error {
	...
	if err != nil {
		common.SysError(i18n.Translate(i18n.DefaultLang, "oauth.failed_to_load_custom_oauth_providers") + err.Error())
		return err
	}
	...
	for _, config := range customProviders {
		...
		common.SysLog(i18n.Translate(i18n.DefaultLang, "oauth.loaded_custom_oauth_provider") + config.Name + " (" + config.Slug + ")")
	}

	common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "oauth.loaded_custom_oauth_providers"), len(customProviders)))
	return nil
}
