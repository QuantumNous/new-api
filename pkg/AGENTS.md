...
## Purpose
内部工具包集合，每个子包封装一个独立的基础设施能力，供业务层按需引用。当前包含：计费表达式引擎（billingexpr）、混合缓存（cachex）、IO.NET 云 GPU 客户端（ionet）、性能指标采集（perf_metrics）。

## Subdirectories
| Directory | Purpose |
|-----------|---------|
| `billingexpr/` | **Rule 7 核心**：基于表达式语言的动态计费引擎，支持分级定价、多 token 维度、版本化。详见子目录 AGENTS.md 和 `expr.md` |
...
- `billingexpr` 受 **Rule 7** 保护，修改前必须阅读 `pkg/billingexpr/expr.md`。
