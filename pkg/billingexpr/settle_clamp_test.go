...
// that an oversized tiered settlement clamps to the 64-bit max instead of
...
func TestComputeTieredQuota_ClampOnOverflow(t *testing.T) {
	...
	// which far exceeds the supported 64-bit range and must saturate.
	...
	assert.Equal(t, common.MaxQuota, result.ActualQuotaAfterGroup, "oversized quota must clamp to 64-bit max, never wrap negative")
