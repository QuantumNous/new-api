...
func oaiImage2AliImageRequest(info *relaycommon.RelayInfo, request dto.ImageRequest, isSync bool) (*AliImageRequest, error) {
	...
	if request.Extra != nil {
		if val, ok := request.Extra["parameters"]; ok {
			...
			if err != nil {
				return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.invalid_parameters_field"), err)
			}
		} else {
		...
		if val, ok := request.Extra["input"]; ok {
			...
			if err != nil {
				return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.invalid_input_field"), err)
			}
		}
	}
...
func getImageBase64sFromForm(c *gin.Context, fieldName string) ([]string, error) {
	...
	for _, file := range imageFiles {
		image, err := file.Open()
		if err != nil {
			return nil, errors.New(i18n.Translate(i18n.DefaultLang, "relay.failed_to_open_image_file_14d3"))
		}

		// 读取文件内容
		imageData, err := io.ReadAll(image)
		if err != nil {
			return nil, errors.New(i18n.Translate(i18n.DefaultLang, "relay.failed_to_read_image_file"))
		}
		...
		// 构造data URL格式
		dataURL := fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.data_base64"), mimeType, base64Data)
...
func oaiFormEdit2AliImageEdit(c *gin.Context, info *relaycommon.RelayInfo, request dto.ImageRequest) (*AliImageRequest, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.get_image_base64s_from_form_failed"), err)
	}
	//dto.MediaContent{}
...
func updateTask(info *relaycommon.RelayInfo, taskID string) (*AliResponse, error, []byte) {
	...
	if err != nil {
		common.SysLog(i18n.Translate(i18n.DefaultLang, "relay.updatetask_client_do_err") + err.Error())
		return &aliResponse, err, nil
	}
	...
	if err != nil {
		common.SysLog(i18n.Translate(i18n.DefaultLang, "relay.updatetask_newdecoder_err") + err.Error())
		return &aliResponse, err, nil
	}
...
func asyncTaskWait(c *gin.Context, info *relaycommon.RelayInfo, taskID string) (*AliResponse, []byte, error) {
	...
	for {
		logger.LogDebug(c, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.asynctaskwait_step_wait_econds"), step, maxStep, waitSeconds))
		step++
	...
	return nil, nil, errors.New(i18n.Translate(i18n.DefaultLang, "relay.aliasynctaskwait_timeout"))
}
...
func aliImageHandler(a *Adaptor, c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*types.NewAPIError, *dto.Usage) {
	...
	if a.IsSyncImageModel {
		logger.LogDebug(c, i18n.Translate(i18n.DefaultLang, "relay.ali_sync_image_result")+string(originRespBody))
	} else {
		logger.LogDebug(c, i18n.Translate(i18n.DefaultLang, "relay.ali_async_image_result")+string(originRespBody))
	}
