...
func HandleStreamFinalResponse(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo) {
	...
	if claudeInfo.Usage.CompletionTokens == 0 || !claudeInfo.Done {
		...
		if claudeInfo.Usage.PromptTokens > 0 && claudeInfo.Usage.CompletionTokens == 0 {
			...
			logger.LogWarn(c, fmt.Sprintf("输入有值但输出为空, requestId: %s, model: %s, channelId: %d, promptTokens: %d, streamDone: %t, stopReason: %s, responseText: %q",
				info.RequestId, info.UpstreamModelName, info.ChannelId, claudeInfo.Usage.PromptTokens, claudeInfo.Done, stopReason, truncatedResponse))
		}
