			...
			if message.Role == "tool" {
				...
				for _, mediaMessage := range message.ParseContent() {
					...
					if mediaMessage.Type == "text" {
						...
						if err != nil {
							return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.get_file_data_failed"), err.Error())
						}
				...
				if message.ToolCalls != nil {
					for _, toolCall := range message.ParseToolCalls() {
						...
						if err := json.Unmarshal([]byte(toolCall.Function.Arguments), &inputObj); err != nil {
							common.SysLog(i18n.Translate(i18n.DefaultLang, "relay.tool_call_function_arguments_is_not_a") + fmt.Sprintf("%v", toolCall.Function.Arguments))
							continue
						}
...
func ResponseClaude2OpenAI(claudeResponse *dto.ClaudeResponse) *dto.OpenAITextResponse {
	...
	fullTextResponse := dto.OpenAITextResponse{
		Id:      fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.chatcmpl"), common.GetUUID()),
...
func HandleStreamFinalResponse(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo) {
	...
	if claudeInfo.Usage.CompletionTokens == 0 || !claudeInfo.Done {
		if common.DebugEnabled {
			common.SysLog(i18n.Translate(i18n.DefaultLang, "relay.claude_response_usage_is_not_complete_maybe"))
		}
	...
	if info.RelayFormat == types.RelayFormatClaude {
		...
		if info.ShouldIncludeUsage {
			...
			err := helper.ObjectData(c, response)
			if err != nil {
				common.SysLog(i18n.Translate(i18n.DefaultLang, "relay.send_final_response_failed") + err.Error())
			}
		}
		helper.Done(c)
	}
