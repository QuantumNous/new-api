...
func sizeToResolution(size string) (string, error) {
	...
	return "", fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.invalid_size"), size)
}
...
func ProcessAliOtherRatios(aliReq *AliVideoRequest) (map[string]float64, error) {
	...
	if otherRatio, ok := aliRatios[aliReq.Model]; ok {
		if ratio, ok := otherRatio[resolution]; ok {
			otherRatios[fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.resolution"), resolution)] = ratio
		}
	}
...
func (a *TaskAdaptor) convertToAliRequest(info *relaycommon.RelayInfo, req relaycommon.TaskSubmitReq) (*AliVideoRequest, error) {
	...
	if req.Size != "" {
		...
		if strings.Contains(req.Model, "t2v") && !strings.Contains(req.Size, "*") {
			return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.invalid_size_example"), req.Size, "1920*1080")
		}
	...
	if aliReq.Model != upstreamModel {
		return nil, errors.New(i18n.Translate(i18n.DefaultLang, "relay.can_t_change_model_with_metadata"))
	}
...
func (a *TaskAdaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (taskID string, taskData []byte, taskErr *dto.TaskError) {
	...
	if aliResp.Output.TaskID == "" {
		taskErr = service.TaskErrorWrapper(errors.New(i18n.Translate(i18n.DefaultLang, "relay.task_id_is_empty")), "invalid_response", http.StatusInternalServerError)
		return
	}
...
func (a *TaskAdaptor) ParseTaskResult(respBody []byte) (*relaycommon.TaskInfo, error) {
	...
	case "FAILED", "CANCELED", "UNKNOWN":
		...
		if aliResp.Message != "" {
			...
			taskResult.Reason = fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.task_failed_code_message"), aliResp.Output.Code, aliResp.Output.Message)
		} else {
