...
func newBillingRelayInfo(modelName string, modelPrice float64) *relaycommon.RelayInfo {
	info := &relaycommon.RelayInfo{
		...
		ChannelMeta: &relaycommon.ChannelMeta{
			ChannelBaseUrl:    "https://api2.example",
			ApiKey:            "test-key",
...
// ValidateBasicTaskRequest) so the duration and resolution under test are the
// ones production actually stores.
...
func TestEstimateBillingConfiguredModelPricesPerSecond(t *testing.T) {
	installVideoPriceRules(t, `[
		{"model":"bytedance/seedance-2.0","match":{"resolution":"1080p"},"price_per_second":0.5,"basis":"output_duration"}
	]`)

	a, c, info := newBillingRequest(t,
		`{"model":"bytedance/seedance-2.0","prompt":"a cat","duration":8,"resolution":"1080p"}`,
		"bytedance/seedance-2.0", 0.5)
...
// The seconds billed must be the ones the upstream body carries. This channel's
// upstream field is `seconds`, a STRING that convertToRequestPayload renders
// from the inbound int duration, so billing has to parse it back rather than
// assume a number.
func TestEstimateBillingFollowsTheUpstreamBody(t *testing.T) {
	installVideoPriceRules(t, `[
		...
		{"model":"bytedance/seedance-2.0","match":{"resolution":"1080p"},"price_per_second":0.4,"basis":"output_duration"}
	]`)
	...
	for _, tc := range []struct {
		...
		{"720p 5s", `{"model":"bytedance/seedance-2.0","prompt":"a cat","duration":5,"resolution":"720p"}`, 0.2, 5},
		{"1080p 10s", `{"model":"bytedance/seedance-2.0","prompt":"a cat","duration":10,"resolution":"1080p"}`, 0.4, 10},
		{"case folded label", `{"model":"bytedance/seedance-2.0","prompt":"a cat","duration":10,"resolution":"720P"}`, 0.2, 10},
		// duration also accepts a JSON string at the top level.
		{"string duration", `{"model":"bytedance/seedance-2.0","prompt":"a cat","duration":"8","resolution":"720p"}`, 0.2, 8},
	} {
...
func TestEstimateBillingUnconfiguredModelStaysPerCall(t *testing.T) {
	...
	a, c, info := newBillingRequest(t,
		`{"model":"bytedance/seedance-2.0","prompt":"a cat","duration":8,"resolution":"720p"}`,
		"bytedance/seedance-2.0", 0.5)
	if ratios := a.EstimateBilling(c, info); len(ratios) != 0 {
...
// The upstream length field is a string, so a value that does not parse as a
// positive whole number of seconds must leave the request on the per-call path
// rather than be coerced to some number.
func TestBillableSeconds(t *testing.T) {
	for _, tc := range []struct {
		...
		{"eight", 0, false},
		{"8s", 0, false},
		// A fractional length is not a whole number of seconds; refusing is
		// safer than truncating a customer's request to a cheaper tier.
		{"8.5", 0, false},
		{"0", 0, false},
		{"-4", 0, false},
	} {
		t.Run(tc.seconds, func(t *testing.T) {
			got, ok := billableSeconds(tc.seconds)
			if ok != tc.wantOK {
...
func TestEstimateBillingWithoutTaskRequestCapturesNothing(t *testing.T) {
	...
	c := newJSONCtx(`{"model":"bytedance/seedance-2.0","prompt":"a cat","duration":8,"resolution":"720p"}`)
	info := newBillingRelayInfo("bytedance/seedance-2.0", 0.5)
	a := &TaskAdaptor{}
	if ratios := a.EstimateBilling(c, info); len(ratios) != 0 {
