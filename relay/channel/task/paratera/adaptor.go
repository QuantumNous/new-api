...
// 并行（Paratera）视频 Channel：复用 MiniMax Hailuo 的请求/响应语义，
// 端点路径在 MiniMax 官方路径基础上加 /p004 前缀。
...
func (a *TaskAdaptor) DoResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (taskID string, taskData []byte, taskErr *dto.TaskError) {
	...
	if hResp.BaseResp.StatusCode != StatusSuccess {
		taskErr = service.TaskErrorWrapper(
			fmt.Errorf("paratera api error: %s", hResp.BaseResp.StatusMsg),
...
func (a *TaskAdaptor) convertToRequestPayload(req *relaycommon.TaskSubmitReq, info *relaycommon.RelayInfo) (*VideoRequest, error) {
	...
	// 标准 task 字段 image / images → first_frame_image，跟 kling/doubao 一致；
	// 若未填，则退一步看 metadata.first_frame_image
	if req.Image != "" {
	...
	// 强制锁回网关侧选定的 model，防止 metadata.model 绕过鉴权/计费
...
func (a *TaskAdaptor) ConvertToOpenAIVideo(originTask *model.Task) ([]byte, error) {
	var hResp QueryTaskResponse
	if err := common.Unmarshal(originTask.Data, &hResp); err != nil {
		return nil, errors.Wrap(err, "unmarshal paratera task data failed")
	}

	openAIVideo := originTask.ToOpenAIVideo()
	if hResp.BaseResp.StatusCode != StatusSuccess {
		openAIVideo.Error = &dto.OpenAIVideoError{
			Message: hResp.BaseResp.StatusMsg,
			Code:    strconv.Itoa(hResp.BaseResp.StatusCode),
		}
	}
...
func (a *TaskAdaptor) buildVideoURL(_, fileID string) string {
	...
	url := fmt.Sprintf("%s%s?file_id=%s", a.baseURL, FileRetrieveEndpoint, fileID)
