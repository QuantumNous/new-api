...
func newVertexTestContext(body string) *gin.Context {
...
func newVertexRelayInfo(originModel, upstreamModel string, modelPrice float64) *relaycommon.RelayInfo {
	info := &relaycommon.RelayInfo{
		OriginModelName: originModel,
		ChannelMeta: &relaycommon.ChannelMeta{
			ChannelBaseUrl:    "https://aiplatform.googleapis.example",
			ApiKey:            "{}",
...
// newVertexBillingRequest walks the real inbound path so the duration and
// resolution defaults under test are the ones production actually applies.
func newVertexBillingRequest(t *testing.T, body, originModel, upstreamModel string, modelPrice float64) (*TaskAdaptor, *gin.Context, *relaycommon.RelayInfo) {
	t.Helper()
	c := newVertexTestContext(body)
	info := newVertexRelayInfo(originModel, upstreamModel, modelPrice)
	a := &TaskAdaptor{}
...
// installVertexVideoPriceRules loads a rule set into the live config and
...
func installVertexVideoPriceRules(t *testing.T, rulesJSON string) {
	t.Helper()
...
// submit time.
func TestVertexResolveDimensions(t *testing.T) {
...
func TestVertexResolveDimensions_RejectsUnknown(t *testing.T) {
...
func TestVertexSecondBillingRatios_UsesConfiguredPrice(t *testing.T) {
	a := &TaskAdaptor{}
...
func TestVertexSecondBillingRatios_NotCapturedIsNoOp(t *testing.T) {
	a := &TaskAdaptor{}
	got, err := a.SecondBillingRatios()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
...
func TestVertexSecondBillingRatios_ConfiguredButUnmatchedErrors(t *testing.T) {
	a := &TaskAdaptor{}
	a.secondBillingModel = "m1"
...
func TestVertexEstimateBillingConfiguredModelPricesPerSecond(t *testing.T) {
	installVertexVideoPriceRules(t, `[
		{"model":"veo-3.1-generate-preview","match":{"resolution":"1080p"},
		 "price_per_second":0.4,"basis":"output_duration"}
	]`)

	a, c, info := newVertexBillingRequest(t,
...
func TestVertexEstimateBillingCoversEveryResolutionSource(t *testing.T) {
	installVertexVideoPriceRules(t, `[
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			a, c, info := newVertexBillingRequest(t, tc.body,
				"veo-3.1-generate-preview", "veo-3.1-generate-preview", 0.4)
...
func TestVertexEstimateBillingUnconfiguredModelKeepsLegacyRatios(t *testing.T) {
	installVertexVideoPriceRules(t, `[
...
func TestVertexEstimateBillingConfiguredModelWithNoMatchingRuleFails(t *testing.T) {
	installVertexVideoPriceRules(t, `[
		{"model":"veo-3.1-generate-preview","match":{"resolution":"720p"},"price_per_second":0.4,"basis":"output_duration"}
	]`)
...
// guessed tier — and for a configured model, ComputeSecondBilling then refuses
// it loudly rather than reserving a wrong amount.
func TestVertexEstimateBillingUnknownResolutionIsNotGuessed(t *testing.T) {
	installVertexVideoPriceRules(t, `[
		{"model":"some-other-model","match":{"resolution":"720p"},"price_per_second":0.4,"basis":"output_duration"}
	]`)
...
// must key on info.OriginModelName. Vertex maps that name to an upstream Veo
// model id; keying on the upstream name would divide one model's per-second
// rate by another model's price.
func TestVertexEstimateBillingKeysTableOnOriginModelNameNotUpstream(t *testing.T) {
	installVertexVideoPriceRules(t, `[
		{"model":"veo-3.1-generate-preview","match":{"resolution":"720p"},"price_per_second":0.4,"basis":"output_duration"}
	]`)
...
func TestVertexEstimateBillingConfiguredButUnpriceableErrors(t *testing.T) {
	installVertexVideoPriceRules(t, `[
