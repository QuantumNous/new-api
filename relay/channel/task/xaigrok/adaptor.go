...
//
// Grok Imagine exposes no output resolution parameter — the upstream submit
// body is model / prompt / image / duration — so no resolution key is reported;
...
// alike. has_video is always "false" (the optional reference is an image, never
// a video) and is emitted anyway so rules stay uniform across channels.
...
func (a *TaskAdaptor) EstimateBilling(c *gin.Context, info *relaycommon.RelayInfo) map[string]float64 {
	...
	// Not the upstream name: the two Grok models carry different rates, so
	// keying on the mapped name would divide one model's rate by the other's
	// price. seconds is always positive here (defaulted just above), so there
	// is no unknowable-length case to skip.
	if info != nil {
