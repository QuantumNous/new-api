...
// TaskAdaptor implements yk-video (KYY model-center) async video API.
// Create: POST /v2/model-center/tasks ; Query: GET /v2/model-center/tasks/{id}
...
func apiOrigin(raw string) string {
	...
	for _, suf := range []string{createPath, "/v2/model-center/tasks", "/v2/model-center", "/v2"} {
...
func (a *TaskAdaptor) ValidateRequestAndSetAction(c *gin.Context, info *relaycommon.RelayInfo) *dto.TaskError {
	if info.Action == constant.TaskActionRemix {
		return service.TaskErrorWrapperLocal(fmt.Errorf("remix is not supported for yk-video"), "not_supported", http.StatusBadRequest)
	}
...
func (a *TaskAdaptor) BuildRequestBody(c *gin.Context, info *relaycommon.RelayInfo) (io.Reader, error) {
	...
	normalizeCreateBody(body, modelName)
