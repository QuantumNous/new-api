...
func tencentStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if err := scanner.Err(); err != nil {
		common.SysLog(i18n.Translate(i18n.DefaultLang, "relay.error_reading_stream") + err.Error())
	}
...
func parseTencentConfig(config string) (appId int64, secretId string, secretKey string, err error) {
	...
	if len(parts) != 3 {
		err = errors.New(i18n.Translate(i18n.DefaultLang, "relay.invalid_tencent_config"))
		return
	}
...
func getTencentSign(req TencentChatRequest, adaptor *Adaptor, secId, secKey string) string {
	...
	canonicalHeaders := fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.content_type_nhost_nx_tc_action_n"),
	...
	// build authorization
	authorization := fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.credential_signedheaders_signature"),
		algorithm,
		secId,
		credentialScope,
		signedHeaders,
