...
func buildStorageRequest(ctx context.Context, input StorageProxyRequest) (*http.Request, error) {
	...
	request.Header = sanitizeStorageHeader(input.Header, true)
...
// sanitizeStorageResponseHeader strips hop-by-hop headers and forbids shared
// caches from reusing a response across gateway tokens.
...
// sanitizeStorageHeader drops hop-by-hop headers, plus every client credential
// header on the request side so a caller can never authenticate to Google with
// anything but the channel's service account token.
func sanitizeStorageHeader(source http.Header, request bool) http.Header {
	...
	for name, values := range source {
		...
		for _, value := range values {
			for nominated := range strings.SplitSeq(value, ",") {
