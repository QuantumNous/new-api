...
func (t MsgType) String() string {
	...
	default:
		return fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.msgtype"), t)
	}
}
...
func (t EventType) String() string {
	...
	default:
		return fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.eventtype"), t)
	}
}
...
func NewMessageFromBytes(data []byte) (*Message, error) {
	if len(data) < 3 {
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.data_too_short_expected_at_least_3_bytes"), len(data))
	}
...
func (m *Message) String() string {
	...
	case MsgTypeAudioOnlyServer, MsgTypeAudioOnlyClient:
		if m.MsgTypeFlag == MsgTypeFlagPositiveSeq || m.MsgTypeFlag == MsgTypeFlagNegativeSeq {
			return fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.sequence_payloadsize"), m.MsgType, m.EventType, m.Sequence, len(m.Payload))
		}
		return fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.payloadsize"), m.MsgType, m.EventType, len(m.Payload))
	case MsgTypeError:
		return fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.errorcode_payload"), m.MsgType, m.EventType, m.ErrorCode, string(m.Payload))
	default:
		if m.MsgTypeFlag == MsgTypeFlagPositiveSeq || m.MsgTypeFlag == MsgTypeFlagNegativeSeq {
			return fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.sequence_payload"),
				m.MsgType, m.EventType, m.Sequence, string(m.Payload))
		}
		return fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.payload"), m.MsgType, m.EventType, string(m.Payload))
	}
}
...
func (m *Message) Unmarshal(data []byte) error {
	...
	if paddingSize := headerSize - readSize; paddingSize > 0 {
		if n, err := buf.Read(make([]byte, paddingSize)); err != nil || n < paddingSize {
			return fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.insufficient_header_bytes_expected_got"), paddingSize, n)
		}
	}
	...
	if _, err := buf.ReadByte(); err != io.EOF {
		return fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.unexpected_data_after_message"), err)
	}
...
func (m *Message) writers() (writers []func(*bytes.Buffer) error, _ error) {
	...
	default:
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.unsupported_message_type"), m.MsgType)
	}
...
func (m *Message) writeSessionID(buf *bytes.Buffer) error {
	...
	if int64(size) > math.MaxUint32 {
		return fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.session_id_size_exceeds_max_uint32"), size)
	}
...
func (m *Message) writePayload(buf *bytes.Buffer) error {
	size := len(m.Payload)
	if int64(size) > math.MaxUint32 {
		return fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.payload_size_exceeds_max_uint32"), size)
	}
...
func ReceiveMessage(conn *websocket.Conn) (*Message, error) {
	...
	if mt != websocket.BinaryMessage && mt != websocket.TextMessage {
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.unexpected_websocket_message_type"), mt)
	}
