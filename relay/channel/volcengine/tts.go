...
func parseVolcengineAuth(apiKey string) (appID, token string, err error) {
	...
	if len(parts) != 2 {
		return "", "", errors.New(i18n.Translate(i18n.DefaultLang, "relay.invalid_api_key_format_expected_appid_access_token"))
	}
...
func handleTTSResponse(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo, encoding string) (usage any, err *types.NewAPIError) {
	...
	if readErr != nil {
		return nil, types.NewErrorWithStatusCode(
			errors.New(i18n.Translate(i18n.DefaultLang, "relay.failed_to_read_volcengine_response")),
	...
	if unmarshalErr := json.Unmarshal(body, &volcResp); unmarshalErr != nil {
		return nil, types.NewErrorWithStatusCode(
			errors.New(i18n.Translate(i18n.DefaultLang, "relay.failed_to_parse_volcengine_response")),
	...
	if decodeErr != nil {
		return nil, types.NewErrorWithStatusCode(
			errors.New(i18n.Translate(i18n.DefaultLang, "relay.failed_to_decode_audio_data")),
			types.ErrorCodeBadResponseBody,
			http.StatusInternalServerError,
		)
	}
...
func handleTTSWebSocketResponse(c *gin.Context, requestURL string, volcRequest VolcengineTTSRequest, info *relaycommon.RelayInfo, encoding string) (usage any, err *types.NewAPIError) {
	...
	header := http.Header{}
	header.Set("Authorization", fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.bearer"), token))

	conn, resp, dialErr := websocket.DefaultDialer.DialContext(context.Background(), requestURL, header)
	if dialErr != nil {
		if resp != nil {
			return nil, types.NewErrorWithStatusCode(
				fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.failed_to_connect_to_websocket_status"), dialErr, resp.StatusCode),
				types.ErrorCodeBadResponseStatusCode,
				http.StatusBadGateway,
			)
		}
		return nil, types.NewErrorWithStatusCode(
			fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.failed_to_connect_to_websocket"), dialErr),
			types.ErrorCodeBadResponseStatusCode,
			http.StatusBadGateway,
		)
	}
	...
	if marshalErr != nil {
		return nil, types.NewErrorWithStatusCode(
			fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.failed_to_marshal_request"), marshalErr),
	...
	if sendErr := FullClientRequest(conn, payload); sendErr != nil {
		return nil, types.NewErrorWithStatusCode(
			fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.failed_to_send_request_2cb6"), sendErr),
			types.ErrorCodeBadRequestBody,
			http.StatusInternalServerError,
		)
	}
	...
	for {
		...
		if recvErr != nil {
			...
			return nil, types.NewErrorWithStatusCode(
				fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.failed_to_receive_message"), recvErr),
				types.ErrorCodeBadResponse,
				http.StatusInternalServerError,
			)
		}
		...
		case MsgTypeError:
			return nil, types.NewErrorWithStatusCode(
				fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.received_error_from_server_code"), msg.ErrorCode, string(msg.Payload)),
				types.ErrorCodeBadResponse,
				http.StatusBadRequest,
			)
		...
		case MsgTypeAudioOnlyServer:
			if len(msg.Payload) > 0 {
				if _, writeErr := c.Writer.Write(msg.Payload); writeErr != nil {
					return nil, types.NewErrorWithStatusCode(
						fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.failed_to_write_audio_data"), writeErr),
						types.ErrorCodeBadResponse,
						http.StatusInternalServerError,
					)
				}
