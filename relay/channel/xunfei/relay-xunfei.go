...
func xunfeiStreamHandler(c *gin.Context, textRequest dto.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string) (*dto.Usage, *types.NewAPIError) {
	...
	c.Stream(func(w io.Writer) bool {
		...
		case xunfeiResponse := <-dataChan:
			...
			if err != nil {
				common.SysLog(i18n.Translate(i18n.DefaultLang, "relay.error_marshalling_stream_response") + err.Error())
				return true
			}
...
func xunfeiMakeRequest(textRequest dto.GeneralOpenAIRequest, domain, authUrl, appId string) (chan XunfeiChatResponse, chan bool, error) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				common.SysLog(i18n.Translate(i18n.DefaultLang, "relay.error_reading_stream_response") + err.Error())
				break
			}
			...
			if response.Payload.Choices.Status == 2 {
				if err != nil {
					common.SysLog(i18n.Translate(i18n.DefaultLang, "relay.error_closing_websocket_connection") + err.Error())
				}
				break
			}
		}
...
func getAPIVersion(c *gin.Context, modelName string) string {
	...
	apiVersion = "v1.1"
	common.SysLog(i18n.Translate(i18n.DefaultLang, "relay.api_version_not_found_using_default") + apiVersion)
	return apiVersion
}
