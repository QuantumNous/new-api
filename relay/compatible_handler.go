...
func TextHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if !ok {
		return types.NewErrorWithStatusCode(fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.invalid_request_type_expected_dto_generalopenairequest_got"), info.Request), types.ErrorCodeInvalidRequest, http.StatusBadRequest, types.ErrOptionWithSkipRetry())
	}

	request, err := common.DeepCopy(textReq)
	if err != nil {
		return types.NewError(fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.failed_to_copy_request_to_generalopenairequest"), err), types.ErrorCodeInvalidRequest, types.ErrOptionWithSkipRetry())
	}
	...
	if adaptor == nil {
		return types.NewError(fmt.Errorf(i18n.Translate(i18n.DefaultLang, "relay.invalid_api_type"), info.ApiType), types.ErrorCodeInvalidApiType, types.ErrOptionWithSkipRetry())
	}
	adaptor.Init(info)
	...
	if passThroughGlobal || info.ChannelSetting.PassThroughBodyEnabled {
		...
		logger.LogDebug(c, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.text_request_body"), string(jsonData)))
...
func postConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent ...string) {
	...
	if totalTokens == 0 {
		...
		logger.LogError(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "relay.total_tokens_is_0_cannot_consume_quota")+
