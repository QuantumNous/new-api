...
func ModelPriceHelper(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta) (types.PriceData, error) {
	...
	if !usePrice {
		...
		preConsumedQuota = billingexpr.QuotaFloor(float64(preConsumedTokens) * ratio)
	} else {
		if meta.ImagePriceRatio != 0 {
			modelPrice = modelPrice * meta.ImagePriceRatio
		}
		preConsumedQuota = billingexpr.QuotaFloor(modelPrice * common.QuotaPerUnit * groupRatioInfo.GroupRatio)
	}
...
func ModelPriceHelperPerCall(c *gin.Context, info *relaycommon.RelayInfo) (types.PriceData, error) {
	...
	if usePrice {
		quota = billingexpr.QuotaFloor(modelPrice * common.QuotaPerUnit * groupRatioInfo.GroupRatio)
		if !operation_setting.GetQuotaSetting().EnableFreeModelPreConsume {
		...
		// 按量计费：以模型倍率的一半作为预扣额度
		quota = billingexpr.QuotaFloor(modelRatio / 2 * common.QuotaPerUnit * groupRatioInfo.GroupRatio)
		modelPrice = -1
		if !operation_setting.GetQuotaSetting().EnableFreeModelPreConsume {
