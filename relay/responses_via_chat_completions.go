...
func responsesViaChatCompletions(c *gin.Context, info *relaycommon.RelayInfo, adaptor channel.Adaptor, request *dto.OpenAIResponsesRequest) (*dto.Usage, *types.NewAPIError) {
	...
	if isStream && info.IsStream {
		// Caller requested streaming: translate SSE chunks incrementally
		usage, newApiErr = oaiChatStreamToResponsesStreamHandler(c, info, httpResp)
	} else if isStream {
		// Upstream is streaming but caller did not request it: buffer into JSON
...
func oaiChatStreamToResponsesHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
		...
		for _, choice := range chunk.Choices {
			...
			for _, tc := range choice.Delta.ToolCalls {
				...
				for len(toolCalls) <= idx {
					toolCalls = append(toolCalls, dto.ToolCallResponse{Type: "function"})
				}
				if tc.ID != "" {
	...
	msg := dto.Message{Role: "assistant", Content: contentBuilder.String()}
	if len(toolCalls) > 0 {
...
// oaiChatStreamToResponsesStreamHandler reads a streaming Chat Completions
// response and translates each chunk into Responses API SSE events, emitting
// them incrementally. Use this when the caller requested stream=true.
func oaiChatStreamToResponsesStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	if resp == nil || resp.Body == nil {
		return nil, types.NewOpenAIError(nil, types.ErrorCodeBadResponse, http.StatusInternalServerError)
	}
	defer service.CloseResponseBodyGracefully(resp)
