...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/verification", middleware.EmailVerificationRateLimit(), middleware.CaptchaCheckFor(middleware.CaptchaSceneRegister), controller.SendEmailVerification)
		apiRouter.GET("/reset_password", middleware.CriticalRateLimit(), middleware.CaptchaCheckFor(middleware.CaptchaSceneReset), controller.SendPasswordResetEmail)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			userRoute.POST("/register", middleware.CriticalRateLimit(), anonymousRequestBodyLimit, middleware.CaptchaCheckFor(middleware.CaptchaSceneRegister), controller.Register)
			userRoute.POST("/login", middleware.CriticalRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, middleware.CaptchaCheckFor(middleware.CaptchaSceneLogin), controller.Login)
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				selfRoute.POST("/checkin", middleware.CaptchaCheckFor(middleware.CaptchaScene("checkin")), controller.DoCheckin)
		...
		dataRoute.GET("/channel_profit", middleware.AdminAuth(), controller.GetChannelProfit)
