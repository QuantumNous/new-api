...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		agentAdminReadRoute := apiRouter.Group("/agent-admin"); agentAdminReadRoute.Use(middleware.AdminAuth())
		agentAdminReadRoute.GET("/agents", controller.AdminListAgents); agentAdminReadRoute.GET("/agents/:user_id/credit-logs", controller.AdminListAgentCreditLogs)
		agentAdminReadRoute.GET("/offers", controller.AdminListAgentPlanOffers); agentAdminReadRoute.GET("/orders", controller.AdminListAgentOrders); agentAdminReadRoute.GET("/codes", controller.AdminListAgentCodes); agentAdminReadRoute.GET("/agents/:user_id/reconciliation", controller.AdminReconcileAgentAccount)
		agentAdminMutationRoute := apiRouter.Group("/agent-admin"); agentAdminMutationRoute.Use(middleware.RootAuth())
		agentAdminMutationRoute.POST("/agents/:user_id/enable", controller.RootEnableAgent); agentAdminMutationRoute.POST("/agents/:user_id/disable", controller.RootDisableAgent); agentAdminMutationRoute.PATCH("/agents/:user_id/limit", controller.RootUpdateAgentDailyLimit); agentAdminMutationRoute.POST("/agents/:user_id/credit-adjustments", controller.RootAdjustAgentCredit); agentAdminMutationRoute.PUT("/offers/:plan_id", controller.RootUpsertAgentPlanOffer); agentAdminMutationRoute.POST("/codes/refund", middleware.CriticalRateLimit(), controller.RootRefundAgentCodes)
		agentRoute := apiRouter.Group("/agent"); agentRoute.Use(middleware.UserAuth())
		agentRoute.GET("/overview", controller.GetAgentOverview); agentRoute.GET("/promotion", controller.GetAgentPromotion); agentRoute.GET("/offers", controller.GetAgentOffers); agentRoute.GET("/orders", controller.GetAgentOrders); agentRoute.GET("/codes", controller.GetAgentCodes); agentRoute.GET("/codes/export", controller.ExportAgentCodes); agentRoute.GET("/credit-logs", controller.GetAgentCreditLogs); agentRoute.GET("/customers", controller.GetAgentCustomers); agentRoute.GET("/logs", controller.GetAgentCustomerLogs); agentRoute.GET("/logs/stat", controller.GetAgentCustomerLogStats); agentRoute.POST("/orders", middleware.CriticalRateLimit(), controller.CreateAgentOrder); agentRoute.POST("/codes/refund", middleware.CriticalRateLimit(), controller.RefundAgentCodes)
