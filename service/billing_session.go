...
// 注意：企业折扣已在 PreConsumeBilling 中应用过一次，此处的折扣不应重复应用
func NewBillingSession(c *gin.Context, relayInfo *relaycommon.RelayInfo, preConsumedQuota int, discountRate float64) (*BillingSession, *types.NewAPIError) {
	...
	logger.LogInfo(c, fmt.Sprintf("开始创建 BillingSession：用户ID=%d, 模型名称=%s, 已应用折扣率=%.4f",
		relayInfo.UserId, modelName, discountRate))

	// 如果折扣已经在 PreConsumeBilling 中应用过（discountRate < 1.0），则不再重复应用
	// 只记录日志
	if discountRate < 1.0 {
		logger.LogInfo(c, fmt.Sprintf("企业折扣已在 PreConsumeBilling 中应用，跳过重复应用"))
	}
