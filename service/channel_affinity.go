...
func GetChannelAffinityCacheStats() ChannelAffinityCacheStats {
	...
	keys, err := cache.Keys()
	if err != nil {
		common.SysError(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.channel_affinity_cache_list_keys_failed_err"), err))
		keys = nil
	}
	total := len(keys)
	unknown := 0
...
func ClearChannelAffinityCacheAll() int {
	...
	if len(keys) > 0 {
		if _, err := cache.DeleteMany(keys); err != nil {
			common.SysError(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.channel_affinity_cache_delete_many_failed_err"), err))
		}
	}
	return len(keys)
}
...
func GetPreferredChannelByAffinity(c *gin.Context, modelName string, usingGroup string) (int, bool) {
	...
	for _, rule := range setting.Rules {
		...
		if err != nil {
			common.SysError(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.channel_affinity_cache_get_failed_key_err"), cacheKeyFull, err))
			return 0, false
		}
		if found {
...
func RecordChannelAffinity(c *gin.Context, channelID int) {
	...
	if err := cache.SetWithTTL(cacheKey, channelID, time.Duration(ttlSeconds)*time.Second); err != nil {
		common.SysError(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.channel_affinity_cache_set_failed_key_err"), cacheKey, err))
	}
}
