...
func cacheGetRandomSatisfiedChannel(
	...
	if param.TokenGroup == "auto" {
		...
		for i := startGroupIndex; i < len(autoGroups); i++ {
			...
			channel, _ = selectChannel(autoGroup, priorityRetry)
			if channel == nil {
			...
			if crossGroupRetry && priorityRetry >= maxRetries {
				// Current group has exhausted all retries, prepare to switch to next group
				// This request still uses current group, but next retry will use next group
				// 当前分组已用完所有重试次数，准备切换到下一个分组
				// 本次请求仍使用当前分组，但下次重试将使用下一个分组
				logger.LogDebug(param.Ctx, "Current group %s retries exhausted (priorityRetry=%d >= maxRetries=%d), preparing switch to next group for next retry", autoGroup, priorityRetry, maxRetries)
				common.SetContextKey(param.Ctx, constant.ContextKeyAutoGroupIndex, i+1)
				// Reset retry counter so outer loop can continue for next group
				// 重置重试计数器，以便外层循环可以为下一个分组继续
				param.SetRetry(0)
