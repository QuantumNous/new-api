...
func StartCodexCredentialAutoRefreshTask() {
	codexCredentialRefreshOnce.Do(func() {
		...
		gopool.Go(func() {
			logger.LogInfo(context.Background(), fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.codex_credential_auto_refresh_task_started_tick"), codexCredentialRefreshTickInterval, codexCredentialRefreshThreshold))
...
func runCodexCredentialAutoRefreshOnce() {
	...
	for {
		...
		err := model.DB.
			...
			Find(&channels).Error
		if err != nil {
			logger.LogError(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.codex_credential_auto_refresh_query_channels_failed"), err))
			return
		}
		if len(channels) == 0 {
			break
		...
		for _, ch := range channels {
			...
			cancel()
			if err != nil {
				logger.LogWarn(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.codex_credential_auto_refresh_channel_id_name"), ch.Id, ch.Name, err))
				continue
			}
	...
	if refreshed > 0 {
		func() {
			defer func() {
				if r := recover(); r != nil {
					logger.LogWarn(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.codex_credential_auto_refresh_initchannelcache_panic"), r))
				}
			}()
	...
	if common.DebugEnabled {
		logger.LogDebug(ctx, i18n.Translate(i18n.DefaultLang, "svc.codex_credential_auto_refresh_scanned_refreshed"), scanned, refreshed)
	}
}
