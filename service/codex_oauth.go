...
func refreshCodexOAuthToken(
	...
	if rt == "" {
		return nil, errors.New(i18n.Translate(i18n.DefaultLang, "svc.empty_refresh_token"))
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.codex_oauth_refresh_failed_status"), resp.StatusCode)
	}

	if strings.TrimSpace(payload.AccessToken) == "" || strings.TrimSpace(payload.RefreshToken) == "" || payload.ExpiresIn <= 0 {
		return nil, errors.New(i18n.Translate(i18n.DefaultLang, "svc.codex_oauth_refresh_response_missing_fields"))
	}
...
func exchangeCodexAuthorizationCode(
	...
	if c == "" {
		return nil, errors.New(i18n.Translate(i18n.DefaultLang, "svc.empty_authorization_code"))
	}
	if v == "" {
		return nil, errors.New(i18n.Translate(i18n.DefaultLang, "svc.empty_code_verifier"))
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.codex_oauth_code_exchange_failed_status"), resp.StatusCode)
	}
	if strings.TrimSpace(payload.AccessToken) == "" || strings.TrimSpace(payload.RefreshToken) == "" || payload.ExpiresIn <= 0 {
		return nil, errors.New(i18n.Translate(i18n.DefaultLang, "svc.codex_oauth_token_response_missing_fields"))
	}
	return &CodexOAuthTokenResult{
		AccessToken:  strings.TrimSpace(payload.AccessToken),
		RefreshToken: strings.TrimSpace(payload.RefreshToken),
...
func createStateHex(nBytes int) (string, error) {
	if nBytes <= 0 {
		return "", errors.New(i18n.Translate(i18n.DefaultLang, "svc.invalid_state_bytes_length"))
	}
	b := make([]byte, nBytes)
