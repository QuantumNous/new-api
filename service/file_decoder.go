...
func GetFileTypeFromUrl(c *gin.Context, url string, reason ...string) (string, error) {
	...
	if err != nil {
		common.SysLog(fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.fail_to_get_file_type_from_url"), url, err.Error()))
		return "", err
	}
	defer response.Body.Close()

	if response.StatusCode != 200 {
		logger.LogError(c, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_download_file_from_status_code"), url, response.StatusCode))
		return "", fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_download_file_status_code"), response.StatusCode)
	}
	...
	for _, limit := range limits {
		logger.LogDebug(c, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.trying_to_read_bytes_to_determine_file_type"), limit))
