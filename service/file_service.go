...
func getContextCacheKey(url string) string {
	return fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.file_cache"), common.GenerateHMAC(url))
}
...
func LoadFileSource(c *gin.Context, source *types.FileSource, reason ...string) (*types.CachedFileData, error) {
	if source == nil {
		return nil, errors.New(i18n.Translate(i18n.DefaultLang, "svc.file_source_is_nil"))
	}

	if common.DebugEnabled {
		logger.LogDebug(c, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.loadfilesource_starting_for"), source.GetIdentifier()))
	}
...
func loadFromURL(c *gin.Context, url string, reason ...string) (*types.CachedFileData, error) {
	...
	if common.DebugEnabled {
		logger.LogDebug(c, i18n.Translate(i18n.DefaultLang, "svc.loadfromurl_initiating_download"))
	}
	resp, err := DoDownloadRequest(url, reason...)
	if err != nil {
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_download_file_from"), url, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 {
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_download_file_status_code"), resp.StatusCode)
	}

	// 读取文件内容（限制大小）
	if common.DebugEnabled {
		logger.LogDebug(c, i18n.Translate(i18n.DefaultLang, "svc.loadfromurl_reading_response_body"))
	}
	fileBytes, err := io.ReadAll(io.LimitReader(resp.Body, int64(maxFileSize+1)))
	if err != nil {
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_read_file_content"), err)
	}
	if len(fileBytes) > maxFileSize {
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.file_size_exceeds_maximum_allowed_size_mb"), constant.MaxFileDownloadMB)
	}
	...
	if shouldUseDiskCache(base64Size) {
		...
		if err != nil {
			// 磁盘缓存失败，回退到内存
			logger.LogWarn(c, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_write_to_disk_cache_falling"), err))
			...
			if common.DebugEnabled {
				logger.LogDebug(c, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.file_cached_to_disk_size_bytes"), diskPath, base64Size))
			}
		}
	} else {
		// 使用内存缓存
	...
	if strings.HasPrefix(mimeType, "image/") {
		if common.DebugEnabled {
			logger.LogDebug(c, i18n.Translate(i18n.DefaultLang, "svc.loadfromurl_decoding_image_config"))
		}
...
func loadFromBase64(base64String string, providedMimeType string) (*types.CachedFileData, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_decode_base64_data"), err)
	}
...
func GetImageConfig(c *gin.Context, source *types.FileSource) (image.Config, string, error) {
	...
	if err != nil {
		return image.Config{}, "", fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_get_base64_data"), err)
	}
	decodedData, err := base64.StdEncoding.DecodeString(base64Str)
	if err != nil {
		return image.Config{}, "", fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_decode_base64_for_image_config"), err)
	}
...
func GetBase64Data(c *gin.Context, source *types.FileSource, reason ...string) (string, string, error) {
	...
	base64Str, err := cachedData.GetBase64Data()
	if err != nil {
		return "", "", fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_get_base64_data"), err)
	}
...
func decodeImageConfig(data []byte) (image.Config, string, error) {
	...
	return image.Config{}, "", errors.New(i18n.Translate(i18n.DefaultLang, "svc.failed_to_decode_image_config_unsupported_format"))
}
