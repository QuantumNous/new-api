...
// shardedRoundTripper fans requests for each origin across independent
// transports so each origin can keep the requested number of HTTP/2 connections.
...
func closeIdleConnections(roundTripper http.RoundTripper) {
	...
	if closer, ok := roundTripper.(idleCloser); ok {
		closer.CloseIdleConnections()
	}
}

// applyHTTP1Force disables automatic HTTP/2 on a never-used transport.
// ForceAttemptHTTP2=false alone is insufficient; an empty TLSNextProto map
// prevents net/http from wiring HTTP/2.
func applyHTTP1Force(transport *http.Transport) {
	...
	transport.TLSNextProto = make(map[string]func(authority string, connection *tls.Conn) http.RoundTripper)
