...
func checkRedisLimit(userId int, notifyType string) (bool, error) {
	key := fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.notify_limit"), userId, notifyType, time.Now().Format("2006010215"))

	// Get current count
	count, err := common.RedisGet(key)
	if err != nil && err.Error() != "redis: nil" {
		return false, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_get_notification_count"), err)
	}
	...
	if err != nil {
		return false, fmt.Errorf(i18n.Translate(i18n.DefaultLang, "svc.failed_to_increment_notification_count"), err)
	}
