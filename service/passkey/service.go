...
func BuildWebAuthn(r *http.Request) (*webauthn.WebAuthn, error) {
	...
	if settings == nil {
		return nil, errors.New(i18n.Translate(i18n.DefaultLang, "passkey.setting_not_found"))
	}
	...
	if originsStr != "" {
		...
		for _, origin := range originList {
			...
			if !settings.AllowInsecureOrigin && strings.HasPrefix(strings.ToLower(trimmed), "http://") {
				return nil, fmt.Errorf("%s", i18n.Translate(i18n.DefaultLang, "passkey.insecure_origin", map[string]any{"Origin": trimmed}))
			}
...
autoDetect:
	...
	if scheme == "http" && !settings.AllowInsecureOrigin && r.Host != "localhost" && r.Host != "127.0.0.1" && !strings.HasPrefix(r.Host, "127.0.0.1:") && !strings.HasPrefix(r.Host, "localhost:") {
		return nil, fmt.Errorf("%s", i18n.Translate(i18n.DefaultLang, "passkey.insecure_origin", map[string]any{"Origin": fmt.Sprintf("%s://%s", scheme, r.Host)}))
	}
	// 优先使用请求的完整Host（包含端口）
	host := r.Host
	...
	if host == "" {
		return nil, fmt.Errorf("%s", i18n.Translate(i18n.DefaultLang, "passkey.origin_undetermined", map[string]any{"Host": r.Host, "ServerAddress": system_setting.ServerAddress}))
	}
	if scheme == "" {
		scheme = "https"
	}
...
func resolveRPID(r *http.Request, settings *system_setting.PasskeySettings, origins []string) (string, error) {
	...
	if len(origins) == 0 {
		return "", errors.New(i18n.Translate(i18n.DefaultLang, "passkey.origin_not_configured"))
	}
	parsed, err := url.Parse(origins[0])
	if err != nil {
		return "", fmt.Errorf("%s", i18n.Translate(i18n.DefaultLang, "passkey.origin_parse_failed", map[string]any{"Error": err.Error()}))
	}
