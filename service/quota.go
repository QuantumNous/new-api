...
func (info QuotaInfo) effectiveBillingUSDToCNYRate() float64 {
	if info.BillingUSDToCNYRate <= 0 || math.IsNaN(info.BillingUSDToCNYRate) || math.IsInf(info.BillingUSDToCNYRate, 0) {
		return 1
	}
	return info.BillingUSDToCNYRate
}
...
func calculateAudioQuota(info QuotaInfo) (int, *common.QuotaClamp) {
	if info.UsePrice {
		...
		quotaPerUnit := decimal.NewFromFloat(common.QuotaPerUnit).Mul(decimal.NewFromFloat(info.effectiveBillingUSDToCNYRate()))
	...
	ratio := groupRatio.Mul(modelRatio).Mul(decimal.NewFromFloat(info.effectiveBillingUSDToCNYRate()))
