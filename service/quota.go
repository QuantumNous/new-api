...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	if totalTokens == 0 {
		...
		quota = 0
		logContent += " (upstream timeout may have occurred)"
	...
	gopool.Go(func() {
		...
		if quotaTooLow {
			...
			if notifyType == dto.NotifyTypeBark {
				...
				content = "{{value}}, current remaining quota is {{value}}. To avoid interruption, please top up in time.<br/>Top-up link: <a href='{{value}}'>{{value}}</a>"
