...
func checkAndSendQuotaNotify(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int) {
	gopool.Go(func() {
		...
		if quotaTooLow {
			prompt := "Your balance is running low"
			...
			if notifyType == dto.NotifyTypeBark {
				...
				content = "{{value}} Remaining balance: {{value}}. Please top up soon."
				...
				content = "{{value}} Your remaining balance is {{value}}. To avoid service interruption, please top up your account.<br/>Top up: <a href='{{value}}'>{{value}}</a>"
...
func checkAndSendSubscriptionQuotaNotify(relayInfo *relaycommon.RelayInfo) {
	gopool.Go(func() {
		...
		prompt := "Your subscription balance is running low"
		topUpLink := fmt.Sprintf("%s/console/topup", system_setting.ServerAddress)
