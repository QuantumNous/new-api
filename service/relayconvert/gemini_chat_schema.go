...
func cleanGeminiFunctionParameters(params interface{}) interface{} {
...
func removeAdditionalPropertiesWithDepth(schema interface{}, depth int) interface{} {
	...
	case "object":
		...
		if properties, ok := value["properties"].(map[string]interface{}); ok {
			for key, nested := range properties {
				properties[key] = removeAdditionalPropertiesWithDepth(nested, depth+1)
			}
		}
	...
	case "array":
		if items, ok := value["items"].(map[string]interface{}); ok {
			value["items"] = removeAdditionalPropertiesWithDepth(items, depth+1)
		}
	}
...
func convertOpenAIToolChoiceToGeminiConfig(toolChoice any) *dto.ToolConfig {
	if toolChoice == nil {
		return nil
	}
