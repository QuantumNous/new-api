...
func UsageFromResponsesUsage(src *dto.Usage) *dto.Usage {
	...
	// Reasoning tokens must be resolved across both usage schemas: the Responses
	// API reports them under output_tokens_details, not completion_tokens_details,
	// so reading only the latter silently drops them for /v1/responses upstreams.
	if reasoningTokens := src.ResolveReasoningTokens(); reasoningTokens != 0 {
