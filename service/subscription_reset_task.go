...
func StartSubscriptionQuotaResetTask() {
	subscriptionResetOnce.Do(func() {
		...
		gopool.Go(func() {
			logger.LogInfo(context.Background(), fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.subscription_quota_reset_task_started_tick"), subscriptionResetTickInterval))
...
func runSubscriptionQuotaResetOnce() {
	...
	for {
		...
		if err != nil {
			logger.LogWarn(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.subscription_expire_task_failed"), err))
			return
		}
		if n == 0 {
			break
	...
	for {
		...
		if err != nil {
			logger.LogWarn(ctx, fmt.Sprintf(i18n.Translate(i18n.DefaultLang, "svc.subscription_quota_reset_task_failed"), err))
			return
		}
		if n == 0 {
			break
	...
	if common.DebugEnabled && (totalReset > 0 || totalExpired > 0) {
		logger.LogDebug(ctx, i18n.Translate(i18n.DefaultLang, "svc.subscription_maintenance_reset_count_expired_count"), totalReset, totalExpired)
	}
}
