package service

import (
	"fmt"
	"io"
	"strconv"
	"strings"

	"github.com/QuantumNous/new-api/constant"
	"github.com/QuantumNous/new-api/model"
	"github.com/QuantumNous/new-api/relay/channel/task/taskcommon"
)

// NeedsUpstreamVideoURLRefresh reports whether a completed task still lacks a usable upstream media URL
// (empty, self-referencing proxy URL, or likely-expired signed URL).
func NeedsUpstreamVideoURLRefresh(task *model.Task) bool {
	if task == nil {
		return false
	}
	stored := strings.TrimSpace(task.GetResultURL())
	if stored == "" || taskcommon.IsTaskProxyContentURL(stored, task.TaskID) {
		return true
	}
	resolved := taskcommon.ResolveTaskVideoURL(task)
	if resolved == "" || taskcommon.IsTaskProxyContentURL(resolved, task.TaskID) {
		return true
	}
	return taskcommon.IsLikelyExpiredSignedVideoURL(resolved)
}

// RefreshTaskVideoURL re-fetches the upstream task and extracts a direct media URL.
func RefreshTaskVideoURL(channel *model.Channel, task *model.Task) (string, []byte, error) {
	if channel == nil || task == nil {
		return "", nil, fmt.Errorf("invalid channel or task")
	}
	if GetTaskAdaptorFunc == nil {
		return "", nil, fmt.Errorf("task adaptor factory is not configured")
	}

	adaptor := GetTaskAdaptorFunc(constant.TaskPlatform(strconv.Itoa(channel.Type)))
	if adaptor == nil {
		return "", nil, fmt.Errorf("task adaptor not found for channel type %d", channel.Type)
	}

	baseURL := channel.GetBaseURL()
	if baseURL == "" {
		baseURL = constant.GetChannelDefaultBaseURL(channel.Type)
	}
	if baseURL == "" {
		return "", nil, fmt.Errorf("channel base URL is empty")
	}

	upstreamTaskID := task.GetUpstreamTaskID()
	if alt := taskcommon.ExtractUpstreamTaskIDFromJSON(task.Data, task.TaskID); alt != "" {
		upstreamTaskID = alt
	}

	key := channel.Key
	if task.PrivateData.Key != "" {
		key = task.PrivateData.Key
	}

	proxy := channel.GetSetting().Proxy
	resp, err := adaptor.FetchTask(baseURL, key, map[string]any{
		"task_id": upstreamTaskID,
		"action":  task.Action,
		"model":   task.Properties.UpstreamModelName,
	}, proxy)
	if err != nil {
		return "", nil, err
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return "", nil, err
	}

	taskInfo, err := adaptor.ParseTaskResult(body)
	if err != nil {
		return "", body, err
	}
	if taskInfo == nil {
		return "", body, fmt.Errorf("empty task result")
	}

	videoURL := strings.TrimSpace(taskInfo.Url)
	if videoURL == "" || taskcommon.IsTaskProxyContentURL(videoURL, task.TaskID) {
		videoURL = taskcommon.ExtractVideoURLFromJSON(body)
	}
	if videoURL == "" {
		return "", body, fmt.Errorf("video url not found in upstream response")
	}
	return videoURL, body, nil
}

// PersistRefreshedTaskVideo writes a recovered upstream media URL back onto the task.
func PersistRefreshedTaskVideo(task *model.Task, videoURL string, responseBody []byte) {
	if task == nil || strings.TrimSpace(videoURL) == "" {
		return
	}
	snap := task.Snapshot()
	task.PrivateData.ResultURL = videoURL
	if len(responseBody) > 0 {
		task.Data = responseBody
	}
	if snap.Equal(task.Snapshot()) {
		return
	}
	_, _ = task.UpdateWithStatus(snap.Status)
}

// RepairTaskResultURLIfNeeded re-fetches upstream when a SUCCESS task only has a local proxy URL
// (or no URL). Recovers from "succeeded but empty video_url" races for historical tasks.
func RepairTaskResultURLIfNeeded(task *model.Task) {
	if task == nil || task.Status != model.TaskStatusSuccess {
		return
	}
	if !NeedsUpstreamVideoURLRefresh(task) {
		return
	}
	channel, err := model.CacheGetChannel(task.ChannelId)
	if err != nil || channel == nil {
		return
	}
	videoURL, responseBody, err := RefreshTaskVideoURL(channel, task)
	if err != nil || strings.TrimSpace(videoURL) == "" {
		return
	}
	if taskcommon.IsTaskProxyContentURL(videoURL, task.TaskID) {
		return
	}
	PersistRefreshedTaskVideo(task, videoURL, responseBody)
}
