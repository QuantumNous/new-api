...
func TestConsumeLogPromptTokensPreservesOpenRouterInputTotal(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "5m only without aggregate", cache5m: 10, wantQuota: 973},
		{name: "1h only without aggregate", cache1h: 20, wantQuota: 1000},
		{name: "split writes without aggregate", cache5m: 10, cache1h: 20, wantQuota: 1013},
		{name: "aggregate smaller than split writes", cacheWrite: 10, cache5m: 10, cache1h: 20, wantQuota: 1003},
		{name: "aggregate equals split writes", cacheWrite: 30, cache5m: 10, cache1h: 20, wantQuota: 983},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// QuotaFromDecimalChecked rounds half away from zero, including .5 totals.
