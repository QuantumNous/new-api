...
func setupUpstreamStatusTestDB(t *testing.T) {
	...
	model.DB = db
	require.NoError(t, db.AutoMigrate(&model.SupplierStatusSync{}))
...
func TestBuildPublicUpstreamStatusGroupsRecentHistoryByGroupAndModel(t *testing.T) {
	setupUpstreamStatusTestDB(t)
	now := time.Now().Unix()
