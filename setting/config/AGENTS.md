...
- DB 反序列化：从数据库键值对还原到结构体字段（支持 string/bool/int/float/slice/map/struct 类型）

## Key Files

| File | Description |
|------|-------------|
| `config.go` | `ConfigManager` 实现，`GlobalConfig` 单例，`LoadFromDB`/`SaveToDB`/`ExportAllConfigs` 方法 |
...
- 复杂类型（slice、map、struct）使用 JSON 序列化为字符串存储；map 字段在反序列化时会先分配新 map 再填充（避免遗留旧 key）。
