...
type Config struct {
	...
	MaxEndpointsPerModel  int                  `json:"max_endpoints_per_model"`
	NonStreamBudget       time.Duration        `json:"non_stream_budget"`
	StreamFirstByteBudget time.Duration        `json:"stream_first_byte_budget"`
	MaxCostMultiplier     float64              `json:"max_cost_multiplier"`
	QualityThresholds     map[TaskType]float64 `json:"quality_thresholds"`
...
func Normalize(input Config) (Config, error) {
	...
	if input.MaxEndpointsPerModel == 0 {
		input.MaxEndpointsPerModel = 2
	}
	if input.NonStreamBudget == 0 {
		input.NonStreamBudget = 30 * time.Second
	}
	if input.StreamFirstByteBudget == 0 {
		input.StreamFirstByteBudget = 12 * time.Second
	}
	...
	for task, value := range input.QualityThresholds {
		if value < 0 || value > 1 {
			return Config{}, fmt.Errorf("quality threshold for %s must be between 0 and 1", task)
		}
		defaults[task] = value
	}
	...
	for _, policy := range input.Models {
		...
		if _, ok := seen[policy.Model]; ok {
			return Config{}, fmt.Errorf("duplicate model policy %q", policy.Model)
		}
...
func clone(input Config) Config {
	input.Models = append([]ModelPolicy(nil), input.Models...)
	for i := range input.Models {
		input.Models[i].Capabilities = append([]string(nil), input.Models[i].Capabilities...)
	}
	thresholds := input.QualityThresholds
	input.QualityThresholds = make(map[TaskType]float64, len(thresholds))
	for task, value := range thresholds {
		input.QualityThresholds[task] = value
	}
	return input
}
