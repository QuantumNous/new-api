...
func ParseHTTPStatusCodeRanges(input string) ([]StatusCodeRange, error) {
	...
	if len(invalid) > 0 {
		return nil, fmt.Errorf(common.Translate(common.DefaultLang, "setting.invalid_http_status_code_rules"), strings.Join(invalid, ", "))
	}
	if len(ranges) == 0 {
		return nil, nil
	}
...
func parseHTTPStatusCodeToken(token string) (StatusCodeRange, error) {
	...
	if token == "" {
		return StatusCodeRange{}, errors.New(common.Translate(common.DefaultLang, "setting.empty_token"))
	}

	if strings.Contains(token, "-") {
		parts := strings.Split(token, "-")
		if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
			return StatusCodeRange{}, fmt.Errorf(common.Translate(common.DefaultLang, "setting.invalid_range_token"), token)
		}
		start, err := strconv.Atoi(parts[0])
		if err != nil {
			return StatusCodeRange{}, fmt.Errorf(common.Translate(common.DefaultLang, "setting.invalid_range_start"), token)
		}
		end, err := strconv.Atoi(parts[1])
		if err != nil {
			return StatusCodeRange{}, fmt.Errorf(common.Translate(common.DefaultLang, "setting.invalid_range_end"), token)
		}
		if start > end {
			return StatusCodeRange{}, fmt.Errorf(common.Translate(common.DefaultLang, "setting.range_start_end"), token)
		}
		if start < 100 || end > 599 {
			return StatusCodeRange{}, fmt.Errorf(common.Translate(common.DefaultLang, "setting.range_out_of_bounds"), token)
		}
	...
	if err != nil {
		return StatusCodeRange{}, fmt.Errorf(common.Translate(common.DefaultLang, "setting.invalid_status_code"), token)
	}
	if code < 100 || code > 599 {
		return StatusCodeRange{}, fmt.Errorf(common.Translate(common.DefaultLang, "setting.status_code_out_of_bounds"), token)
	}
