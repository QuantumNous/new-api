...
- **gpt-image-2 图像出图定价**：`gpt-image-2` 同时出现在 `defaultModelRatio`（`2.5`，token 计费方案 b）、`defaultCompletionRatio`（`6`，output 倍率）和 `defaultImageRatio`（`1.6`，图片输入倍率）三张表中。这是 codex 订阅出图的计费安排，运营可按需通过管理接口覆盖对应比率。
