...
import { fetchChannelModels, fetchChannelModelsWithMeta } from "@/services/api/image";
import { syncAppDataToWebdav, type AppSyncDomainKey, type AppSyncProgressEvent } from "@/services/app-sync";
import { testWebdavConnection, WEBDAV_MANIFEST_FILE_NAME } from "@/services/webdav-sync";
import { audioFormatOptions, audioVoiceOptions, normalizeAudioSpeedValue } from "@/lib/audio-generation";
import { BUILTIN_MODE, createModelChannel, defaultBaseUrlForApiFormat, filterModelsByCapability, modelOptionLabel, modelOptionsFromChannels, normalizeModelOptionValue, useConfigStore, type AiConfig, type ApiCallFormat, type ModelCapability, type ModelChannel } from "@/stores/use-config-store";
...
export function AppConfigModal() {
    ...
    const finishConfig = () => {
        const ready = config.channels.some((channel) => channel.baseUrl.trim() && (BUILTIN_MODE || channel.apiKey.trim()) && channel.models.length);
    ...
    const channelRunnable = (channel: ModelChannel) => channel.baseUrl.trim() && (BUILTIN_MODE || channel.apiKey.trim());

    const refreshChannelModels = async (channel: ModelChannel) => {
        if (!channelRunnable(channel)) {
            message.error("请先填写该渠道的 Base URL 和 API Key");
            return;
        }
        setLoadingChannelId(channel.id);
        try {
            const { models, endpointTypes } = await fetchChannelModelsWithMeta(channel);
            updateConfig("modelEndpointTypes", { ...config.modelEndpointTypes, ...endpointTypes });
    ...
    const refreshAllModels = async () => {
        const runnable = config.channels.filter(channelRunnable);
        if (!runnable.length) {
        ...
        try {
            const entries = await Promise.all(runnable.map(async (channel) => [channel.id, await fetchChannelModelsWithMeta(channel)] as const));
            const modelMap = new Map(entries);
            const mergedEndpointTypes = entries.reduce((acc, [, meta]) => ({ ...acc, ...meta.endpointTypes }), { ...config.modelEndpointTypes });
            updateConfig("modelEndpointTypes", mergedEndpointTypes);
            updateChannels(config.channels.map((channel) => (modelMap.has(channel.id) ? { ...channel, models: modelMap.get(channel.id)?.models || [] } : channel)));
                                ...
                                <div className="space-y-3">
                                    {config.channels.map((channel) => (
                                        <section key={channel.id} className="rounded-lg border border-stone-200 p-3 dark:border-stone-800">
                                            ...
                                            {BUILTIN_MODE ? (
                                                <div className="text-xs leading-5 text-stone-500">站内渠道由平台统一配置,使用你账号可用的模型并按平台计费;点击「拉取模型」获取模型列表。</div>
                                            ) : (
