...
import { builtinHeaders, handleBuiltinAuthError } from "@/lib/builtin-auth";

import { buildApiUrl, isBuiltinRequestConfig, resolveModelRequestConfig, type AiConfig, type ModelChannel } from "@/stores/use-config-store";
...
function aiHeaders(config: AiConfig, contentType?: string) {
    // BUILTIN_MODE: 站内渠道无 apiKey,改用 session + New-Api-User 头
    const auth = isBuiltinRequestConfig(config) ? builtinHeaders() : { Authorization: `Bearer ${config.apiKey}` };
    return {
        ...auth,
...
async function requestStreamingResponse(config: AiConfig, body: Record<string, unknown>, onDelta?: (text: string) => void, options?: RequestOptions): Promise<ToolResponseResult> {
    ...
    if (!response.ok) {
        handleBuiltinAuthError(response.status, isBuiltinRequestConfig(config));
        throw new Error(await readFetchError(response, "请求失败"));
    }
    if (!response.body) {
...
export async function fetchImageModels(config: Pick<AiConfig, "baseUrl" | "apiKey" | "apiFormat">) {
    try {
        ...
        const response = await axios.get<{ data?: Array<{ id?: string; supported_endpoint_types?: string[] }>; error?: { message?: string } }>(buildApiUrl(config.baseUrl, "/models"), {
            headers: isBuiltinRequestConfig(config as Pick<AiConfig, "apiKey">) ? builtinHeaders() : { Authorization: `Bearer ${config.apiKey}` },
        });
...
export type ChannelModelsWithMeta = { models: string[]; endpointTypes: Record<string, string[]> };

// BUILTIN_MODE: 除模型列表外,同时返回 new-api /pg/models 的 supported_endpoint_types 能力元数据
export async function fetchChannelModelsWithMeta(channel: ModelChannel): Promise<ChannelModelsWithMeta> {
    const config = { baseUrl: channel.baseUrl, apiKey: channel.apiKey, apiFormat: channel.apiFormat };
    if (config.apiFormat === "gemini") {
        return { models: await fetchImageModels(config), endpointTypes: {} };
    }
    try {
        const response = await axios.get<{ data?: Array<{ id?: string; supported_endpoint_types?: string[] }>; error?: { message?: string } }>(buildApiUrl(config.baseUrl, "/models"), {
            headers: isBuiltinRequestConfig(config) ? builtinHeaders() : { Authorization: `Bearer ${config.apiKey}` },
        });
        const entries = (response.data.data || []).filter((model): model is { id: string; supported_endpoint_types?: string[] } => Boolean(model.id));
        const endpointTypes: Record<string, string[]> = {};
        for (const model of entries) {
            if (Array.isArray(model.supported_endpoint_types) && model.supported_endpoint_types.length) {
                endpointTypes[model.id] = model.supported_endpoint_types;
            }
        }
        return { models: entries.map((model) => model.id).sort((a, b) => a.localeCompare(b)), endpointTypes };
    } catch (error) {
        throw new Error(readAxiosError(error, "读取模型失败"));
    }
}
