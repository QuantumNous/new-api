...
import { formatDisplayedTokenPrice } from '../../../../../helpers';
import { parseTiersFromExpr } from '../../../../../helpers/utils';
...
const VAR_LABELS = { p: '输入', c: '输出', len: '长度' };
const OP_LABELS = { '<': '<', '<=': '≤', '>': '>', '>=': '≥' };
...
export default function DynamicPricingBreakdown({
  ...
  const tierColumns = [
    ...
    ...priceFields
      ...
      .map(([field, label]) => ({
        title: t(PRICE_LABEL_OVERRIDES[field] || label),
        dataIndex: field,
        render: (v) =>
          v > 0 ? (
            <Text strong>
    ...
    <div>
      ...
      {hasTiers && (
        <div style={{ marginBottom: 16 }}>
          ...
          <div className='text-xs text-[var(--semi-color-text-2)] mb-2'>
