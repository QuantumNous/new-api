...
import GenerationDebugEntry from '../../components/table/usage-logs/generation-debug/GenerationDebugEntry';
...
export const useLogsData = () => {
  ...
  const [showGenerationDebugModal, setShowGenerationDebugModal] =
    useState(false);
  const [generationDebugTarget, setGenerationDebugTarget] = useState(null);
  ...
  const openGenerationDebugModal = (log, other) => {
    setGenerationDebugTarget({ log, other });
    setShowGenerationDebugModal(true);
  };

  const closeGenerationDebugModal = () => {
    setShowGenerationDebugModal(false);
    setGenerationDebugTarget(null);
  };
  ...
  const setLogsFormat = (logs) => {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      if (other?.generation_debug) {
        expandDataLocal.push({
          key: t('Generation Debug'),
          value: (
            <GenerationDebugEntry
              generationDebug={other.generation_debug}
              t={t}
              onOpen={(event) => {
                event.stopPropagation();
                openGenerationDebugModal(logs[i], other);
              }}
            />
          ),
        });
