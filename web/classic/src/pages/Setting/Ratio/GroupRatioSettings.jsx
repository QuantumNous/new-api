...
export default function GroupRatioSettings(props) {
  ...
  const [visibilityDigest, setVisibilityDigest] = useState('');
  const [visibilityMode, setVisibilityMode] = useState('legacy');
  const [visibilityEnabled, setVisibilityEnabled] = useState(false);
  const [visibilityReadSource, setVisibilityReadSource] = useState('');
  const [visibilityDegraded, setVisibilityDegraded] = useState(false);
  ...
  const loadVisibilityPolicies = useCallback(async () => {
    ...
    if (res.data.data) {
      ...
      if (!res.data.data.degraded) {
        setVisibilityPolicies(res.data.data.policies || []);
      }
      setVisibilityDigest(res.data.data.digest || '');
      setVisibilityMode(res.data.data.mode || 'legacy');
      setVisibilityReadSource(res.data.data.read_source || 'database');
      setVisibilityDegraded(Boolean(res.data.data.degraded));
    }
    if (!res.data.success) {
      throw new Error(res.data.message || t('读取令牌分组可见性策略失败'));
    }
  }, [t]);
  ...
  const saveVisibilityPolicies = async () => {
    if (!visibilityEnabled) {
      showWarning(
        t('请先完成 schema-first 迁移并启用 TOKEN_GROUP_VISIBILITY_ENABLED'),
      );
      return;
    }
    if (visibilityDegraded) {
      showError(t('读取状态已降级，修复配置后才能保存策略'));
      return;
    }
    try {
      const res = await API.put('/api/group/token-visibility/batch', {
        policies: visibilityPolicies,
        expected_digest: visibilityDigest,
        allow_empty: visibilityPolicies.length === 0,
      });
      if (!res.data.success) {
  ...
  const renderVisualMode = () => (
    <Form key='form-visual' values={inputs} style={{ marginBottom: 15 }}>
      ...
      <Form.Section text={t('令牌分组可见性')}>
        ...
        {visibilityMode !== 'enforce' && visibilityEnabled && (
          <Text
            type='warning'
            size='small'
            style={{ display: 'block', marginBottom: 12 }}
          >
            {t('当前运行模式为 {{mode}}，策略仅观察不执行。', {
              mode: visibilityMode,
            })}
          </Text>
        )}
        <Text
          type={visibilityDegraded ? 'warning' : 'tertiary'}
          size='small'
          style={{ display: 'block', marginBottom: 12 }}
        >
          {t('读取来源')}: {visibilityReadSource || 'database'} ·{' '}
          {t('策略摘要')}: {visibilityDigest || 'empty'}
          {visibilityDegraded
            ? ` · ${t('读取降级，当前策略不可用于授权')}`
            : ''}
        </Text>
        <div style={{ display: 'grid', gap: 8 }}>
          {visibilityPolicies.map((policy, index) => (
            <Row key={`${policy.group}-${index}`} gutter={8} align='center'>
              <Col span={5}>
                <Form.Input
                  value={policy.group}
                  placeholder={t('分组')}
                  ...
                  disabled={!visibilityEnabled || loading || visibilityDegraded}
                />
              </Col>
              <Col span={4}>
                <Form.Select
                  ...
                  onChange={(value) =>
                    setVisibilityPolicies((items) =>
                      items.map((item, i) =>
                        i === index ? { ...item, visibility: value } : item,
                      ),
                    )
                  }
                  optionList={[
                    { value: 'targeted', label: t('定向') },
                ...
                <Form.Input
                  ...
                  onChange={(value) =>
                    setVisibilityPolicies((items) =>
                      items.map((item, i) =>
                        i === index
                          ? { ...item, start_time: Number(value) || 0 }
                          : item,
                      ),
                    )
                  }
        ...
        <Button
          style={{ marginTop: 12 }}
          onClick={saveVisibilityPolicies}
          disabled={!visibilityEnabled || loading || visibilityDegraded}
        >
          {t('保存令牌分组可见性策略')}
        </Button>
