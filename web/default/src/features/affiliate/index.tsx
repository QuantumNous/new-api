      ...
      <div className='space-y-5 p-6'>
        <div>
          <div className='inline-flex items-center gap-2.5 rounded-full bg-gradient-to-r from-violet-500 to-pink-500 px-5 py-2.5 shadow-lg shadow-pink-200/50 dark:shadow-none'>
            <span className='text-lg font-bold text-white'>{t('推广有礼')}</span>
            <span className='h-4 w-px bg-white/40' />
            <span className='text-sm text-white/85'>
              {t('邀请好友，邀请者返佣')}: {affRatio}%
            </span>
          </div>
        </div>
        ...
        <div className={`${cardCls} rounded-3xl p-6 space-y-4`}>
          ...
          <div className='flex flex-col gap-3 border-t border-border/50 pt-4 sm:flex-row sm:items-center'>
            ...
            <div className='flex items-center gap-2'>
              <span className='text-muted-foreground shrink-0 text-sm'>{t('邀请链接')}</span>
              <Input
                value={affiliateLink}
                readOnly
        ...
        <div className='grid grid-cols-2 gap-4'>
          <div className={`${cardCls} p-5`}>
            <div className='mb-2 flex items-center gap-2'>
              ...
              <span className='text-muted-foreground text-xs'>{t('累计邀请人数')}</span>
            </div>
          ...
          <div className={`${cardCls} p-5`}>
            <div className='mb-2 flex items-center gap-2'>
              ...
              <span className='text-muted-foreground text-xs'>{t('累计返还余额')}</span>
            </div>
        ...
        <div className={`${cardCls} overflow-hidden`}>
          <div className='border-b border-border/50 px-5 py-3.5'>
            <h2 className='text-base font-semibold'>{t('邀请记录')}</h2>
          </div>
          <Tabs defaultValue='commission' className='p-4'>
            <TabsList variant='line' className='mb-2'>
              <TabsTrigger value='invite' className='data-active:text-orange-500 data-active:after:bg-orange-500'>{t('邀请记录')}</TabsTrigger>
              <TabsTrigger value='commission' className='data-active:text-orange-500 data-active:after:bg-orange-500'>{t('返利记录')}</TabsTrigger>
            </TabsList>

            <TabsContent value='invite' className='px-1 pb-1'>
              {logsLoading ? (
                <p className='text-muted-foreground py-6 text-center text-sm'>{t('加载中...')}</p>
              ) : inviteList.length === 0 ? (
                <p className='text-muted-foreground py-6 text-center text-sm'>{t('暂无邀请记录')}</p>
              ) : (
                <table className='w-full text-sm'>
                  <thead>
                    <tr className='text-muted-foreground border-b text-xs'>
                      <th className='py-2 text-left'>{t('用户')}</th>
                      <th className='py-2 text-right'>{t('注册时间')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {inviteList.map((u, i) => (
            ...
            <TabsContent value='commission' className='px-1 pb-1'>
              {logsLoading ? (
                ...
                <p className='text-muted-foreground py-6 text-center text-sm'>{t('暂无返利记录')}</p>
              ) : (
                <table className='w-full text-sm'>
                  <thead>
                    <tr className='text-muted-foreground border-b text-xs'>
                      <th className='py-2 text-left'>{t('付款金额')}</th>
                      <th className='py-2 text-right'>{t('返佣金额')}</th>
                      <th className='py-2 text-right'>{t('时间')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {affLogs.map((log) => (
