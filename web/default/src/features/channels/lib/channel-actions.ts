...
export async function handleTestChannel(
  ...
  try {
    ...
    if (response.success) {
      ...
      onTestComplete?.(true, response.time)
    } else {
      if (!options?.silent) {
...
/**
 * Batch set skip_auto_test on selected channels.
 */
export async function handleBatchSkipAutoTest(
  ids: number[],
  skip: boolean,
  queryClient?: QueryClient,
  onSuccess?: () => void
  ...
  try {
    const response = await batchSetChannelSkipAutoTest({ ids, skip })
    if (response.success) {
      toast.success(
        skip
          ? i18next.t('{{count}} channel(s) will skip auto test', {
              count: response.data || ids.length,
            })
          : i18next.t('{{count}} channel(s) will join auto test', {
              count: response.data || ids.length,
            })
      )
      queryClient?.invalidateQueries({ queryKey: channelsQueryKeys.lists() })
      ...
      toast.error(
        response.message || i18next.t('Failed to update skip auto test')
      )
    }
  } catch (_error) {
    toast.error(i18next.t('Failed to update skip auto test'))
  }
}
