...
export function ApiKeysMutateDrawer({
  ...
  // Simple ↔ Advanced mode toggle. Simple shows only name + expiry +
  // unlimited_quota (+ remain_quota_dollars). Advanced shows everything.
  // Initial value gets set in the open-effect below — start in Simple to
  // avoid a frame of "Advanced fields visible then hidden" on first paint.
  const [mode, setMode] = useState<CreateMode>('simple')
  const defaultUseAutoGroup = status?.default_use_auto_group === true
  // The user's "default" group from the perspective of detectAdvancedMode —
  // if backend defaults to auto-group routing, '' would never be the choice
  // so we compare against 'auto'; otherwise compare against the literal
  // DEFAULT_GROUP constant ('').
  const userDefaultGroup = defaultUseAutoGroup ? 'auto' : DEFAULT_GROUP
  ...
  // Load existing data when updating, and pick the initial Simple/Advanced
  // mode. For edit: smart-detect from the fetched token (any non-default
  // Advanced field = Advanced). For create: respect the user's last-used
    ...
    if (open && isUpdate && currentRow) {
      ...
      getApiKey(currentRow.id).then((result) => {
        if (result.success && result.data) {
          ...
          setMode(
            detectAdvancedMode(result.data, userDefaultGroup)
              ? 'advanced'
              : 'simple'
          )
        }
