...
export function formatPurchaseLimitPeriod(
  plan: Partial<SubscriptionPlan>,
  t: TFunction
): string {
  const unit = plan?.period_purchase_unit || 'month'
  const value = plan?.period_purchase_value || 1
  const unitLabels: Record<string, string> = {
    year: t('years'),
    month: t('months'),
    day: t('days'),
  ...
  if (unit === 'custom') {
    const seconds = Number(plan?.period_purchase_custom_seconds || 0)
    if (seconds >= 86400) return `${Math.floor(seconds / 86400)} ${t('days')}`
    if (seconds >= 3600) return `${Math.floor(seconds / 3600)} ${t('hours')}`
    if (seconds >= 60) return `${Math.floor(seconds / 60)} ${t('minutes')}`
    return `${seconds} ${t('seconds')}`
