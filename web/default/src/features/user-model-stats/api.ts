...
export async function fetchStats(params: FetchStatsParams): Promise<StatsResponse> {
  const { viewType, startTimestamp, endTimestamp, username, modelName, userGroup, page = 1, pageSize = 20 } = params
...
export async function exportStats(
  ...
  const res = await api.get(`/api/data/export?${search.toString()}`, { responseType: 'blob' })
