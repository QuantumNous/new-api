  ...
  persist(
      ...
      setConfig: (newConfig) =>
        set((state) => ({
          config: {
            ...state.config,
            ...newConfig,
            logo: normalizeBrandLogoUrl(
              newConfig.logo ?? state.config.logo
            ),
            currency: {
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        const logo = normalizeBrandLogoUrl(merged.config?.logo)
        const loadedLogoUrl = normalizeBrandLogoUrl(merged.loadedLogoUrl) || logo
        return {
          ...merged,
          config: { ...merged.config, logo },
          loadedLogoUrl,
        }
      },
    }
