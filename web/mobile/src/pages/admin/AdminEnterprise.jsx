...
const AdminEnterprise = () => {
  ...
  const handleApprove = (row) => {
    Dialog.confirm({
      content: `通过「${row.company_name}」的企业认证？`,
      onConfirm: async () => {
        try {
          const res = await API.put(
            `/api/user/enterprise/admin/${row.id}/approve`,
          );
          if (res.data.success) {
            showSuccess('已通过');
            reload();
  ...
  const handleReject = async (reason) => {
    try {
      const res = await API.put(
        `/api/user/enterprise/admin/${rejectId}/reject`,
        { reason },
      );
      if (res.data.success) {
        showSuccess('已驳回');
            ...
            return (
              <div className='m-card' key={row.id}>
                ...
                <div style={{ fontSize: 12.5, color: '#6b7280', marginTop: 6 }}>
                  {row.username} · 法人 {row.legal_rep_name} · 信用代码{' '}
                  {row.uscc_masked}
                </div>
        ...
        content={
          inspect && (
            <div style={{ fontSize: 14 }}>
              <p>企业名称：{inspect.reveal?.company_name}</p>
              <p>统一社会信用代码：{inspect.reveal?.uscc}</p>
              <p>法人：{inspect.reveal?.legal_rep_name}</p>
              <p>法人证件号：{inspect.reveal?.legal_rep_id}</p>
              {inspect.images?.license_image && (
                <Image src={inspect.images.license_image} width='100%' />
              )}
              {inspect.images?.legal_front_image && (
                <Image
                  src={inspect.images.legal_front_image}
                  width='100%'
                  style={{ marginTop: 8 }}
                />
              )}
              {inspect.images?.legal_back_image && (
                <Image
                  src={inspect.images.legal_back_image}
                  width='100%'
                  style={{ marginTop: 8 }}
                />
              )}
