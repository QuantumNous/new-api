    ...
    <div className='min-h-screen grid lg:grid-cols-2 overflow-hidden'>
      <div className='relative hidden lg:flex flex-col justify-center items-center p-12' style={{
        ...
        <div className='relative z-10 flex items-end justify-center flex-1'>
          <AnimatedCharacters isTyping={false} showPassword={!!newPassword} passwordLength={newPassword.length} />
        </div>
      </div>
              ...
              <Card className='border-0 !rounded-2xl overflow-hidden'>
                ...
                <div className='px-2 py-8'>
                  ...
                  <Form
                    ...
                    className='space-y-4'
                  >
                    <Form.Input field='email' label={t('邮箱')} name='email' disabled={true} prefix={<IconMail />} placeholder={email ? '' : t('等待获取邮箱信息...')} />
                    {newPassword && (
                      <Form.Input
                        field='newPassword'
                        label={t('新密码')}
                        ...
                        prefix={<IconLock />}
                        suffix={
                          <Button icon={<IconCopy />} type='tertiary' theme='borderless' onClick={async () => { await copy(newPassword); showNotice(`${t('密码已复制到剪贴板：')} ${newPassword}`); }}>
                            {t('复制')}
                          </Button>
                        }
                      />
                    ...
                    <div className='space-y-2 pt-2'>
                      <Button theme='solid' className='w-full !rounded-full' type='primary' htmlType='submit' onClick={handleSubmit} loading={loading} disabled={disableButton || newPassword || !isValidResetLink}>
                  ...
                  <div className='mt-6 text-center text-sm'>
                    <Text>
                      <Link to='/login' className='text-blue-600 hover:text-blue-800 font-medium'>{t('返回登录')}</Link>
                    </Text>
                  </div>
                </div>
              </Card>
