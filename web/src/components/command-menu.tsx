...
import { useVisibleRootNavGroups } from '@/hooks/use-sidebar-view'
...
export function CommandMenu() {
  ...
  // the current URL; otherwise fall back to the visible root navigation.
                ...
                {group.items.map((navItem) => {
                  if (navItem.url) {
                    return (
                      <CommandItem
                        ...
                        value={navItem.title}
                        onSelect={() => {
                          runCommand(() => openNavItem(navItem.url ?? ''))
                        }}
                      >
                  ...
                  return navItem.items?.map((subItem) => (
                    <CommandItem
                      ...
                      onSelect={() => {
                        runCommand(() => openNavItem(subItem.url ?? ''))
                      }}
                    >
                      <div className='flex size-4 items-center justify-center'>
                        <ArrowRight className='text-muted-foreground/80 size-2' />
