        ...
        {item.parsed?.choices?.[0] && (
          <div className='flex flex-wrap gap-2 text-xs'>
            {item.parsed.choices[0].delta?.content && (
              <Badge count={`Content: "${item.parsed.choices[0].delta.content.substring(0, 20)}..."`} type='primary' />
            )}
            {item.parsed.choices[0].delta?.reasoning_content && (
              <Badge count={t('有 Reasoning')} type='warning' />
            )}
            {item.parsed.choices[0].finish_reason && (
              <Badge count={`Finish: ${item.parsed.choices[0].finish_reason}`} type='success' />
            )}
            {item.parsed.usage && (
              <Badge 
                count={`Tokens: ${item.parsed.usage.prompt_tokens || 0}/${item.parsed.usage.completion_tokens || 0}`} 
                type='tertiary' 
              />
            )}
          </div>
    ...
    <div className='h-full flex flex-col bg-gray-50 dark:bg-gray-900/50 rounded-lg'>
      ...
      <div className='flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 flex-shrink-0'>
        <div className='flex items-center gap-3'>
          ...
          <Typography.Text strong>{t('SSE 数据流')}</Typography.Text>
                  ...
                  {item.isDone ? (
                    ...
                    <>
                      <span className='text-gray-600'>
                        {item.parsed?.id || item.parsed?.object || 'SSE Event'}
                      </span>
