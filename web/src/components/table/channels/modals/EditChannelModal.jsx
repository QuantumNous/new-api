...
const normalizeOptionalTimeoutSeconds = (value) => {
...
const getDefaultChannelTimeoutSettings = (timeoutDefaults = {}) => ({
  request_timeout_enabled: timeoutDefaults.request_timeout_enabled === true,
  request_timeout_seconds: timeoutDefaults.request_timeout_seconds,
  response_header_timeout_enabled:
    timeoutDefaults.response_header_timeout_enabled === true,
  response_header_timeout_seconds:
    timeoutDefaults.response_header_timeout_seconds,
  stream_idle_timeout_enabled:
    timeoutDefaults.stream_idle_timeout_enabled === true,
  stream_idle_timeout_seconds: timeoutDefaults.stream_idle_timeout_seconds,
  stream_response_header_timeout_enabled:
    timeoutDefaults.stream_response_header_timeout_enabled === true,
...
const EditChannelModal = (props) => {
  ...
  const loadChannel = async () => {
    ...
    if (success) {
      ...
      if (data.setting) {
        try {
          ...
          data.request_timeout_enabled =
            typeof parsedSettings.request_timeout_enabled === 'boolean'
              ? parsedSettings.request_timeout_enabled
              : defaultTimeoutSettings.request_timeout_enabled;
          data.request_timeout_seconds =
            typeof parsedSettings.request_timeout_seconds === 'number'
              ? parsedSettings.request_timeout_seconds
              : defaultTimeoutSettings.request_timeout_seconds;
          data.response_header_timeout_enabled =
            typeof parsedSettings.response_header_timeout_enabled === 'boolean'
              ? parsedSettings.response_header_timeout_enabled
              : defaultTimeoutSettings.response_header_timeout_enabled;
          data.response_header_timeout_seconds =
            typeof parsedSettings.response_header_timeout_seconds === 'number'
              ? parsedSettings.response_header_timeout_seconds
              : defaultTimeoutSettings.response_header_timeout_seconds;
          data.stream_idle_timeout_enabled =
            typeof parsedSettings.stream_idle_timeout_enabled === 'boolean'
              ? parsedSettings.stream_idle_timeout_enabled
              : defaultTimeoutSettings.stream_idle_timeout_enabled;
          data.stream_idle_timeout_seconds =
            typeof parsedSettings.stream_idle_timeout_seconds === 'number'
              ? parsedSettings.stream_idle_timeout_seconds
              : defaultTimeoutSettings.stream_idle_timeout_seconds;
          data.stream_response_header_timeout_enabled =
            typeof parsedSettings.stream_response_header_timeout_enabled ===
            'boolean'
              ? parsedSettings.stream_response_header_timeout_enabled
              : defaultTimeoutSettings.stream_response_header_timeout_enabled;
          data.stream_response_header_timeout_seconds =
            typeof parsedSettings.stream_response_header_timeout_seconds ===
            'number'
              ? parsedSettings.stream_response_header_timeout_seconds
              : defaultTimeoutSettings.stream_response_header_timeout_seconds;
        } catch (error) {
      ...
      setChannelSettings({
        ...
        response_header_timeout_enabled: data.response_header_timeout_enabled,
        response_header_timeout_seconds: data.response_header_timeout_seconds,
        stream_idle_timeout_enabled: data.stream_idle_timeout_enabled,
        stream_idle_timeout_seconds: data.stream_idle_timeout_seconds,
        stream_response_header_timeout_enabled:
          data.stream_response_header_timeout_enabled,
        stream_response_header_timeout_seconds:
          data.stream_response_header_timeout_seconds,
  ...
  const submit = async () => {
    ...
    delete localInputs.response_header_timeout_enabled;
    delete localInputs.response_header_timeout_seconds;
    delete localInputs.stream_idle_timeout_enabled;
    delete localInputs.stream_idle_timeout_seconds;
    delete localInputs.stream_response_header_timeout_enabled;
    delete localInputs.stream_response_header_timeout_seconds;
              ...
              <div className='space-y-4'>
                ...
                <div className='pt-3'>
                  ...
                  <Form.InputNumber
                    ...
                    extraText={t(
                      '用于按渠道折算成本价。最终成本价 = 原价 × 成本倍率，默认 1',
                    )}
                  />

                  <div className='pt-2'>
                    <Text strong>{t('非流式超时')}</Text>
                    <div className='text-xs text-gray-500 mt-1'>
                      {t(
                        '默认开启。默认值：请求超时 {{requestTimeout}} 秒、响应头超时 {{responseHeaderTimeout}} 秒；关闭后仅当前渠道禁用对应超时',
                        {
                          requestTimeout:
                            defaultTimeoutSettings.request_timeout_seconds,
                  ...
                  <Form.Switch
                    ...
                    extraText={t(
                      '控制非流式请求的总超时时间。超时发生在拿到完整响应前，会走现有重试逻辑',
                    )}
                  />
                  ...
                  <Form.InputNumber
                    ...
                    min={1}
                    precision={0}
                    disabled={channelSettings.request_timeout_enabled !== true}
                    onChange={(value) =>
                      handleChannelSettingsChange(
                        'request_timeout_seconds',
                        normalizeOptionalTimeoutSeconds(value),
                      )
                    }
                    extraText={t('仅在开启非流式请求超时后生效')}
                  />

                  <Form.Switch
                    field='response_header_timeout_enabled'
                    label={t('非流式响应头超时')}
                    checkedText={t('开')}
                    uncheckedText={t('关')}
                    onChange={(value) =>
                    ...
                    extraText={t(
                      '控制等待上游响应头的时间。超时发生在首包前，会走现有重试逻辑',
                    )}
                  />
                  ...
                  <Form.InputNumber
                    ...
                    precision={0}
                    disabled={
                      channelSettings.response_header_timeout_enabled !== true
                    }
                    onChange={(value) =>
                      handleChannelSettingsChange(
                        'response_header_timeout_seconds',
                    ...
                    extraText={t('仅在开启非流式响应头超时后生效')}
                  />

                  <div className='pt-2'>
                    <Text strong>{t('流式超时')}</Text>
                    <div className='text-xs text-gray-500 mt-1'>
                      {t(
                        '默认开启。默认值：响应头超时 {{responseHeaderTimeout}} 秒、空闲超时 {{idleTimeout}} 秒。流式响应头超时发生在首包前，可按现有逻辑重试；流式空闲超时发生在开始输出后，会直接结束当前流',
                        {
                          responseHeaderTimeout:
                            defaultTimeoutSettings.stream_response_header_timeout_seconds,
                          idleTimeout:
                  ...
                  <Form.Switch
                    field='stream_response_header_timeout_enabled'
                    label={t('流式响应头超时')}
                    checkedText={t('开')}
                    uncheckedText={t('关')}
                    onChange={(value) =>
                    ...
                    extraText={t('控制流式请求等待上游响应头的时间')}
                  />

                  <Form.InputNumber
                    field='stream_response_header_timeout_seconds'
                    label={t('流式响应头超时秒数')}
                    min={1}
                    precision={0}
                    disabled={
                    ...
                    extraText={t('仅在开启流式响应头超时后生效')}
                  />

                  <Form.Switch
                    field='stream_idle_timeout_enabled'
                    label={t('流式空闲超时')}
                    checkedText={t('开')}
                    uncheckedText={t('关')}
                    onChange={(value) =>
                    ...
                    extraText={t(
                      '控制流式输出过程中两次上游数据之间允许的最大空闲时间',
                    )}
                  />

                  <Form.InputNumber
                    field='stream_idle_timeout_seconds'
                    label={t('流式空闲超时秒数')}
                    min={1}
                    precision={0}
                    disabled={
                      channelSettings.stream_idle_timeout_enabled !== true
                    }
                    onChange={(value) =>
                      handleChannelSettingsChange(
                        'stream_idle_timeout_seconds',
