...
const BALANCE_QUERY_NEWAPI_TEMPLATE = {
  request: {
...
const EditChannelModal = (props) => {
  ...
  const loadBalanceQueryInstances = async () => {
    ...
    try {
      ...
      if (success) {
        setBalanceQueryInstances(Array.isArray(data) ? data : []);
      } else {
        showError(message);
      }
    } catch (error) {
      ...
      if (data.settings) {
        try {
          ...
          data.balance_query_interval_seconds =
            typeof balanceQuery.interval_seconds === 'number'
              ? balanceQuery.interval_seconds
              : 300;
          data.balance_query_source_channel_id =
            Number(balanceQuery.source_channel_id) || 0;
          ...
          data.balance_query_enabled = false;
          data.balance_query_template = 'newapi';
          data.balance_query_interval_seconds = 300;
          data.balance_query_source_channel_id = 0;
          data.balance_query_access_token = '';
          data.balance_query_user_id = '';
          data.balance_query_request_url =
            BALANCE_QUERY_NEWAPI_TEMPLATE.request.url;
          data.balance_query_request_method =
          ...
          data.balance_query_request_body = '';
          data.balance_query_plan_name_path =
            BALANCE_QUERY_NEWAPI_TEMPLATE.extractor.plan_name_path;
          data.balance_query_remaining_path =
            BALANCE_QUERY_NEWAPI_TEMPLATE.extractor.remaining_path;
  ...
  const submit = async () => {
    ...
    let balanceQueryHeaders = {};
    const hasBalanceQueryHeaders =
      typeof localInputs.balance_query_request_headers === 'string' &&
      localInputs.balance_query_request_headers.trim() !== '';
    if (hasBalanceQueryHeaders) {
      if (!verifyJSON(localInputs.balance_query_request_headers)) {
        showInfo(t('余额查询请求头必须是合法的 JSON 格式！'));
        return;
      }
      try {
        balanceQueryHeaders = JSON.parse(
          localInputs.balance_query_request_headers,
        );
      } catch (error) {
        showInfo(t('余额查询请求头必须是合法的 JSON 格式！'));
        return;
      ...
      if (
        !balanceQueryHeaders ||
        typeof balanceQueryHeaders !== 'object' ||
        Array.isArray(balanceQueryHeaders)
      ) {
        showInfo(t('余额查询请求头必须是 JSON 对象'));
        return;
      }
    }

    const balanceInterval = Number(localInputs.balance_query_interval_seconds);
    settings.balance_query = {
      ...(settings.balance_query || {}),
      enabled: localInputs.balance_query_enabled === true,
      template: localInputs.balance_query_template || 'custom',
      interval_seconds: Number.isFinite(balanceInterval)
        ? Math.max(0, Math.round(balanceInterval))
        : 300,
      source_channel_id:
        Number(localInputs.balance_query_source_channel_id) > 0
          ? Math.round(Number(localInputs.balance_query_source_channel_id))
          : 0,
      access_token: localInputs.balance_query_access_token || '',
      user_id: localInputs.balance_query_user_id || '',
      request: {
        url: localInputs.balance_query_request_url || '',
        method: localInputs.balance_query_request_method || 'GET',
        headers: balanceQueryHeaders,
        body: localInputs.balance_query_request_body || '',
      },
      extractor: {
        plan_name_path: localInputs.balance_query_plan_name_path || '',
      ...
      last_check_time: Number(settings.balance_query?.last_check_time) || 0,
    ...
    delete localInputs.balance_query_enabled;
    delete localInputs.balance_query_template;
    delete localInputs.balance_query_interval_seconds;
    delete localInputs.balance_query_source_channel_id;
    delete localInputs.balance_query_access_token;
    delete localInputs.balance_query_user_id;
    delete localInputs.balance_query_request_url;
    delete localInputs.balance_query_request_method;
    delete localInputs.balance_query_request_headers;
    delete localInputs.balance_query_request_body;
    delete localInputs.balance_query_plan_name_path;
  ...
  const balanceQueryInstanceOptions = useMemo(() => {
    ...
    balanceQueryInstances.forEach((item) => {
    ...
    const selectedId = Number(inputs.balance_query_source_channel_id) || 0;
    if (
      selectedId > 0 &&
              ...
              <div className='space-y-4'>
                ...
                <div className='py-3 border-b border-gray-100'>
                  ...
                  <Form.Switch
                    ...
                    extraText={t(
                      '启用后，手动更新余额和后台检测都会优先使用此配置',
                    )}
                  />
                  <Row gutter={12}>
                    <Col span={12}>
                      <Form.Select
                        field='balance_query_template'
                        label={t('模板')}
                        optionList={[
                    ...
                    <Col span={12}>
                      <Form.InputNumber
                        field='balance_query_interval_seconds'
                        label={t('检测间隔（秒）')}
                        min={0}
                        style={{ width: '100%' }}
                  ...
                  <Form.Select
                    field='balance_query_source_channel_id'
                    label={t('余额查询实例')}
                    placeholder={t('选择已存在的余额查询实例')}
                    optionList={balanceQueryInstanceOptions}
                    loading={balanceQueryInstancesLoading}
                    filter
                    onChange={(value) =>
                      handleBalanceQueryInputChange(
                    ...
                    extraText={t(
                      '选择已存在实例后，将复用该实例的余额查询配置和查询结果；后台检测同一实例只请求一次',
                    )}
                  />
                  <Form.Input
                    field='balance_query_access_token'
                    label='Access Token'
                    placeholder={t(
                  ...
                  />
                  <Form.Input
                    field='balance_query_request_url'
                    label={t('请求地址')}
                    placeholder='{{baseUrl}}/api/user/self'
                    showClear
                    onChange={(value) =>
                      handleBalanceQueryInputChange(
                        'balance_query_request_url',
                        value,
                      )
                    }
                    extraText={t(
                  ...
                  <Row gutter={12}>
                    <Col span={8}>
                      <Form.Select
                        field='balance_query_request_method'
                        label={t('请求方法')}
                        optionList={[
                          { label: 'GET', value: 'GET' },
                  ...
                  <Form.TextArea
                    field='balance_query_request_headers'
                    label={t('请求头 JSON')}
                    autosize
                    onChange={(value) =>
                      handleBalanceQueryInputChange(
                        'balance_query_request_headers',
                        value,
                      )
                    }
                  />
                  <Form.TextArea
                    field='balance_query_request_body'
                    label={t('请求体')}
                    autosize
                    showClear
                    onChange={(value) =>
                  ...
                  <Row gutter={12}>
                    <Col span={12}>
                      <Form.Input
                        field='balance_query_remaining_path'
                        label={t('剩余额度路径')}
                  ...
                  <div className='text-xs text-gray-500'>
                    {t('上次检测时间')}:&nbsp;
                    {formatUnixTime(inputs.balance_query_last_check_time)}
