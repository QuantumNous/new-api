...
const EditTagModal = (props) => {
  ...
  const handleSave = async (values) => {
    ...
    if (formVals.model_mapping) {
      if (!verifyJSON(formVals.model_mapping)) {
        showInfo(t('modals.tags.modelMappingError'));
        setLoading(false);
        return;
      }
    ...
    if (
      ...
      showWarning(t('modals.tags.noChangesWarning'));
      setLoading(false);
      return;
    }
    await submit(data);
  ...
  const submit = async (data) => {
    try {
      ...
      if (res?.data?.success) {
        showSuccess(t('modals.tags.tagUpdateSuccess'));
        refresh();
        handleClose();
      }
    } catch (error) {
            ...
            <div className='p-2'>
              <Card className='!rounded-2xl shadow-sm border-0 mb-6'>
                ...
                <div className='flex items-center mb-2'>
                  ...
                  <div>
                    <Text className='text-lg font-medium'>
                      {t('modals.tags.tagInfo')}
                    </Text>
                    <div className='text-xs text-gray-600'>
                      {t('modals.tags.tagBasicConfig')}
                    </div>
                  </div>
                </div>
                ...
                <div className='space-y-4'>
                  <Form.Input
                    ...
                    placeholder={t('modals.tags.newTagPlaceholder')}
              ...
              <Card className='!rounded-2xl shadow-sm border-0 mb-6'>
                ...
                <div className='flex items-center mb-2'>
                  ...
                  <div>
                    <Text className='text-lg font-medium'>
                      {t('modals.tags.modelConfig')}
                    </Text>
                    <div className='text-xs text-gray-600'>
                      {t('modals.tags.modelSelectionAndMapping')}
                    </div>
                  </div>
                </div>
                  ...
                  <Form.TextArea
                    ...
                    extraText={
                      <Space>
                        ...
                        >
                          {t('modals.tags.clearRedirect')}
                        </Text>
                        <Text
                          className='!text-semi-color-primary cursor-pointer'
                          onClick={() => handleInputChange('model_mapping', '')}
                        >
                          {t('modals.tags.noChange')}
                        </Text>
                      </Space>
                    }
                  />
              ...
              <Card className='!rounded-2xl shadow-sm border-0'>
                ...
                <div className='flex items-center mb-2'>
                  ...
                  <div>
                    <Text className='text-lg font-medium'>
                      {t('modals.tags.groupSettings')}
                    </Text>
                    <div className='text-xs text-gray-600'>
                      {t('modals.tags.userGroupConfig')}
                    </div>
                  </div>
                </div>
                ...
                <div className='space-y-4'>
                  <Form.Select
                    ...
                    multiple
                    allowAdditions
                    additionLabel={t('modals.tags.addGroupHint')}
