...
const MultiKeyManageModal = ({ visible, onCancel, channel, onRefresh }) => {
  ...
  const loadKeyStatus = async (
    ...
    try {
      ...
      console.error(error);
      showError(t('modals.multiKey.errors.fetchStatusFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDisableKey = async (keyIndex) => {
    ...
    try {
      ...
      if (res.data.success) {
        showSuccess(t('modals.multiKey.messages.keyDisabled'));
      ...
      showError(t('modals.multiKey.errors.disableFailed'));
    } finally {
  ...
  const handleEnableKey = async (keyIndex) => {
    ...
    try {
      ...
      if (res.data.success) {
        showSuccess(t('modals.multiKey.messages.keyEnabled'));
        await loadKeyStatus(currentPage, pageSize); // Reload current page
        onRefresh && onRefresh(); // Refresh parent component
      } else {
        showError(res.data.message);
      ...
      showError(t('modals.multiKey.errors.enableFailed'));
    } finally {
      setOperationLoading((prev) => ({ ...prev, [operationId]: false }));
    }
  };
  ...
  const handleEnableAll = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        showSuccess(res.data.message || t('modals.multiKey.messages.allEnabled'));
      ...
      showError(t('modals.multiKey.errors.enableAllFailed'));
    } finally {
  ...
  const handleDisableAll = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        showSuccess(res.data.message || t('modals.multiKey.messages.allDisabled'));
        // Reset to first page after bulk operation
        setCurrentPage(1);
        await loadKeyStatus(1, pageSize);
        onRefresh && onRefresh(); // Refresh parent component
      ...
      showError(t('modals.multiKey.errors.disableAllFailed'));
    } finally {
  ...
  const handleDeleteDisabledKeys = async () => {
    ...
    try {
      ...
      showError(t('modals.multiKey.errors.deleteDisabledFailed'));
    } finally {
  ...
  const handleDeleteKey = async (keyIndex) => {
    ...
    try {
      ...
      if (res.data.success) {
        showSuccess(t('modals.multiKey.messages.keyDeleted'));
        await loadKeyStatus(currentPage, pageSize); // Reload current page
        onRefresh && onRefresh(); // Refresh parent component
      } else {
        showError(res.data.message);
      ...
      showError(t('modals.multiKey.errors.deleteFailed'));
    } finally {
      setOperationLoading((prev) => ({ ...prev, [operationId]: false }));
    }
  };
      ...
      case 1:
        return (
          <Tag color='green' shape='circle' size='small'>
            {t('modals.multiKey.status.enabled')}
          </Tag>
        );
      case 2:
        return (
          <Tag color='red' shape='circle' size='small'>
            {t('modals.multiKey.status.disabled')}
          </Tag>
        );
      case 3:
        return (
          <Tag color='orange' shape='circle' size='small'>
            {t('modals.multiKey.status.autoDisabled')}
          </Tag>
        );
      default:
        return (
          <Tag color='grey' shape='circle' size='small'>
            {t('modals.multiKey.stats.unknown')}
          </Tag>
        );
    }
  };
  ...
  const columns = [
    ...
    {
      title: t('modals.multiKey.columns.reason'),
      dataIndex: 'reason',
    ...
    {
      title: t('modals.multiKey.columns.disabledTime'),
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            title={t('modals.multiKey.confirm.delete.title')}
            content={t('modals.multiKey.confirm.delete.content')}
    ...
    <Modal
      title={
        <Space>
          ...
          <Tag size='small' shape='circle' color='white'>
            {t('modals.multiKey.totalKeys', { count: total })}
          </Tag>
          {channel?.channel_info?.multi_key_mode && (
            <Tag size='small' shape='circle' color='white'>
              {channel.channel_info.multi_key_mode === 'random'
                ? t('modals.multiKey.mode.random')
                : t('modals.multiKey.mode.polling')}
            </Tag>
          )}
        </Space>
      }
            ...
            <Col span={8}>
              ...
              >
                <div className='flex items-center gap-2 mb-2'>
                  ...
                  <Text type='tertiary'>{t('modals.multiKey.stats.manualDisabled')}</Text>
                </div>
                <div className='flex items-end gap-2 mb-2'>
                  <Text
            ...
            <Col span={8}>
              ...
              >
                <div className='flex items-center gap-2 mb-2'>
                  ...
                  <Text type='tertiary'>{t('modals.multiKey.stats.autoDisabled')}</Text>
                </div>
                <div className='flex items-end gap-2 mb-2'>
                  <Text
              ...
              <Table
                      ...
                      <Row gutter={12} style={{ alignItems: 'center' }}>
                        <Col>
                          <Select
                            ...
                            size='small'
                            placeholder={t('modals.multiKey.filters.all')}
                          >
                            <Select.Option value={null}>
                              {t('modals.multiKey.filters.all')}
                            </Select.Option>
                    ...
                    >
                      <Space>
                        ...
                        {manualDisabledCount + autoDisabledCount > 0 && (
                          <Popconfirm
                            title={t('modals.multiKey.confirm.enableAll.title')}
                        ...
                        {enabledCount > 0 && (
                          <Popconfirm
                            title={t('modals.multiKey.confirm.disableAll.title')}
                        ...
                        <Popconfirm
                          title={t('modals.multiKey.confirm.deleteAutoDisabled.title')}
                          content={t('modals.multiKey.confirm.deleteAutoDisabled.content')}
                        ...
                        >
                          ...
                          >
                            {t('modals.multiKey.actions.deleteAutoDisabled')}
                          </Button>
                        </Popconfirm>
                      </Space>
                    </Col>
                ...
                empty={
                  <Empty
                    ...
                    title={t('modals.multiKey.empty.title')}
                    description={t('modals.multiKey.empty.description')}
                    style={{ padding: 30 }}
                  />
                }
              />
