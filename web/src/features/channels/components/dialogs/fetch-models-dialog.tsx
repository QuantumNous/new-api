...
export function FetchModelsDialog({
  ...
  const handleValidate = async () => {
    ...
    const models = [...new Set([...fetchedModels, ...selectedModels])].filter(
      Boolean
    )
    if (models.length === 0) {
    ...
    try {
      let response: ValidateModelsResponse
      if (customValidator) {
        response = await customValidator(models)
      } else if (activeChannel) {
        response = await validateModels({
