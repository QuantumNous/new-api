...
type FetchModelsDialogProps = {
  ...
  onModelsSelected?: (
    models: string[],
    discoveredModelMapping?: Record<string, string>
  ) => void
...
export function FetchModelsDialog({
  ...
  const [discoveredModelMapping, setDiscoveredModelMapping] = useState<
  ...
  const handleFetchModels = async () => {
    ...
    try {
      if (customFetcher) {
        ...
        setDiscoveredModelMapping(result.modelMapping || {})
        ...
        if (response.success) {
          ...
          setFetchedModels(list)
          setDiscoveredModelMapping(response.model_mapping || {})
          setSelectedModels(existingModels)
          toast.success(t('Fetched {{count}} models', { count: list.length }))
        } else {
          toast.error(response.message || t('Failed to fetch models'))
          setFetchedModels([])
          setDiscoveredModelMapping({})
        }
      }
    } catch (error: unknown) {
      toast.error(
  ...
  const handleSave = async () => {
    ...
    if (onModelsSelected) {
      onModelsSelected(selectedModels, discoveredModelMapping)
