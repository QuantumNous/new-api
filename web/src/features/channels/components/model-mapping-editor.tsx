      ...
      <Tabs value={mode} onValueChange={handleModeChange} className='space-y-2'>
        ...
        <TabsContent value='visual' className='space-y-2'>
          <p className='text-muted-foreground text-xs'>
            {t(
              'Requests use the left model name. This channel sends the right model name to its upstream provider.'
            )}
          </p>
          {rows.length > 0 ? (
            <div className='space-y-2'>
              <div className='grid grid-cols-[1fr_1fr_auto] gap-2 text-sm font-medium'>
                <div>{t('Downstream Request Model')}</div>
                <div>{t('Upstream Provider Model')}</div>
                <div className='w-10' />
              </div>
              {rows.map((row) => (
                <div
