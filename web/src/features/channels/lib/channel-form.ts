...
export const channelFormSchema = z
  .object({
    ...
    native_openai_batch: z.boolean().optional(), // OpenAI native File/Batch API
...
export function transformChannelToFormDefaults(
  ...
  if (channel.settings) {
    try {
      ...
      nativeOpenAIBatch = parsed.native_openai_batch === true
  ...
  return {
    ...
    native_openai_batch: nativeOpenAIBatch,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if (formData.type === 1) {
    settingsObj.native_openai_batch = formData.native_openai_batch === true
  } else if ('native_openai_batch' in settingsObj) {
    delete settingsObj.native_openai_batch
  }
