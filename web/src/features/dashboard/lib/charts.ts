...
function uniqueUserChartLabels(
  ...
  for (const username of usernames) {
    const displayName = displayNameByUser.get(username) || ''
    const label = displayName || username
    preferred.set(username, label)
    labelCounts.set(label, (labelCounts.get(label) || 0) + 1)
  }

  const labels = new Map<string, string>()
  for (const username of usernames) {
    const label = preferred.get(username) || username
    if ((labelCounts.get(label) || 0) > 1 && label !== username) {
      labels.set(username, `${label} (${username})`)
      continue
    }
    labels.set(username, label)
  }
  return labels
}
...
export function processUserChartData(
  ...
  const userColorMap = topUsers.reduce<Record<string, string>>(
    (acc, user, i) => {
      const label = labelByUser.get(user) || user
  ...
  sortedTimePoints.forEach((time) => {
    topUsers.forEach((user) => {
      ...
      trendValues.push({
        Time: time,
        User: labelByUser.get(user) || user,
        rawQuota: q,
