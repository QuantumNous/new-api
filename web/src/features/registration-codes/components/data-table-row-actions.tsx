...
export function DataTableRowActions<TData>({
  ...
  const registrationCode = registrationCodeSchema.parse(row.original)
  const { setOpen, setCurrentRow } = useRegistrationCodes()
  const isUnused = registrationCode.status === REGISTRATION_CODE_STATUS.UNUSED
  const isExpired = isRegistrationCodeExpired(
    registrationCode.expired_time,
    registrationCode.status
  )

  const canEdit = isUnused && !isExpired
        ...
        <TooltipTrigger
          render={
            <Button
              ...
              size='icon-sm'
              onClick={() => {
                setCurrentRow(registrationCode)
                setOpen('update')
              }}
              disabled={!canEdit}
              aria-label={t('Edit')}
