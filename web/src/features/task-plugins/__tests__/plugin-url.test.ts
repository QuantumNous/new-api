...
describe('plugin source URL normalization', () => {
  ...
  test('strips a line anchor that would break the raw request', () => {
    ...
    ).toBe('https://raw.githubusercontent.com/owner/repo/main/plugin.js')
  })

  test('rewrites a gist page URL to its raw URL', () => {
    expect(
      normalizePluginSourceUrl('https://gist.github.com/someone/abc123')
    ).toBe('https://gist.githubusercontent.com/someone/abc123/raw')
  })
  ...
  test('passes a raw URL through unchanged', () => {
    ...
    expect(normalizePluginSourceUrl(raw)).toBe(raw)
  })
  ...
  test('passes an arbitrary https URL through unchanged', () => {
    ...
    ).toBe('https://cdn.jsdelivr.net/gh/owner/repo/plugin.js')
  })

  test('leaves a GitHub repository URL alone when it names no file', () => {
    expect(normalizePluginSourceUrl('https://github.com/owner/repo')).toBe(
      'https://github.com/owner/repo'
    )
  })

  test('rejects a relative path with no origin to resolve against', () => {
    expect(normalizePluginSourceUrl('plugins/tasks/x/plugin.js')).toBe(null)
  })

  test('rejects a non-http scheme', () => {
    expect(normalizePluginSourceUrl('file:///etc/passwd')).toBe(null)
    expect(normalizePluginSourceUrl('javascript:alert(1)')).toBe(null)
  })

  test('rejects empty input', () => {
    expect(normalizePluginSourceUrl('   ')).toBe(null)
  })
})

describe('plugin source byte length', () => {
  test('counts UTF-8 bytes rather than UTF-16 code units', () => {
    expect(pluginSourceByteLength('abc')).toBe(3)
    expect(pluginSourceByteLength('中文')).toBe(6)
  })
})
...
describe('browser plugin source fetch', () => {
  ...
  test('reports the status when the host answers with an error', async () => {
    await expect(
      ...
      (error: unknown) =>
        error instanceof PluginSourceFetchError &&
  ...
  test('rejects a declared content-length above the 1 MiB backend limit before reading the body', async () => {
    ...
    await expect(
      ...
      (error: unknown) =>
        error instanceof PluginSourceFetchError && error.reason === 'too_large'
    )
  ...
  test('accepts a body exactly at the limit', async () => {
    ...
    expect(pluginSourceByteLength(text)).toBe(MAX_PLUGIN_SOURCE_BYTES)
  })
})
