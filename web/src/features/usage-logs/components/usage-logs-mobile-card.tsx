...
import { getUsageTokenBreakdown, parseLogOther } from '../lib/format'
import {
  getLogTypeConfig,
...
function MobileTokensField({ log }: { log: UsageLog }) {
  ...
  const showCache =
    tokenUsage.cacheReadTokens > 0 || tokenUsage.cacheWriteTokens > 0
        ...
        {showCache ? (
          <div className='text-muted-foreground flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11px] leading-none'>
            ...
            {tokenUsage.cacheWriteTokens > 0 && (
              <span>↑ {tokenUsage.cacheWriteTokens.toLocaleString()}</span>
            )}
          </div>
        ) : (
