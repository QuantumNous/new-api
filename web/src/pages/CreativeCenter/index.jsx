...
const DropButton = ({ icon, label, open, onClick, children }) => (
  <div className='relative'>
    <button
      onClick={onClick}
      className={`group flex items-center gap-2 rounded-[1.25rem] border px-4 py-2.5 text-[13px] font-bold transition-all duration-300 backdrop-blur-xl shadow-[0_4px_15px_-5px_rgba(0,0,0,0.05)] ${
        open 
          ? 'border-blue-300 bg-blue-50/90 text-blue-700 shadow-md scale-105' 
          : 'border-blue-100/60 bg-white/70 text-slate-600 hover:bg-white hover:border-blue-200 hover:text-blue-600 hover:shadow-md hover:scale-105'
      }`}
    >
      <span className={open ? 'text-blue-500' : 'text-slate-400 group-hover:text-blue-500 transition-colors'}>{icon}</span>
      {label}
      <ChevronDown size={14} className={`text-slate-400 transition-transform duration-300 ${open ? 'rotate-180 text-blue-400' : 'group-hover:text-blue-400'}`} />
    </button>
    {children}
  </div>
);
...
const DropSelectButton = ({
  ...
  >
    {openMenu === menuKey && (
      <div className={`absolute bottom-[130%] left-0 z-50 mb-2 ${widthClass} rounded-[1.5rem] border border-blue-100/60 bg-white/95 backdrop-blur-3xl p-2 shadow-[0_20px_60px_-10px_rgba(59,130,246,0.15)] overflow-hidden animate-in fade-in slide-in-from-bottom-3 duration-200`}>
        ...
        <div className='relative flex flex-col gap-0.5'>
          {options.map((option) => (
            <button
              ...
              className={`flex w-full items-center justify-between rounded-[1rem] px-3.5 py-2.5 text-[13px] font-bold transition-all duration-200 ${
                value === option.value
                  ? 'bg-blue-50 text-blue-600 shadow-sm border border-blue-100/50'
                  : 'text-slate-600 border border-transparent hover:bg-blue-50/50 hover:text-blue-600 hover:border-blue-100/30'
              }`}
            >
    ...
    <div className='relative flex h-[calc(100vh-64px)] min-h-[calc(100vh-64px)] mt-16 w-full bg-[#f0f5ff] bg-gradient-to-br from-[#eaf2ff] via-white to-[#f0f5ff] text-slate-800 font-sans selection:bg-blue-500/20 selection:text-blue-900 overflow-hidden'>
      ...
      <aside className='relative z-10 flex w-[320px] shrink-0 flex-col border-r border-slate-200/50 bg-white/70 backdrop-blur-3xl shadow-[4px_0_24px_-16px_rgba(0,0,0,0.05)]'>
        <div className='shrink-0 px-7 pt-8 pb-5'>
          <h1 className='text-[24px] font-black tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-sky-500 drop-shadow-sm'>
            释放你的创造力
          </h1>
        </div>
          ...
          {currentDisplayModels.map((model) => (
            ...
            >
              ...
              <div className='min-w-0 flex-1 pl-1'>
                ...
                {model.priceLabel ? (
                  <div className={`mt-2 inline-flex shrink-0 rounded-md px-2 py-0.5 text-[10px] font-black tracking-wide transition-all ${
                    activeModel === model.id
                      ? 'bg-blue-100/60 text-blue-700 border border-blue-200/50'
                      : 'bg-slate-100 text-slate-500 border border-slate-200/60 group-hover:bg-blue-50 group-hover:border-blue-100 group-hover:text-blue-600'
                  }`}>
                    {model.priceLabel}
                  </div>
                ) : null}
                <p className={`mt-2 text-[11px] leading-relaxed line-clamp-2 transition-colors ${activeModel === model.id ? 'text-indigo-700' : 'text-slate-500 group-hover:text-slate-600'}`}>{model.desc}</p>
              </div>
            </button>
          ))}
          {hoveredSidebarModel ? (
            <div className='pointer-events-none absolute left-[105%] top-2 z-30 ml-2 hidden w-[320px] lg:block animate-in fade-in slide-in-from-left-2 duration-300'>
              <div className='relative overflow-hidden rounded-[1.75rem] border border-blue-100/50 bg-white/90 p-5 shadow-[0_20px_60px_-10px_rgba(59,130,246,0.15)] backdrop-blur-3xl'>
                <div className='absolute inset-x-0 top-0 h-32 bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.05),_transparent_70%)]' />
                <div className='relative'>
                  <div className='mb-4 flex items-start justify-between gap-4'>
                    <div className='flex min-w-0 items-center gap-3'>
                      <div className='flex h-12 w-12 shrink-0 items-center justify-center rounded-[1rem] bg-gradient-to-br from-blue-50 to-white text-blue-500 shadow-sm border border-blue-100'>
                      ...
                      <div className='min-w-0'>
                        ...
                        <div className='mt-1 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-blue-500'>
                    ...
                    {hoveredSidebarModel.priceLabel ? (
                      <div className='shrink-0 rounded-lg bg-blue-50 border border-blue-200/50 px-2.5 py-1 text-[10px] font-black text-blue-600'>
                        {hoveredSidebarModel.priceLabel}
                      </div>
                    ) : null}
                  </div>
                  <div className='rounded-xl border border-slate-200/50 bg-white/80 px-4 py-3 text-[13px] leading-relaxed text-slate-600 shadow-sm'>
        ...
        <div className='mt-auto shrink-0 border-t border-slate-200/40 p-5 bg-white/30 backdrop-blur-md'>
          <button
            onClick={() => window.location.href = '/file'}
            className='flex w-full items-center justify-center gap-2.5 rounded-[1.25rem] border border-blue-100/60 bg-white/70 px-4 py-3.5 text-[14px] font-bold text-slate-600 shadow-sm transition-all duration-300 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 hover:shadow-[0_4px_20px_rgba(59,130,246,0.1)]'
          >
      ...
      <main className='relative flex flex-1 flex-col overflow-hidden bg-slate-100/40 backdrop-blur-xl'>
        ...
        <div className='absolute top-4 left-6 z-40 flex items-center gap-3'>
          <div className='relative'>
            <button
              ...
              className={
                'inline-flex items-center gap-2 rounded-2xl border border-blue-100/60 bg-white/80 backdrop-blur-xl px-5 py-2.5 text-[13px] font-bold text-slate-600 shadow-sm transition-all duration-300 hover:scale-105 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-600 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50 disabled:transform-none ' +
                (isSessionPanelOpen ? 'bg-blue-50 border-blue-300 text-blue-600 shadow-md' : '')
              }
            >
              <History size={16} />
              历史会话
            ...
            {isSessionPanelOpen && (
              <div className='absolute left-0 top-[120%] mt-2 z-50 w-[300px] rounded-[1.75rem] border border-blue-100/50 bg-white/95 backdrop-blur-3xl p-4 shadow-[0_20px_60px_-10px_rgba(59,130,246,0.15)] overflow-hidden animate-in fade-in slide-in-from-top-2'>
                <div className='absolute inset-0 bg-gradient-to-b from-blue-50/50 to-transparent pointer-events-none' />
                <div className='relative mb-4 px-2'>
                  <div className='text-[14px] font-black text-slate-900'>历史会话</div>
                  <div className='text-[11px] font-bold text-slate-500 mt-0.5 tracking-wide'>仅删除会话，图片视频资源仍保留</div>
                </div>
                  ...
                  {currentTabSessions
                    ...
                    .map((session) => {
                      ...
                      return (
                        <div
                          key={session.id}
                          className={`group rounded-[1.25rem] border px-4 py-3.5 transition-all duration-300 ${
                            isCurrentSession
                              ? 'border-blue-300 bg-blue-50/80 shadow-sm'
                              : 'border-slate-200/50 bg-white/60 hover:bg-white hover:border-blue-200'
                          }`}
                        >
                          <button
                            type='button'
                            onClick={() => {
                              openCreativeSession(activeTab, session.id);
                              setIsSessionPanelOpen(false);
                            }}
                            disabled={isSubmitPending}
                            className='min-w-0 w-full text-left disabled:cursor-not-allowed'
                          >
                            <div className={`truncate text-[14px] font-bold transition-colors ${isCurrentSession ? 'text-blue-700' : 'text-slate-700 group-hover:text-slate-900'}`}>
                              {session.name || '未命名会话'}
                            </div>
                          ...
                          <div className='mt-3.5 flex items-center justify-end gap-2'>
                            <button
                              ...
                              disabled={isSubmitPending}
                              className='rounded-lg border border-slate-200 px-3 py-1.5 text-[11px] font-bold text-slate-500 transition-all hover:border-blue-300 hover:bg-blue-50 hover:text-blue-600 disabled:cursor-not-allowed disabled:opacity-50'
                            >
                              重命名
                            </button>
                            <div className="group/del relative">
                              <button
                                ...
                                disabled={isSubmitPending}
                                className='rounded-lg border border-slate-200 p-1.5 text-slate-500 transition-all hover:border-red-300 hover:bg-red-50 hover:text-red-500'
                              >
                                <Trash2 size={14} />
                              </button>
          ...
          <button
            ...
            disabled={isSubmitPending}
            className='inline-flex items-center gap-2 rounded-2xl border border-blue-100/60 bg-gradient-to-b from-white/90 to-white/70 backdrop-blur-xl px-5 py-2.5 text-[13px] font-bold text-slate-600 shadow-sm transition-all duration-300 hover:scale-105 hover:border-blue-300 hover:from-blue-50 hover:to-white/80 hover:text-blue-600 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50 disabled:transform-none group'
          >
        ...
        {activeTab === 'chat' && (
          <div className='flex flex-1 min-h-0 flex-col overflow-hidden'>
            <div ref={scrollRef} className='flex-1 min-h-0 overflow-y-auto px-8 pb-10 pt-20 space-y-6 custom-scrollbar'>
              {chatMessages.length === 0 && !isGenerating && (
                <div className='flex h-full items-center justify-center'>
                  <div className='group relative max-w-2xl rounded-[3rem] border border-white/80 bg-white/40 p-[2px] shadow-[0_20px_80px_-20px_rgba(59,130,246,0.15)] transition-all duration-500 hover:shadow-[0_40px_100px_-20px_rgba(59,130,246,0.2)] hover:-translate-y-1 overflow-hidden backdrop-blur-md'>
                    <div className='absolute inset-0 bg-gradient-to-br from-blue-100/50 via-white/20 to-sky-100/50 opacity-50 group-hover:opacity-100 transition-opacity duration-500' />
                    <div className='relative rounded-[2.85rem] bg-white/70 backdrop-blur-3xl px-12 py-16 text-center border border-white/50'>
                      <div className='absolute inset-0 bg-[radial-gradient(ellipse_at_top,_rgba(59,130,246,0.08),_transparent_60%)] pointer-events-none rounded-[2.85rem]' />
                      
                      <div className='relative mx-auto mb-8 flex h-28 w-28 items-center justify-center rounded-[2rem] bg-gradient-to-br from-blue-500 via-blue-400 to-sky-400 text-white shadow-[0_15px_40px_-5px_rgba(59,130,246,0.4)] ring-4 ring-white transition-transform duration-500 group-hover:scale-105'>
                        <div className="absolute inset-0 rounded-[2rem] bg-gradient-to-b from-white/30 to-transparent"></div>
                        <div className="relative scale-150">
                          {selectedModel?.icon || <MessageSquare size={32} />}
                        </div>
                      </div>
                      
                      <div className='relative flex items-center justify-center gap-4 text-[12px] font-black uppercase tracking-[0.4em] text-blue-500'>
                        <div className="h-[2px] w-8 rounded-full bg-gradient-to-r from-transparent to-blue-500/50" />
                        当前模型
                        <div className="h-[2px] w-8 rounded-full bg-gradient-to-l from-transparent to-blue-500/50" />
                      </div>
                      
                      <h3 className='relative mt-6 text-4xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-b from-slate-900 to-slate-700 drop-shadow-sm'>
                        {selectedModel?.name || '对话模型'}
                      </h3>
                      
                      <p className='relative mt-6 text-[15px] leading-relaxed text-slate-500 font-medium max-w-lg mx-auto'>
        ...
        {activeTab !== 'chat' && (
          <div ref={scrollRef} className='relative flex-1 min-h-0 overflow-y-auto px-10 pb-10 pt-20 custom-scrollbar'>
                ...
                <div className='group relative max-w-2xl rounded-[3rem] border border-white/80 bg-white/40 p-[2px] shadow-[0_20px_80px_-20px_rgba(59,130,246,0.15)] transition-all duration-500 hover:shadow-[0_40px_100px_-20px_rgba(59,130,246,0.2)] hover:-translate-y-1 overflow-hidden backdrop-blur-md'>
                  ...
                  <div className='relative rounded-[2.85rem] bg-white/70 backdrop-blur-3xl px-12 py-16 text-center border border-white/50'>
                    ...
                    <div className='relative mx-auto mb-8 flex h-28 w-28 items-center justify-center rounded-[2rem] bg-gradient-to-br from-blue-500 via-blue-400 to-sky-400 text-white shadow-[0_15px_40px_-5px_rgba(59,130,246,0.4)] ring-4 ring-white transition-transform duration-500 group-hover:scale-105'>
                      <div className="absolute inset-0 rounded-[2rem] bg-gradient-to-b from-white/30 to-transparent"></div>
                      <div className="relative scale-150">
                        {selectedModel?.icon || (activeTab === 'image' ? <ImageIcon size={32} /> : <Video size={32} />)}
                      </div>
                    </div>
