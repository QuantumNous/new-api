...
function ProgressBar({ startTime }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      <p style={{ fontSize: 12, color: 'var(--semi-color-text-2)', textAlign: 'center', marginTop: 6 }}>已等待 {s}s</p>
    </div>
  );
}
...
export default function ImageStudio() {
  ...
  const doGenerate = useCallback(async (id, params, f, dp) => {
    ...
    } catch (e) { updateTask(id, { status: 'error', error: e.message || '生成失败' }); }
  }, [updateTask]);

  const handleGenerate = useCallback(() => {
    if (!prompt.trim()) return;
    if (!getApiKey()) { Toast.warning('请先选择令牌'); return; }
    const id = nextTaskId();
    const p = prompt.trim();
  ...
  const doEdit = useCallback(async (id, p, imgs, m, s, q) => {
    updateTask(id, { status: 'loading', error: undefined, startTime: Date.now(), retryCount: 0 });
    try {
      const prefix = ASPECT_PREFIX[s] || '';
      const res = await apiEdit({ prompt: prefix + p, images: imgs, mask: m || undefined, size: s, quality: q }, (attempt) => updateTask(id, { retryCount: attempt }));
      const b64s = await Promise.all(res.data.map(d => imageToBase64(d)));
      updateTask(id, { status: 'done', images: b64s });
      saveHistoryDB({ type: 'edit', prompt: p, params: { size: s, quality: q }, images: b64s, timestamp: Date.now() });
    } catch (e) { updateTask(id, { status: 'error', error: e.message || '编辑失败' }); }
  }, [updateTask]);
  ...
  const handleEdit = useCallback(() => {
    ...
    const imgs = [...eImages]; const m = eMask; const s = eSize; const q = eQuality;
    addTask({ id, type: 'edit', images: [], format: 'png', prompt: p, status: 'loading', startTime: Date.now() });
    retryMap.current[id] = () => doEdit(id, p, imgs, m, s, q);
    doEdit(id, p, imgs, m, s, q);
  }, [ePrompt, eImages, eMask, eSize, eQuality, addTask, doEdit]);
  ...
  const searchServerImages = useCallback(async () => {
    ...
    try {
      ...
      if (res.data.success && res.data.data?.length > 0) {
        ...
        setSearchError('未找到该请求ID对应的图片');
      } else {
        setSearchError(res.data.message || '查询失败');
      }
    } catch {
      setSearchError('查询失败，请检查请求ID是否正确');
    } finally {
      setSearching(false);
    }
  }, [searchRequestId]);
        ...
        <div style={{ width: 360, flexShrink: 0, background: 'var(--semi-color-bg-1)', borderRadius: 12, border: '1px solid var(--semi-color-border)', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: 16 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: 'block', fontSize: 14, color: 'var(--semi-color-text-1)', marginBottom: 8, fontWeight: 500 }}>令牌</label>
              <select
              ...
              >
                <option value="">{getApiKey() ? '已配置令牌' : '请选择令牌'}</option>
                {tokens.map(t => (
                  <option key={t.id} value={t.id}>{t.name}</option>
                ))}
              </select>
              {getApiKey() && <p style={{ fontSize: 12, color: 'var(--semi-color-success)', marginTop: 4 }}>● 令牌已就绪</p>}
            </div>

            {mode === 'generate' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 14, color: 'var(--semi-color-text-1)', marginBottom: 8, fontWeight: 500 }}>描述画面</label>
                  <textarea className="is-panel-input" style={{ height: 120 }} placeholder="描述你想要的画面..." value={prompt} onChange={e => setPrompt(e.target.value)} onKeyDown={e => { if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') handleGenerate(); }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 14, color: 'var(--semi-color-text-1)', marginBottom: 8, fontWeight: 500 }}>画面比例</label>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 14, color: 'var(--semi-color-text-1)', marginBottom: 8, fontWeight: 500 }}>生成质量</label>
                  <div className="is-opt-group">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 14, color: 'var(--semi-color-text-1)', marginBottom: 8, fontWeight: 500 }}>格式</label>
                    <div className="is-opt-group">
                      {['png','jpeg','webp'].map(f => <button key={f} onClick={() => setFmt(f)} className={`is-opt-item ${fmt === f ? 'active' : ''}`}>{f.toUpperCase()}</button>)}
                    </div>
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: 14, color: 'var(--semi-color-text-1)', marginBottom: 8, fontWeight: 500 }}>数量 <span style={{ color: 'var(--semi-color-primary)' }}>{count}</span></label>
                ...
                <button className="is-btn-generate" disabled={!prompt.trim()} onClick={handleGenerate}>开始生成</button>
                <p style={{ fontSize: 12, color: 'var(--semi-color-text-2)', textAlign: 'center' }}>⌘ + Enter 快速生成</p>
              </div>
            )}
            ...
            {mode === 'edit' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 14, color: 'var(--semi-color-text-1)', marginBottom: 8, fontWeight: 500 }}>参考图片 <span style={{ fontWeight: 400, color: 'var(--semi-color-text-2)' }}>最多 10 张</span></label>
                  {ePreviews.length > 0 && (
                  ...
                  {eImages.length < 10 && (
                    <div onClick={() => imgRef.current?.click()} style={{ height: 80, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: '1px dashed var(--semi-color-border)', background: 'var(--semi-color-fill-0)', color: 'var(--semi-color-text-2)' }}>
                      {eImages.length === 0 ? '点击上传图片' : '继续添加图片'}
                    </div>
                  )}
                  <input ref={imgRef} type="file" accept="image/*" multiple style={{ display: 'none' }} onChange={handleImgChange} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 14, color: 'var(--semi-color-text-1)', marginBottom: 8, fontWeight: 500 }}>编辑区域 <span style={{ fontWeight: 400, color: 'var(--semi-color-text-2)' }}>可选</span></label>
                  <div onClick={() => maskRef.current?.click()} style={{ height: 64, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: '1px dashed var(--semi-color-border)', background: 'var(--semi-color-fill-0)', color: 'var(--semi-color-text-2)', fontSize: 13 }}>
                    {eMask ? eMask.name : '上传 PNG 蒙版，透明区域将被 AI 重绘'}
                  </div>
                  <input ref={maskRef} type="file" accept="image/png" style={{ display: 'none' }} onChange={e => setEMask(e.target.files?.[0] || null)} />
                  {eMask && <button onClick={() => setEMask(null)} style={{ fontSize: 12, color: 'var(--semi-color-text-2)', background: 'none', border: 'none', cursor: 'pointer', marginTop: 4 }}>移除蒙版</button>}
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 14, color: 'var(--semi-color-text-1)', marginBottom: 8, fontWeight: 500 }}>描述最终效果</label>
                  <textarea className="is-panel-input" style={{ height: 96 }} placeholder="描述你想要的最终图片效果..." value={ePrompt} onChange={e => setEPrompt(e.target.value)} onKeyDown={e => { if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') handleEdit(); }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 14, color: 'var(--semi-color-text-1)', marginBottom: 8, fontWeight: 500 }}>画面比例</label>
                  <div className="is-opt-group">
                ...
                <button className="is-btn-generate" disabled={!ePrompt.trim() || eImages.length === 0} onClick={handleEdit}>开始编辑</button>
              </div>
            )}
          </div>
        </div>
          ...
          tasks.length === 0 ? (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ color: 'var(--semi-color-text-2)', fontSize: 15, marginTop: 12 }}>在左侧输入描述，开始创作</p>
              </div>
            </div>
          ) : (
                ...
                <div key={task.id} className="is-animate-in">
                    ...
                    <div style={{ borderRadius: 12, background: 'var(--semi-color-fill-0)', border: '1px solid var(--semi-color-border)', padding: 24 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 0' }}>
                        <div style={{ textAlign: 'center', width: '100%', maxWidth: 240 }}>
                          ...
                          <p style={{ fontSize: 14, color: 'var(--semi-color-text-2)', marginTop: 16 }}>AI 正在创作中...</p>
                          {task.retryCount > 0 && <p style={{ fontSize: 13, color: 'var(--semi-color-warning)', marginTop: 4 }}>第 {task.retryCount} 次重试...</p>}
                  ...
                  {task.status === 'error' && (
                    <div style={{ borderRadius: 12, background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.1)', padding: 16 }}>
                      ...
                      <button onClick={() => retryMap.current[task.id]?.()} style={{ padding: '8px 16px', background: 'rgba(239,68,68,0.1)', color: 'rgb(248,113,113)', fontSize: 14, borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 500 }}>重试</button>
                    </div>
                  )}
                          ...
                          <div key={i} className="is-img-card" style={{ position: 'relative', cursor: 'pointer' }} onClick={() => setRefPreview(`data:${mime};base64,${b64}`)}>
                            ...
                            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.5), transparent)', opacity: 0, transition: 'opacity 0.2s', display: 'flex', alignItems: 'flex-end', justifyContent: 'flex-end', padding: 12 }} onMouseEnter={e => e.currentTarget.style.opacity = 1} onMouseLeave={e => e.currentTarget.style.opacity = 0}>
                              <div style={{ display: 'flex', gap: 8 }}>
                                <button onClick={(e) => { e.stopPropagation(); sendToEdit(b64); }} style={{ padding: '6px 12px', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(8px)', borderRadius: 8, color: '#fff', fontSize: 12, fontWeight: 500, border: 'none', cursor: 'pointer' }}>编辑</button>
                                <button onClick={(e) => { e.stopPropagation(); downloadImg(b64, task.format, i); }} style={{ padding: '6px 12px', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(8px)', borderRadius: 8, color: '#fff', fontSize: 12, fontWeight: 500, border: 'none', cursor: 'pointer' }}>下载</button>
                              </div>
                            </div>
                          </div>
                        );
          ...
          <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '12px 32px 0', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  {history.length > 0 && <span style={{ fontSize: 14, color: 'var(--semi-color-text-2)' }}>{history.length} 条创作记录</span>}
                  <button onClick={loadHist} disabled={refreshing} style={{ fontSize: 14, color: 'var(--semi-color-primary)', background: 'none', border: 'none', cursor: 'pointer' }}>{refreshing ? '刷新中...' : '刷新'}</button>
                </div>
                {history.length > 0 && <button onClick={async () => { await clearHistoryDB(); setHistory([]); setPreview(null); }} style={{ fontSize: 14, color: 'var(--semi-color-text-2)', background: 'none', border: 'none', cursor: 'pointer' }}>清空本地</button>}
              </div>
            </div>
              ...
              {serverImages && serverImages.length > 0 && (
                <div style={{ marginTop: 12 }}>
                  <p style={{ fontSize: 13, color: 'var(--semi-color-text-2)', marginBottom: 8 }}>
                    服务端图片 · {serverImages.length} 张 · 请求ID: {serverImages[0].request_id}
                  </p>
                  ...
                  <p style={{ fontSize: 11, color: 'var(--semi-color-text-2)', marginTop: 8 }}>
                    保存时间: {new Date(serverImages[0].created_at * 1000).toLocaleString('zh-CN')} · 图片保留7天
                  </p>
                </div>
              )}
            </div>
            {history.length === 0 ? (
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 300 }}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 40, color: 'var(--semi-color-text-2)', opacity: 0.2 }}>◷</div>
                  <p style={{ color: 'var(--semi-color-text-2)', fontSize: 15, marginTop: 12 }}>暂无本地历史记录</p>
                  <p style={{ color: 'var(--semi-color-text-2)', fontSize: 13, marginTop: 4 }}>可在上方通过请求ID查询服务端保存的图片</p>
                </div>
              </div>
            ) : (
              <div style={{ padding: '24px 32px 32px' }}>
                <p style={{ fontSize: 13, color: 'var(--semi-color-text-2)', marginBottom: 12 }}>本地历史</p>
                    ...
                    <div key={item.id ?? i} className="is-img-card" onClick={() => setPreview(item)} style={{ cursor: 'pointer', breakInside: 'avoid', marginBottom: 16 }}>
                      ...
                      <div style={{ padding: 12 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                          <span style={{ fontSize: 12, padding: '2px 8px', borderRadius: 4, fontWeight: 500, background: item.type === 'generate' ? 'var(--semi-color-primary-light-default)' : 'rgba(16,185,129,0.1)', color: item.type === 'generate' ? 'var(--semi-color-primary)' : 'rgb(52,211,153)' }}>{item.type === 'generate' ? '生成' : '编辑'}</span>
                          {item.images.length > 1 && <span style={{ fontSize: 12, color: 'var(--semi-color-text-2)' }}>{item.images.length} 张</span>}
                        </div>
              ...
              {preview.images.map((b64, j) => (
                <div key={j} style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', border: '1px solid var(--semi-color-border)', background: 'var(--semi-color-fill-0)', minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.4), transparent)', opacity: 0, transition: 'opacity 0.2s', display: 'flex', alignItems: 'flex-end', justifyContent: 'flex-end', padding: 12 }} onMouseEnter={e => e.currentTarget.style.opacity = 1} onMouseLeave={e => e.currentTarget.style.opacity = 0}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => { sendToEdit(b64); }} style={{ padding: '6px 12px', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(8px)', borderRadius: 8, color: '#fff', fontSize: 12, fontWeight: 500, border: 'none', cursor: 'pointer' }}>编辑</button>
                      <button onClick={() => downloadImg(b64, 'png', j)} style={{ padding: '6px 12px', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(8px)', borderRadius: 8, color: '#fff', fontSize: 12, fontWeight: 500, border: 'none', cursor: 'pointer' }}>下载</button>
                    </div>
                  </div>
                </div>
              ))}
