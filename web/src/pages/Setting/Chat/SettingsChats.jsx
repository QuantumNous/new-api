...
export default function SettingsChats(props) {
      ...
      modalFormRef.current.validate()
        .then((values) => {
          if (isEdit) {
            ...
            const maxId = chatConfigs.length > 0 ? Math.max(...chatConfigs.map(c => c.id)) : -1;
            const newConfig = {
              id: maxId + 1,
              name: values.name,
              url: values.url
  ...
  const filteredConfigs = chatConfigs.filter(config => 
    !searchText || config.name.toLowerCase().includes(searchText.toLowerCase())
  );
  ...
  return (
    <Spin spinning={loading}>
      <Space vertical style={{ width: '100%' }}>
        <Form.Section text={t('聊天设置')}>
          ...
          <div style={{ marginBottom: 16 }}>
            <span style={{ marginRight: 16, fontWeight: 600 }}>{t('编辑模式')}:</span>
            <RadioGroup 
              type='button' 
              value={editMode} 
              onChange={(e) => {
        ...
        <Space>
          <Button type='primary' onClick={onSubmit}>{t('保存聊天设置')}</Button>
        </Space>
      </Space>
      ...
      >
        <Form
          getFormApi={(api) => (modalFormRef.current = api)}
        >
          <Form.Input
            field='name'
            label={t('聊天应用名称')}
          ...
          <Banner
            type='info'
            description={t('提示：链接中的{key}将被替换为API密钥，{address}将被替换为服务器地址')}
            style={{ marginTop: 16 }}
          />
        </Form>
      </Modal>
