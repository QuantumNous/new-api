...
const emailLoginButtonName =
  /(Continue with Email or Username|使用 邮箱或用户名 登录)/;
const adminUsernamePlaceholder =
  /(Please enter the admin username|请输入管理员用户名)/;
const adminPasswordPlaceholder =
  /(Please enter the admin password|请输入管理员密码)/;
const adminConfirmPasswordPlaceholder =
  /(Please confirm the admin password|请确认管理员密码)/;
const loginUsernamePlaceholder =
  /(Please enter your username or email address|请输入您的用户名或邮箱地址)/;
...
test.describe.serial('Setup and login baseline', () => {
  test('anonymous home navigation redirects to setup when the system is uninitialized', async ({
    page,
  }) => {
    await page.goto('/');
  ...
  test('setup wizard can initialize the system and root user can sign in', async ({
    page,
  }) => {
  ...
  test('system settings show docker image version and update info', async ({
    page,
  }) => {
  ...
  test('contact page submissions appear in feedback management and logo can be uploaded', async ({
    page,
  }) => {
    ...
    await page
      .getByRole('button', { name: /采购咨询|Sales Inquiry|Consulting/ })
      .click();
    await expect(
      page.getByText(/当前反馈类型：|Current feedback type:/),
    ).toBeVisible();
    await expect(page.getByLabel(/反馈类型|Feedback Type/)).toHaveCount(0);
    await page
      .getByPlaceholder(/请输入你的称呼|name/i)
      .fill('E2E Contact User');
    await page
      .getByPlaceholder(/请输入可联系的邮箱|email/i)
      .fill('e2e-feedback@example.com');
    await page
      .getByPlaceholder(
        /适用于套餐、计费、部署、私有化或商务合作咨询|deployment|billing/i,
      )
      .fill(
        'Submitting a bug report from the e2e contact page should make it visible to administrators.',
      );
    await page
      .getByRole('button', { name: /提交反馈|Submit feedback/ })
      .click();
    await expect(page.getByText(/反馈已提交|submitted/i)).toBeVisible();

    await loginAsRoot(page);
    await page.goto('/contact');
    await expect(page.getByPlaceholder(/请输入你的称呼|name/i)).toHaveValue(
      adminUsername,
    );
    ...
    await expect(page.getByText(/已选择文件|Selected file/)).toContainText(
      'logo.png',
    );
    await page
      .getByRole('button', { name: /上传并设置 Logo|Upload and set Logo/ })
      .click();
    await expect(page.getByText(/\/uploads\/branding\/logo-/)).toBeVisible();
  });

  test('sticky multi-key channels show the sticky mode in multi-key management', async ({
    page,
    request,
  }) => {
    const rootUserId = await loginAsRootApi(request);
    const headers = {
      'New-API-User': String(rootUserId),
    };
    const channelName = `sticky-e2e-${Date.now()}`;
